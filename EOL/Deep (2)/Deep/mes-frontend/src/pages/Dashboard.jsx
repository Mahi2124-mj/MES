...
// 2026-08-31 — Zone OEE drill-down for MANAGEMENT.  Click a zone's OEE tile on
// the dashboard header and this modal explains WHY that zone's OEE is low:
//   • the A/P/Q split (which OEE pillar is dragging — downtime vs slow vs NG),
//   • a loss Pareto (which loss category cost the most minutes — the reason),
//   • a line-wise table (which line is the culprit + any LIVE andon reason),
//     each row click-through to that line's Management Dashboard for the full
//     recorded remarks / breakdown reasons.
// Read-only: computed entirely from the realtime data the dashboard already
// holds in rtMapRef — NO backend call, nothing on the counting/OEE path touched.
function ZoneOeeDetailModal({ zone, lines, getRt, onClose }) {
  const LOSS_CATS = [
    { key: "breakdown",   label: "Breakdown",   color: "#ef4444", why: "Maintenance / Tool-room downtime" },
    { key: "quality",     label: "Quality",     color: "#f97316", why: "NG / rejection / rework" },
    { key: "material",    label: "Material",    color: "#eab308", why: "material / part shortage" },
    { key: "setup",       label: "Setup",       color: "#84cc16", why: "setup / warm-up" },
    { key: "change_over", label: "Change Over", color: "#06b6d4", why: "model change-over" },
    { key: "speed",       label: "Speed Loss",  color: "#3b82f6", why: "slow cycles / minor stops" },
    { key: "others",      label: "Others",      color: "#8b5cf6", why: "other losses" },
  ];
  // Re-read the ref snapshot every 5 s so the drill-down stays live while open.
  ...
  const rtMap = getRt() || {};
  const zoneLines = (lines || []).filter(l => String(l.zone_id) === String(zone.zone_id));

  const rows = zoneLines.map(l => {
    const cell   = rtMap[l.id] || {};
    const rt     = cell.rt || {};
    const losses = LOSS_CATS.map(c => ({ ...c, sec: rt[`loss_${c.key}_seconds`] || 0 }));
    const topLoss = losses.filter(c => c.sec > 0).sort((a, b) => b.sec - a.sec)[0] || null;
    return {
      id: l.id,
      name:    l.line_name || "—",
      hasData: (rt.overall_oee || 0) > 0,
      oee:   rt.overall_oee   || 0,
      avail: rt.availability  || 0,
      perf:  rt.performance   || 0,
      qual:  rt.quality_oee   || 0,
      ok:    rt.ok_count      || 0,
      ng:    rt.ng_count      || 0,
  ...
  const avg = (k) => withData.length ? withData.reduce((s, r) => s + Number(r[k] || 0), 0) / withData.length : 0;
  const zAvail = avg("avail"), zPerf = avg("perf"), zQual = avg("qual"), zOee = avg("oee");
  const runningN = rows.filter(r => r.running).length;

  // Loss Pareto aggregated across the whole zone.
  const totals = LOSS_CATS.map(c => ({
    ...c, sec: rows.reduce((s, r) => s + (r.losses.find(x => x.key === c.key)?.sec || 0), 0),
  }));
  const totalLoss = totals.reduce((s, c) => s + c.sec, 0);
  const pareto = totals.filter(c => c.sec > 0).sort((a, b) => b.sec - a.sec)
    .map(c => ({ ...c, min: c.sec / 60, pct: totalLoss ? (c.sec / totalLoss) * 100 : 0 }));

  // Headline: the lowest OEE pillar is the biggest drag.
  const pillars = [
    { label: "Availability", val: zAvail, why: "downtime / breakdown / stoppage" },
    { label: "Performance",  val: zPerf,  why: "slow cycles / minor stops / speed loss" },
    { label: "Quality",      val: zQual,  why: "NG / rejection / rework" },
  ];
  const driver = [...pillars].filter(p => p.val > 0).sort((a, b) => a.val - b.val)[0] || null;

  const band   = (v) => v >= 75 ? "#16a34a" : v >= 60 ? "#d97706" : "#dc2626";
  const fmtMin = (m) => {
    if (m < 60) return `${m.toFixed(0)}m`;
    let hh = Math.floor(m / 60), mm = Math.round(m % 60);
    if (mm === 60) { hh += 1; mm = 0; }   // avoid "2h 60m" when minutes round up
    return `${hh}h ${mm}m`;
  };
  const liveAndon = rows.filter(r => r.andon);
  const openLine = (name) => window.open(
    `/${encodeURIComponent(String(name).trim().replace(/\s+/g, "-"))}/MANAGEMENT`, "_blank");

  const oeeBand = band(Number(zOee) || 0);
  const th = { textAlign: "left", padding: "8px 10px", fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: ".04em", borderBottom: "1px solid #e2e8f0", whiteSpace: "nowrap" };
  const td = { padding: "8px 10px", fontSize: 13, color: "#0f172a", borderBottom: "1px solid #f1f5f9", whiteSpace: "nowrap" };

  return (
    <div onClick={e => e.target === e.currentTarget && onClose()}
         style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,.55)", zIndex: 4000,
                  display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "40px 16px", overflowY: "auto" }}>
      <div style={{ background: "#fff", borderRadius: 16, width: "min(920px,100%)", boxShadow: "0 20px 60px rgba(0,0,0,.35)", overflow: "hidden" }}>
        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 22px", borderBottom: "1px solid #e2e8f0" }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: ".06em" }}>{zone.zone_name} · OEE Detail</div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginTop: 2 }}>
              <div style={{ fontSize: 36, fontWeight: 600, color: oeeBand, lineHeight: 1 }}>{withData.length ? `${zOee.toFixed(1)}%` : "—"}</div>
              <div style={{ fontSize: 12, color: "#64748b" }}>{withData.length} line{withData.length !== 1 ? "s" : ""} with data · {runningN} running</div>
            </div>
          </div>
          <button onClick={onClose} style={{ border: "none", background: "#f1f5f9", width: 34, height: 34, borderRadius: 9, fontSize: 18, cursor: "pointer", color: "#475569" }}>✕</button>
        </div>

        <div style={{ padding: "18px 22px", maxHeight: "72vh", overflowY: "auto" }}>
          {/* Driver callout */}
          {driver && (
            <div style={{ background: `${band(driver.val)}12`, border: `1px solid ${band(driver.val)}40`, borderRadius: 10, padding: "12px 14px", marginBottom: 18 }}>
              <span style={{ fontSize: 13, color: "#0f172a" }}>
                Sabse bada drag: <b style={{ color: band(driver.val) }}>{driver.label} {driver.val.toFixed(1)}%</b> — {driver.why}.
                {pareto[0] && <> Main loss: <b>{pareto[0].label}</b> ({fmtMin(pareto[0].min)}, {pareto[0].pct.toFixed(0)}%).</>}
              </span>
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 22 }}>
            {pillars.map(p => {
              const isDrag = driver && p.label === driver.label;
              return (
                <div key={p.label} style={{ border: `1px solid ${isDrag ? band(p.val) + "66" : "#e2e8f0"}`, borderRadius: 10, padding: "12px 14px", background: isDrag ? `${band(p.val)}0a` : "#fff" }}>
                  <div style={{ fontSize: 11, color: "#64748b", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".04em" }}>{p.label}{isDrag ? " ⚠" : ""}</div>
                  <div style={{ fontSize: 24, fontWeight: 600, color: band(p.val), margin: "2px 0 6px" }}>{withData.length ? `${p.val.toFixed(1)}%` : "—"}</div>
                  <div style={{ height: 6, borderRadius: 4, background: "#f1f5f9", overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${Math.min(100, p.val)}%`, background: band(p.val) }} />
                  </div>
                </div>
              );
            })}
          ...
          {/* Loss Pareto — the "why" */}
          <div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", marginBottom: 10 }}>Loss Pareto — OEE kyun kam hai</div>
          {pareto.length === 0 ? (
            <div style={{ fontSize: 13, color: "#64748b", background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 8, padding: "12px 14px", marginBottom: 22 }}>
              Is shift koi recorded loss nahi — OEE gap plan/timing ka hai, koi breakdown/quality loss registered nahi.
            </div>
          ) : (
              ...
              {pareto.map(c => (
                <div key={c.key} style={{ marginBottom: 9 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 3 }}>
                    <span style={{ color: "#0f172a", fontWeight: 600 }}>{c.label} <span style={{ color: "#94a3b8", fontWeight: 400 }}>· {c.why}</span></span>
                    <span style={{ color: "#475569" }}>{fmtMin(c.min)} · {c.pct.toFixed(0)}%</span>
                  </div>
                  <div style={{ height: 10, borderRadius: 5, background: "#f1f5f9", overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${Math.max(2, c.pct)}%`, background: c.color }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Line-wise table */}
          <div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", marginBottom: 8 }}>Line-wise (kaun si line gira rahi hai)</div>
          <div style={{ overflowX: "auto", border: "1px solid #e2e8f0", borderRadius: 10 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 640 }}>
              <thead><tr>
                <th style={th}>Line</th><th style={th}>OEE</th><th style={th}>Avail</th>
                <th style={th}>Perf</th><th style={th}>Qual</th><th style={th}>OK/NG</th>
                <th style={th}>Top Loss</th><th style={th}>Reason / Status</th><th style={th}></th>
              </tr></thead>
              <tbody>
                {[...rows].sort((a, b) => (a.hasData ? a.oee : 999) - (b.hasData ? b.oee : 999)).map(r => (
                  <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => openLine(r.name)}
                      title="Is line ki poori Management Dashboard (remarks/breakdown reasons) kholein">
                    <td style={{ ...td, fontWeight: 600 }}>{r.name}</td>
                    <td style={{ ...td, fontWeight: 700, color: r.hasData ? band(r.oee) : "#94a3b8" }}>{r.hasData ? `${r.oee.toFixed(1)}%` : "—"}</td>
                    <td style={{ ...td, color: r.hasData ? band(r.avail) : "#94a3b8" }}>{r.hasData ? `${r.avail.toFixed(0)}%` : "—"}</td>
                    <td style={{ ...td, color: r.hasData ? band(r.perf) : "#94a3b8" }}>{r.hasData ? `${r.perf.toFixed(0)}%` : "—"}</td>
          ...
          <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 10 }}>
            Kisi bhi line pe click → uski Management Dashboard khulegi (poore recorded remarks + breakdown reasons wahin). Andon-covered lines pe breakdown = Maintenance + Tool-room downtime.
          </div>
        </div>
      </div>
    </div>
...
function LineCard({ line, globalStatus, token, user, onRtUpdate, onNpdUpdate }) {
  ...
  const [npdFormShifts, setNpdFormShifts] = useState([]);   // multi-select: A and/or B
  ...
  // 2026-06-19 — production shifts for the OT popover (was hardcoded ['A','B']).
  // Loaded from the line's OT config (production shifts only) when the menu
  // opens; falls back to ['A','B'] if the fetch fails or returns nothing.
  const [otShifts,    setOtShifts]    = useState(["A", "B"]);
  ...
  // Load this line's real production shifts for the OT popover when it opens.
  useEffect(() => {
    if (!showOtMenu || !line?.id) return;
    api.get(`/api/lines/${line.id}/ot-config`, token)
      .then(rows => {
        const names = Array.isArray(rows) ? rows.map(r => r.shift_name).filter(Boolean) : [];
        if (names.length) setOtShifts(names);
      })
      .catch(() => {});
  }, [showOtMenu, line?.id, token]);
  ...
  const markNPD = async () => {
    setNpdLoading(true);
    try {
      // Multi-shift NPD: one entry PER selected shift (backend keys on
      // line+date+shift).  Hourly-slot granularity only applies when a
      // SINGLE shift is selected; picking 2+ shifts (e.g. A and B) marks
      // each as a full-shift non-production.  Empty selection = whole day.
      const shifts = npdFormShifts.length > 0 ? npdFormShifts : [null];
      const slots  = (npdFormShifts.length === 1 && npdFormSlots.length > 0)
                       ? npdFormSlots : null;
  ...
  const plan = (isNPD || isOfflineShift) ? 0 : (() => {
    ...
    for (const b of (detail?.breaks || [])) {
      // Only subtract breaks that belong to THIS shift.  A night-shift break
      // (e.g. 01:00, flagged crosses_midnight) applied to the day shift gets a
      // +1-day end window that swallows the whole morning → plan pinned to 0.
      const aps = (b.applies_to_shifts || "").split(",").map(s => s.trim()).filter(Boolean);
      if (aps.length && !aps.includes(shift)) continue;
    ...
    const cb         = shiftTotal > 0 ? Math.min(shiftTotal, computed) : computed;
    // 2026-09-15 — never show LESS than the DB's plan-so-far. On register-mode
    // lines (Loop Pipe / Sub-Assembly) the clock-based math breaks — their
    // B-shift record_date rolls to the next calendar day, so shiftStart lands in
    // the FUTURE → elapsed 0 → plan 0. Floor it with shift_plan_completed (now
    // computed server-side for those zones). Normal lines: cb ≥ spc, so no change.
    return Math.max(cb, rt.shift_plan_completed || 0);
  })();
    ...
    <>
      ...
      >
        ...
        {canToggleNPD && showNpdForm && (() => {
          ...
          const slotsForShift = (detail?.hourly_slots || []).filter(s => s.shift_name === (npdFormShifts[0] || ""));
          ...
          return (
            <div style={{ background: "rgba(217,119,6,.06)", border: "1px solid rgba(217,119,6,.25)", borderRadius: 10, padding: "14px 16px", fontSize: 12 }}>
              ...
              {/* Step 2: Shift (MULTI-SELECT — A and/or B together) */}
              {npdStep === 2 && (() => {
                const toggleShift = (name) => setNpdFormShifts(prev =>
                  prev.includes(name) ? prev.filter(x => x !== name) : [...prev, name]);
                const nSel = npdFormShifts.length;
                return (
                <div>
                  <div style={{ fontWeight: 700, color: "#92400e", marginBottom: 4 }}>Select shift(s) — {npdFormDate}</div>
                  <div style={{ fontSize: 10, color: "#92400e", marginBottom: 8 }}>Tap to pick one or more (e.g. A and B together)</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
                    {lineShifts.length === 0
                      ? <div style={{ color: "#94a3b8", fontSize: 11 }}>No shifts configured for this line</div>
                      : lineShifts.map(s => {
                        const sel = npdFormShifts.includes(s.shift_name);
                        return (
                        <button key={s.shift_name}
                          onClick={() => toggleShift(s.shift_name)}
                          style={{
                            textAlign: "left", padding: "8px 12px", borderRadius: 7,
                            border: sel ? "1.5px solid #d97706" : "1.5px solid #e2e8f0",
                            background: sel ? "rgba(217,119,6,.12)" : "#fff",
                            color: "#0f172a", fontSize: 12, fontWeight: 600, cursor: "pointer",
                          }}>
                          <span style={{ marginRight: 6, fontWeight: 800, color: sel ? "#d97706" : "#94a3b8" }}>{sel ? "☑" : "☐"}</span>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => setNpdStep(1)}
                      style={{ padding: "8px 14px", borderRadius: 7, border: "1px solid #e2e8f0", background: "#f8fafc", fontSize: 12, fontWeight: 600, cursor: "pointer", color: "#64748b" }}>
                      ← Back</button>
                    <button disabled={nSel === 0 || npdLoading}
                      onClick={() => { if (nSel === 1) { setNpdFormSlots([]); setNpdStep(3); } else { markNPD(); } }}
                      style={{ flex: 1, padding: "8px 0", borderRadius: 7, border: "none",
                        background: nSel === 0 ? "#e2e8f0" : "#d97706", color: "#fff", fontSize: 12, fontWeight: 700,
                        cursor: nSel === 0 ? "not-allowed" : "pointer" }}>
                      {nSel <= 1 ? "Next → Hours" : `Mark ${nSel} shifts Non-Production`}</button>
                  </div>
                </div>
                );
              })()}
              ...
              {npdStep === 3 && (
                <div>
                  <div style={{ fontWeight: 700, color: "#92400e", marginBottom: 4 }}>
                    Select non-production hours — Shift {npdFormShifts[0]} · {npdFormDate}
                  </div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", maxWidth: 820 }}>
          ...
          <Btn onClick={() => window.open(
                 `/${encodeURIComponent(
                    String(line.line_name || line.id).trim().replace(/\s+/g, "-"))}/MANAGEMENT`,
                 "_blank")} variant="primary">🖥 Management Dashboard</Btn>
          {/* 65" wall TV shortcut — the multi-machine CT wallboard.
              The summary/histogram dashboard was folded into the management
              view as the "CT Distribution" toggle, so no separate button.
              2026-08-11 — the link now carries the LINE NAME instead of its id
              (/wallboard/left/YHB-SS), so the opened tab and its URL both say
              which line it is.  Numeric URLs still work, so old bookmarks and
              the wall TVs are unaffected; the backend resolves either form. */}
          <Btn onClick={() => window.open(
                 `/${encodeURIComponent(
                    String(line.line_name || line.id).trim().replace(/\s+/g, "-"))}/SUPERVISOR`,
                 "_blank")} variant="ghost"
                title="Open the multi-machine CT wallboard (Supervisor Dashboard)">
...
export default function Dashboard() {
  ...
  const [zoneDetail,   setZoneDetail]   = useState(null);  // clicked zone OEE tile → management drill-down
  ...
  useEffect(() => {
    if (!token) return;
    const fetchPending = () => {
      api.get("/api/prod-breakdown-slips/pending", token)
  ...
  const recomputeSummary = useCallback(() => {
    ...
    const npdLineNames = npdPairs.map(([id, v]) => {
      ...
      return { name: l?.line_name || "—", reason: v.reason || null };
    });
    ...
    <>
      <style>{`
        ...
        /* Phone: the 40px side padding (a desktop value) wasted a big strip on
           each edge and squeezed the cards into a narrow centre column. Tighten
           the gutters, let the topbar keep just enough left room for the fixed
                ...
                {zones.map(z => {
                  ...
                  return (
                    <div key={z.id} className={`zone-drop-item${checked?" sel":""}`} onClick={() => toggleZone(zid)}>
                      <input type="checkbox" checked={checked} readOnly style={{ pointerEvents:"none", accentColor: z.color || "#1e40af" }} />
                      {/* colour dot so the filter list reads the same as the
                          sections it filters */}
                      {z.color && <span style={{ display:"inline-block", width:8, height:8,
                                                  borderRadius:2, background:z.color,
        ...
        <div className="db-body">
          ...
          {pendingBd.length > 0 && (() => {
            ...
            for (const b of pendingBd) {
              // 2026-09-01 — source switched to the 9965 andon slips
              // (/api/prod-breakdown-slips/pending): rows carry `line`/`zone`
              // (names), not line_id/line_name.  Map them so the tiles render.
              const k = b.line || b.line_name || b.line_id || "unknown";
              if (!byLine[k]) byLine[k] = {
                line_id: b.line || b.line_id, line_name: b.line || b.line_name,
                zone_name: b.zone || b.zone_name, items: [],
              };
                  ...
                  {groups.map(g => {
                    ...
                    return (
                      <div key={g.line_id || g.line_name}
                        ...
                        <button
                          onClick={() => { window.location.href = "/prod-breakdown-slip"; }}
                          style={{
                          ...
                          <div style={{ minWidth: 0, flex: 1 }}>
                            ...
                            <div style={{ fontSize:10, color:"#94a3b8", marginTop: 2 }}>
                              {g.items.length} pending slip{g.items.length>1?"s":""}
                              {" · "}tap to open Break Down Slip
                            </div>
                          </div>
                          <span style={{
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:16, marginBottom:36 }}>
              ...
              {(summary.zoneOees || []).map(z => {
                ...
                const oeeBand = !z.has_data ? "#94a3b8"
                              ...
                              : "#dc2626";
                return (
                  <div key={`zone-oee-${z.zone_id}`}
                    onClick={() => z.has_data && setZoneDetail(z)}
                    title={z.has_data ? "Click: is zone me OEE kyun kam hai + valid reason" : "No live data yet"}
                    style={{
                    background:"#fff", border:`1px solid ${oeeBand}33`, borderRadius:12,
                    padding:20, position:"relative", overflow:"hidden",
                    boxShadow:"0 1px 3px rgba(0,0,0,0.05)",
                    cursor: z.has_data ? "pointer" : "default",
                    transition:"box-shadow .12s, transform .12s",
                  }}
                    onMouseEnter={e => { if (z.has_data) { e.currentTarget.style.boxShadow = "0 6px 18px rgba(0,0,0,0.12)"; e.currentTarget.style.transform = "translateY(-2px)"; } }}
                    ...
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                      ...
                      {z.has_data && <span style={{ fontSize:10, color:oeeBand, fontWeight:700, letterSpacing:".03em" }}>Details ▸</span>}
                    </div>
                  </div>
                );
              })}
              ...
              {grouped.map(({ zone, zoneLines }) => (
                zone && zoneLines.length > 0 && (
                  <div className="zone-section" key={zone.id}>
                    {/* 2026-08-14 — zone colour set in Admin → Zones.  Falls back
                        to the stylesheet's default when a zone has none, so a
                        zone nobody has coloured looks exactly as it did before. */}
                    <div className="zone-lbl">
                      <div className="zone-lbl-text"
                           style={zone.color ? { color: zone.color } : undefined}>
                        {zone.zone_name || zone.name}
                      </div>
                      <div className="zone-lbl-badge"
                           style={zone.color ? { background:`${zone.color}1a`,
                                                  color: zone.color,
                                                  borderColor:`${zone.color}59` } : undefined}>
                        {zoneLines.length} line{zoneLines.length !== 1 ? "s" : ""}
                      </div>
                      <div className="zone-lbl-line"
                           style={zone.color ? { background:`linear-gradient(90deg, ${zone.color}59, transparent)` } : undefined} />
                    </div>
