...
# Public base for the one-click Approve/Reject links baked into deviation
# emails (must be reachable from the recipient's phone/PC browser).
...
def create_deviation(body: DeviationCreate, user=Depends(get_current_user)):
    ...
    with get_conn() as conn:
        ...
        conn.commit()
    out = dict(row)
    # Mail the Quality approver with one-click Approve/Reject buttons.
    _spawn_deviation_email(out)
    return out
...
def approve_deviation(did: int, body: DeviationApprove,
    ...
    with get_conn() as conn:
        ...
        conn.commit()
    _notify_deviation_decided(did, "APPROVED", actor_user_id=user["id"])
    return dict(row)
...
def reject_deviation(did: int, body: DeviationReject,
    ...
    with get_conn() as conn:
        ...
        if not row:
            raise HTTPException(404, "Deviation not pending or not found")
        conn.commit()
    _notify_deviation_decided(did, "REJECTED", actor_user_id=user["id"])
    return dict(row)


# ══════════════════════════════════════════════════════════════════════
# One-click e-mail Approve / Reject for Online Deviations (2026-09-06)
# ----------------------------------------------------------------------
# When a deviation is raised (status PENDING_QA) the Quality approver gets a
# mail with two buttons. Each button is a SIGNED, expiring link (JWT with the
# app's own SECRET_KEY, 14-day life). Clicking opens a tiny page that performs
# the action via a JS POST and shows a confirmation — no app login needed.
#
# Why the GET link doesn't act on its own: corporate mail scanners PRE-FETCH
# links, which would auto-approve. The GET only renders a page; the real state
# change is the POST that page's JS fires — scanners don't run JS. Single-use
# is enforced naturally by the `status='PENDING_QA'` guard (once actioned the
# link is inert). Reject via one-click stores a generic note (the operator
# chose one-click, so no reason box) — full reason can still be added in-app.
# ══════════════════════════════════════════════════════════════════════
def _make_action_token(did: int, action: str) -> str:
    tok = jwt.encode(
        {"did": int(did), "act": action, "purpose": "dev_action",
         "exp": int((datetime.utcnow() + timedelta(days=14)).timestamp())},
        SECRET_KEY, algorithm=ALGORITHM)
    return tok.decode() if isinstance(tok, bytes) else tok


def _read_action_token(tok: str):
    data = jwt.decode(tok, SECRET_KEY, algorithms=[ALGORITHM])
    if data.get("purpose") != "dev_action":
        raise ValueError("wrong token purpose")
    act = data.get("act")
    if act not in ("approve", "reject"):
        raise ValueError("bad action")
    return int(data["did"]), act


def _dev_recipients():
    """(to_list, cc_list): the deviation_* mail-config list, else the Quality
    bypass_* list. Read lazily so there is no import cycle at module load."""
    def _lv(key):
        try:
            from routers.poka_yoke import _lookup_mail_value
        except Exception:
            try:
                from poka_yoke import _lookup_mail_value
            except Exception:
                return ""
        try:
            return _lookup_mail_value(key) or ""
        except Exception:
            return ""
    to = _lv("deviation_to") or _lv("bypass_to")
    cc = _lv("deviation_cc") or _lv("bypass_cc")
    split = lambda s: [x.strip() for x in (s or "").split(",") if x.strip()]
    return split(to), split(cc)


def _smtp_send(subject: str, html_body: str, to_list, cc_list):
...
def _deviation_email_html(row: dict) -> str:
    did = row.get("id")
    dev_no = row.get("dev_no") or f"#{did}"
    ap = f"{_PUBLIC_BASE}/api/quality/dev-action?token={_make_action_token(did, 'approve')}"
    rj = f"{_PUBLIC_BASE}/api/quality/dev-action?token={_make_action_token(did, 'reject')}"
    esc = lambda v: _html.escape(str(v)) if v not in (None, "") else "—"
    fields = [
        ("Deviation No",  dev_no),
        ("Line",          row.get("line_name")),
        ("Machine",       row.get("machine_name") or row.get("machine_no")),
        ("Category",      row.get("category")),
        ("Process",       row.get("process_name")),
        ("Deviation Qty", row.get("deviation_qty")),
        ("Valid up to",   row.get("deviation_upto_date")),
        ("Reason",        row.get("reason")),
        ("Raised by",     row.get("initiated_by")),
    ]
    rows = "".join(
        f"<tr><td style='padding:6px 12px;color:#64748b;font-size:13px;white-space:nowrap'>{_html.escape(k)}</td>"
        f"<td style='padding:6px 12px;color:#0f172a;font-size:13px;font-weight:600'>{esc(v)}</td></tr>"
        for k, v in fields)
    return f"""<!doctype html><html><body style="margin:0;background:#f1f5f9;font-family:Arial,Helvetica,sans-serif">
  <div style="max-width:560px;margin:0 auto;padding:20px">
    <div style="background:#fff;border-radius:14px;overflow:hidden;border:1px solid #e2e8f0">
      <div style="background:#1e40af;padding:18px 22px">
        <div style="color:#fff;font-size:18px;font-weight:800">Online Deviation — Approval needed</div>
        <div style="color:#bfdbfe;font-size:13px;margin-top:2px">{esc(dev_no)}</div>
      </div>
      <table style="width:100%;border-collapse:collapse;margin:10px 0">{rows}</table>
      <div style="padding:8px 22px 24px;text-align:center">
        <div style="color:#475569;font-size:13px;margin-bottom:16px">Tap a button to record your decision — no login needed.</div>
        <a href="{ap}" style="display:inline-block;background:#16a34a;color:#fff;text-decoration:none;font-size:15px;font-weight:700;padding:13px 30px;border-radius:10px;margin:4px">&#10003; Approve</a>
        <a href="{rj}" style="display:inline-block;background:#dc2626;color:#fff;text-decoration:none;font-size:15px;font-weight:700;padding:13px 30px;border-radius:10px;margin:4px">&#10007; Reject</a>
        <div style="color:#94a3b8;font-size:11px;margin-top:18px">This link is valid for 14 days and works once. TBDI&nbsp;MES.</div>
      </div>
    </div>
  </div></body></html>"""


def _spawn_deviation_email(row: dict):
    """Fire-and-forget so raising a deviation is never blocked/broken by SMTP."""
    def _run():
        try:
            to_list, cc_list = _dev_recipients()
            if not to_list:
                print("[quality] deviation email skipped — no recipients configured")
                return
            dev_no = row.get("dev_no") or f"#{row.get('id')}"
            subj = f"[Deviation {dev_no}] Approval needed"
            ln = row.get("line_name")
            if ln:
                subj += f" — {ln}"
            _smtp_send(subj, _deviation_email_html(row), to_list, cc_list)
        except Exception as e:
            print(f"[quality] deviation email failed: {e}")
    threading.Thread(target=_run, daemon=True).start()


def _notify_deviation_decided(did: int, status: str, actor_user_id=None):
    """Fire phone/app notifications after a deviation is approved/rejected —
    whether that happened in-app or via the e-mail one-click link:
      • the RAISER gets "your deviation was approved/rejected"  (C)
      • the QA approver side (quality_incharge + admin/plant_head) gets a
        confirmation — the actor sees "You approved…", others "Deviation …" (B)
    Threaded + best-effort so it never blocks or breaks the decision."""
    def _run():
        try:
        ...
        try:
            with get_conn() as conn:
                cur = dict_cursor(conn)
                cur.execute("SELECT dev_no, line_name, raised_by_user_id "
                            "FROM mes_quality_deviations WHERE id=%s", (did,))
                d = cur.fetchone()
                if not d:
                    return
                cur.execute("SELECT id FROM mes_admin "
                            "WHERE role IN ('quality_incharge','admin','plant_head')")
                approver_ids = [r["id"] for r in cur.fetchall()]
        except Exception:
            return
        dev_no = d["dev_no"] or f"#{did}"
        where  = f" ({d['line_name']})" if d.get("line_name") else ""
        raiser = d.get("raised_by_user_id")
        url = "/quality-deviations"
        title = f"Deviation {verb}"
        # C — the person who raised it
        if raiser:
            try:
                send_to_user(raiser, title,
                             f"Your deviation {dev_no}{where} was {verb}.",
                             url, tag="deviation")
            except Exception:
                pass
        # B — the QA approver / oversight side (skip the raiser: already told)
        for uid in approver_ids:
            if uid == raiser:
                continue
            body = (f"You {verb} deviation {dev_no}."
                    if actor_user_id and uid == actor_user_id
                    else f"Deviation {dev_no}{where} {verb}.")
            try:
                send_to_user(uid, title, body, url, tag="deviation")
            except Exception:
                pass
    threading.Thread(target=_run, daemon=True).start()


def _dev_result_page(title: str, detail: str, colour: str, icon: str = "✓") -> str:
    return f"""<!doctype html><html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Deviation — TBDI MES</title></head>
<body style="margin:0;background:#f1f5f9;font-family:Arial,Helvetica,sans-serif;
  ...
  <div style="background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:34px 30px;
              max-width:380px;width:90%;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,.06)">
    <div style="width:64px;height:64px;border-radius:50%;background:{colour};margin:0 auto 16px;
                display:flex;align-items:center;justify-content:center;font-size:34px;color:#fff;line-height:1">{icon}</div>
    <div style="font-size:22px;font-weight:800;color:#0f172a">{_html.escape(title)}</div>
    <div style="font-size:14px;color:#64748b;margin-top:10px">{_html.escape(detail)}</div>
    <div style="font-size:12px;color:#94a3b8;margin-top:22px">You can close this tab.</div>
  </div>
</body></html>"""


@router.get("/dev-action", response_class=HTMLResponse)
def deviation_act_page(token: str = Query(...)):
    """Landing page for an email Approve/Reject button. Validates the token but
    does NOT change state here (mail scanners pre-fetch GET links) — the page's
    JS fires the POST that actually acts, then shows the result inline."""
    try:
        _did, act = _read_action_token(token)
    except Exception:
        return HTMLResponse(
            _dev_result_page("Link invalid or expired",
                             "This approval link is no longer valid. Please act from the MES app.",
                             "#dc2626", icon="✗"),
            status_code=400)
    verb = "Approve" if act == "approve" else "Reject"
    tok_js = _html.escape(token)
    return HTMLResponse(f"""<!doctype html><html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>{verb} deviation — TBDI MES</title></head>
<body style="margin:0;background:#f1f5f9;font-family:Arial,Helvetica,sans-serif;
             display:flex;min-height:100vh;align-items:center;justify-content:center">
  <div id="card" style="background:#fff;border:1px solid #e2e8f0;border-radius:16px;padding:34px 30px;
              max-width:380px;width:90%;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,.06)">
    <div id="spin" style="width:34px;height:34px;border-radius:50%;border:3px solid #e2e8f0;
  ...
  <script>
    var TOKEN = "{tok_js}";
    function show(colour,icon,title,detail){{
      document.getElementById('card').innerHTML =
        '<div style="width:64px;height:64px;border-radius:50%;background:'+colour+';margin:0 auto 16px;display:flex;align-items:center;justify-content:center;font-size:34px;color:#fff;line-height:1">'+icon+'</div>'+
    ...
    fetch('/api/quality/dev-action/do',{{method:'POST',headers:{{'Content-Type':'application/json'}},
          body:JSON.stringify({{token:TOKEN}})}})
      .then(function(r){{return r.json().then(function(j){{return {{ok:r.ok,j:j}};}});}})
      .then(function(res){{
        var j=res.j||{{}};
        if(res.ok && j.ok){{
          var appr = j.status==='APPROVED';
          show(appr?'#16a34a':'#dc2626', appr?'\\u2713':'\\u2717',
               appr?'Approved':'Rejected',
               'Deviation '+(j.dev_no||'')+' has been '+(appr?'approved':'rejected')+'.');
        }} else if(j.already){{
          var wasAppr=(j.status||'')==='APPROVED';
          show('#64748b', wasAppr?'\\u2713':'\\u2717',
               'Already '+(j.status||'done').toLowerCase(),
               'Deviation '+(j.dev_no||'')+' was already '+(j.status||'').toLowerCase()+'.');
        }} else {{
          show('#dc2626','\\u2717','Could not complete', (j.detail||'This link is invalid or expired.'));
        }}
      }})
      .catch(function(){{ show('#dc2626','\\u2717','Network error','Please try again from the MES app.'); }});
  </script>
</body></html>""")


class _DevActBody(BaseModel):
    token: str


@router.post("/dev-action/do")
def deviation_act_do(body: _DevActBody):
    """Perform the actual approve/reject from a signed email token. No login —
    the signed token is the authorisation. Single-use via the PENDING_QA guard."""
    try:
        did, act = _read_action_token(body.token)
    except Exception:
    ...
    with get_conn() as conn:
            ...
            cur.execute("""
                UPDATE mes_quality_deviations
                   SET status='APPROVED', approved_at=NOW(), updated_at=NOW(),
                       hod_quality_note = COALESCE(NULLIF(hod_quality_note,''),
                                                   'Approved via e-mail one-click link')
                 WHERE id=%s AND status='PENDING_QA'
                RETURNING dev_no, status
            """, (did,))
        else:
            cur.execute("""
                UPDATE mes_quality_deviations
                   SET status='REJECTED', approved_at=NOW(), updated_at=NOW(),
                       hod_quality_note = 'Rejected via e-mail one-click link'
                 WHERE id=%s AND status='PENDING_QA'
                RETURNING dev_no, status
            """, (did,))
        row = cur.fetchone()
        if row:
            conn.commit()
            # Notify the raiser + the QA approver side (email click = no actor id).
            _notify_deviation_decided(did, row["status"], actor_user_id=None)
            return {"ok": True, "dev_no": row["dev_no"], "status": row["status"], "action": act}
        # Nothing updated → either already actioned, or the id is gone.
        cur.execute("SELECT dev_no, status FROM mes_quality_deviations WHERE id=%s", (did,))
        cur_row = cur.fetchone()
    if cur_row:
        return {"ok": False, "already": True, "dev_no": cur_row["dev_no"], "status": cur_row["status"]}
    raise HTTPException(404, "Deviation not found")
