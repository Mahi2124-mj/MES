...
def load_bindings(base_dir: Optional[str] = None) -> Dict:
    ...
    # 2026-07-29 — tolerate a transient mid-write read.  This is HOT (every
    # /api/plc-edge webhook calls list_bindings), so a half-written file must
    # never 500 the endpoint: retry a few times, then fall back to empty.
...
def save_bindings(data: Dict, base_dir: Optional[str] = None) -> None:
    # 2026-07-29 — ATOMIC write.  The old `open(path,"w")` TRUNCATED the file
    # immediately and wrote it out over time, so a concurrent load_bindings
    # (hot on every /api/plc-edge) could json.load a half-written / empty file
    # -> JSONDecodeError 500 bursts (observed 15:22).  Write to a temp file then
    # os.replace() — an atomic rename, so readers only ever see the complete old
