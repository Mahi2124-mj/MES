...
from recorder import (
    ...
    get_next_cycle_number, open_rtsp_capture, update_cycle_tag,
)
...
class CameraStream:
    ...
    def _start_locked(self, machine: Dict, force_cycle_num: Optional[int] = None, force_start_iso: Optional[str] = None) -> Tuple[bool, str]:
        ...
        vid_dir = os.path.join(BASE_DIR, VIDEOS_DIR, machine_id)
        os.makedirs(vid_dir, exist_ok=True)
        fname = f"cycle_{next_n}.mp4"
        fabs = os.path.join(vid_dir, fname)
        frel = f"{VIDEOS_DIR}/{machine_id}/{fname}"
        ...
        while True:
            ...
            try:
                ...
                for mid, mstate in states.items():
                    ...
                    cam_id = machine.get("camera_id") or binding_camera_map.get(mid)
                    ...
                    elif is_recording and s_rec and target_cycle and s_cyc != target_cycle:
                        with s._lock:
                            s._end_locked()
                            s._start_locked(machine, force_cycle_num=target_cycle, force_start_iso=target_start)
