...
def _ensure_tables() -> None:
    ...
    with get_conn() as conn:
        ...
        """)
        # 2026-09-19 — both column adds live here, run once per process.  The
        # skill_level add used to run on EVERY list/create request; an ALTER
        # takes an exclusive lock even when the column already exists, which
        # is the per-request-DDL pattern that deadlocked handlers elsewhere.
        ...
        # Which zone the operator belongs to (Employee Master → Zone column).
        # Nullable: existing operators show "—" until someone sets it.
        cur.execute("ALTER TABLE mes_operators ADD COLUMN IF NOT EXISTS zone_id INTEGER")
        conn.commit()
    ...
    with get_conn() as conn:
        ...
        try:
            ...
            """, (body.badge_code.strip(), body.full_name.strip(),
                  ...
                  max(1, min(5, body.skill_level)), body.is_active, body.zone_id))
...
def list_operators(user=Depends(get_current_user)):
    ...
    with get_conn() as conn:
        ...
        cur.execute("""SELECT o.id, o.badge_code, o.full_name, o.employee_id, o.department,
                              o.skill_level, o.is_active, o.created_at,
                              o.zone_id, z.zone_name
                         FROM mes_operators o
                         LEFT JOIN mes_zones z ON z.id = o.zone_id
                        ORDER BY o.full_name""")
        return cur.fetchall()


@router.put("/{operator_id}")
def edit_operator(operator_id: int, body: OperatorUpsert, admin=Depends(require_admin)):
    """Edit one operator in place (Employee Master → Edit).

    The create route upserts on badge_code, so it cannot change a badge — a
    corrected badge would have made a second operator.  This updates the row
    by id; a badge already used by another operator is refused.
    """
    _ensure_tables()
    badge = (body.badge_code or "").strip()
    name = (body.full_name or "").strip()
    if not badge or not name:
        raise HTTPException(400, "Badge code and name required")
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute("SELECT full_name FROM mes_operators WHERE badge_code=%s AND id<>%s",
                    (badge, operator_id))
        clash = cur.fetchone()
        if clash:
            raise HTTPException(409, f"Badge {badge} already belongs to {clash[0]}")
        if body.zone_id is not None:
            cur.execute("SELECT 1 FROM mes_zones WHERE id=%s", (body.zone_id,))
            if not cur.fetchone():
                raise HTTPException(400, "Unknown zone")
        cur.execute("""UPDATE mes_operators
                          SET badge_code=%s, full_name=%s, employee_id=%s, department=%s,
                              skill_level=%s, is_active=%s, zone_id=%s
                        WHERE id=%s""",
                    (badge, name, body.employee_id, body.department,
                     max(1, min(5, body.skill_level)), body.is_active, body.zone_id,
                     operator_id))
        if cur.rowcount == 0:
            conn.rollback()
