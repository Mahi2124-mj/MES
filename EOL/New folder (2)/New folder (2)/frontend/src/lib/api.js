...
export const api = {
  ...
  updateCamera: (id, body) => http.put(`/masters/cameras/${id}`, body).then(unwrap),
  ...
  // Storage health (live probe of save_path) + manual retention cleanup
  getStorageHealth: () => http.get('/config/storage-health').then(unwrap),
  runVideoCleanup:  () => http.post('/admin/video-cleanup').then(unwrap),

  // Master Video ON/OFF (camera recording + live view) — to shed PC load
  getVideoEnabled: () => http.get('/config/video-enabled').then(unwrap),
  setVideoEnabled: enabled => http.post('/config/video-enabled', { enabled }).then(unwrap),
