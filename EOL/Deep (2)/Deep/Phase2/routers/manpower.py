...
def _seed_processes_from_machines(line_id: int) -> None:
    ...
    with get_conn() as conn:
        ...
        #
        # 2026-08-14 — MATCH ON A NORMALISED NAME, not just a lower-cased one.
        # The two tables spell the same thing differently: mes_zones says
        # "SEAT SLIDER" and "Loop Pipe" while mes_machines says "SEAT_SLIDER"
        # and "LOOP_PIPE"; line names are "YCA-SS" here and "YCA_SS" there.
        # LOWER() alone therefore matched only the Recliner lines, where the
        # single difference happened to be case — so 15 of 19 lines seeded no
        # processes at all and the Processes / Skill page came up empty for
        # everything except the handful that already had rows.
        # Treating space, hyphen and slash as equivalent to underscore makes
        # the join say what it always meant.
        cur.execute("""
            SELECT id, machine_no, machine_name
              FROM mes_machines
             WHERE LOWER(REPLACE(REPLACE(REPLACE(zone_name,' ','_'),'-','_'),'/','_'))
                 = LOWER(REPLACE(REPLACE(REPLACE(%s,       ' ','_'),'-','_'),'/','_'))
               AND LOWER(REPLACE(REPLACE(REPLACE(line_name,' ','_'),'-','_'),'/','_'))
                 = LOWER(REPLACE(REPLACE(REPLACE(%s,       ' ','_'),'-','_'),'/','_'))
               AND is_active = TRUE
             ORDER BY machine_no
        """, (zone_name, nf2_line))
...
@router.delete("/punches")
def remove_punch(operator_id: int = Query(...),
                 line_id: int = Query(...),
                 date: str = Query(...),
                 shift: Optional[str] = None,
                 user=Depends(get_current_user)):
    """Remove an operator from the shift pool (delete their punch)."""
    _ensure_tables()
    try:
    ...
    with get_conn() as conn:
        cur = conn.cursor()
        if shift:
            cur.execute("""DELETE FROM mes_operator_punches
                            WHERE operator_id=%s AND line_id=%s AND shift_date=%s AND shift_name=%s""",
                        (operator_id, line_id, d, shift))
        else:
            cur.execute("""DELETE FROM mes_operator_punches
                            WHERE operator_id=%s AND line_id=%s AND shift_date=%s""",
                        (operator_id, line_id, d))
        n = cur.rowcount
        conn.commit()
...
def _carry_source_date(cur, line_id: int, shift: str, d: date):
    """Most recent prior shift_date (same line + shift, same ISO week as `d`)
    that still has active allocations — the source to carry forward from."""
    cur.execute("""
        SELECT MAX(shift_date) AS sd
    ...
    """, (line_id, shift, d, d))
    r = cur.fetchone()
    return r["sd"] if r and r["sd"] else None


@router.get("/allocations/carry")
def carry_allocations(line_id: int = Query(...), date: str = Query(...),
                      shift: str = Query(...), user=Depends(get_current_user)):
    """The same operators stay put through the week: for a day with no
    allocation yet, suggest the crew from the most recent same-shift day
    earlier THIS week.  Rows are shaped like the pool (id = operator_id) so the
    page can pre-fill; the supervisor Saves to commit (or changes first)."""
    _ensure_tables()
    try:
        d = datetime.strptime(date, "%Y-%m-%d").date()
    except ValueError:
    ...
    with get_conn() as conn:
        cur = dict_cursor(conn)
        sd = _carry_source_date(cur, line_id, shift, d)
        if not sd:
            return {"carried_from": None, "items": []}
        cur.execute("""
            SELECT a.process_id, o.id AS operator_id, o.full_name, o.badge_code,
                   o.employee_id, o.department, o.skill_level
              FROM mes_manpower_allocations a
              JOIN mes_operators o ON o.id = a.operator_id
             WHERE a.line_id=%s AND a.shift_name=%s AND a.shift_date=%s
               AND a.removed_at IS NULL
             ORDER BY a.process_id
        """, (line_id, shift, sd))
        rows = cur.fetchall()
    return {"carried_from": str(sd),
            "items": [{"process_id": r["process_id"], "id": r["operator_id"],
                       "full_name": r["full_name"], "badge_code": r["badge_code"],
...
def _fire_skill_mismatch_alerts(line_id: int, shift_date: date, shift_name: str,
    ...
    if not cfg:
        return
    line_name = _line_name(line_id) or "Unknown line"
...
def list_alerts(line_id: Optional[int] = None,
    ...
    with get_conn() as conn:
        cur = dict_cursor(conn)
        # 2026-09-16 — carry line_name so the alert banner can show the LINE NAME
        # ("Y17-SS") instead of "Line #15": the payload had no name at all, so
        # the UI had nothing to fall back to but the id.
...
def _watcher_tick() -> None:
    ...
    for cfg in cfgs:
        ...
        if required > 0 and got < required and key not in _unalloc_fired:
            ...
            line_name = cfg.get("line_name") or "Unknown line"
            # 2026-09-15 — also drop it into the line team's Inbox (/my-escalations).
            try:
                from routers.push import send_to_line
                send_to_line(line_id, f"Manpower allocation pending — {line_name}",
                             f"Shift {shift}: only {got}/{required} slots filled after "
                             f"the allocation deadline.",
                             url="/shift-allocation", tag="manpower_pending")
            except Exception as _ex:
                print("[manpower] inbox notify failed:", str(_ex)[:120])
            to_list = _addr_list(cfg["quality_to_addresses"]) + \
                      _addr_list(cfg["section_incharge_to_addresses"])
