...
export default function OperatorBadge({ lineId, token, shift }) {
  ...
  return (
    <>
      ...
      ) : null /* 2026-08-11 — the "Scan badge to sign in" chip is hidden.  It sat
                   on every line card on the dashboard whether or not anyone was
                   signing in, and read as clutter.  The signed-IN chip above is
                   untouched, so an operator who is signed in still shows with a
                   green dot and their badge code.  Nothing else was removed —
                   the modal and all sign-in logic below still exist, so this is
                   a one-line revert if the CTA is ever wanted back. */}
