...
def update_camera(
    ...
    """Edit an existing camera in place.  Only fields that are provided
    (non-None and, for strings, non-blank) are changed — so an admin can
    rename or re-IP a camera WITHOUT re-entering its username/password
    (those are stored encrypted and never shown back in the UI)."""
    payload = load_config(base_dir)
    cameras = payload.get("cameras", [])
    target = None
    for cam in cameras:
    ...
    new_ip   = (ip or "").strip() if ip is not None else None
    new_path = _normalize_path(path) if path is not None else None
    # Reject a rename that would collide with a different camera on ip+path.
    check_ip   = new_ip   if new_ip   else str(target.get("ip", ""))
    check_path = new_path if new_path is not None else str(target.get("path", ""))
    for cam in cameras:
        if str(cam.get("id", "")) == str(camera_id):
            continue
        if str(cam.get("ip", "")) == check_ip and str(cam.get("path", "")) == check_path:
            return False, "Another camera with the same IP and path already exists"
    ...
    if new_ip:
        target["ip"] = new_ip[:64]
    if port is not None:
        target["port"] = int(port) if port else 554
    if new_path is not None:
        target["path"] = new_path
    # Credentials: only overwrite when the admin actually typed a new value.
    ...
    payload["cameras"] = cameras
    save_config(payload, base_dir)
    return True, f"Camera updated: {target.get('name', camera_id)}"
