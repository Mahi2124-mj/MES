...
// Time-on-screen reads as "2h 14m", not 8040000 — the number is only useful if
// you can compare two people at a glance.
function fmtDur(ms) {
...
const tdNum   = { padding:"8px 10px", textAlign:"right", color:"#475569", whiteSpace:"nowrap" };
const hdSm    = { fontSize:10, fontWeight:800, letterSpacing:".05em", textTransform:"uppercase",
                  color:"#64748b", marginBottom:6 };
const rowSm   = { display:"flex", justifyContent:"space-between", gap:12,
                  padding:"3px 0", borderBottom:"1px solid #eef2f7" };
const emptySm = { fontSize:11, color:"#cbd5e1", padding:"4px 0" };
...
export default function Audit() {
  ...
  // Per-user activity roll-up (page views + dwell + changes)
  ...
  // Per-user activity.  Admin-only endpoint, so a non-admin viewer just sees
  // the message instead of a broken card.
  useEffect(() => {
    setActLoading(true); setActErr(""); setDetailFor("");
    api.get(`/api/ui-timing/activity/summary?days=${actDays}`, token)
      .then(d => setActivity(Array.isArray(d?.users) ? d.users : []))
      .catch(e => { setActivity([]); setActErr(e?.message || "Could not load activity"); })
      .finally(() => setActLoading(false));
  }, [token, actDays]);

  const openDetail = useCallback((username) => {
    if (detailFor === username) { setDetailFor(""); return; }
    setDetailFor(username); setDetail({}); setDetailLoading(true);
    api.get(`/api/ui-timing/activity/detail?username=${encodeURIComponent(username)}&days=${actDays}`, token)
      .then(d => setDetail(d || {}))
      .catch(() => setDetail({}))
      .finally(() => setDetailLoading(false));
  }, [detailFor, actDays, token]);
  ...
  return (
    <>
      <style>{`
        ...
        /* Phone: the fixed-width columns (90+130+170+120) overflow a 393px
           screen — let the row wrap and its columns shrink so the whole row
           fits, with the free-text details dropping onto its own line. */
        @media (max-width: 600px) {
          .log-row { flex-wrap: wrap; gap: 8px 12px; padding: 12px 14px; }
          .log-row > div { min-width: 0; flex-shrink: 1 !important; }
          .log-row > div:last-child { flex-basis: 100%; }
        }
      ...
      <div className="audit-root">
        ...
        <div className="audit-body">
          ...
          <div className="users-card">
            <div className="users-card-title">
              ...
              <small>
                {usersLoading
                  ? "loading…"
                  : `${users.length} user${users.length !== 1 ? "s" : ""} · tap any tile to filter log`}
              </small>
            </div>
            {usersLoading ? (
          ...
          {/* ── Per-user activity (2026-08-14) ──────────────────────────
              The log below answers "what changed".  This answers "who opened
              what, and how long were they on it" — the two halves of the same
              question, which used to live in two tables nobody joined. */}
          <div className="users-card">
            <div className="users-card-title">
              <h3>Activity Per User</h3>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <select value={actDays}
                ...
                <small>{actLoading ? "loading…" : `${activity.length} user${activity.length!==1?"s":""}`}</small>
              </div>
            </div>
            ...
            {actLoading ? (
              ...
              <div style={{ fontSize:12, color:"#b91c1c", padding:"10px 0" }}>{actErr}</div>
            ) : activity.length === 0 ? (
              <div style={{ fontSize:12, color:"#94a3b8", padding:"10px 0", lineHeight:1.6 }}>
                No activity recorded for this window yet.<br/>
                <span style={{ fontSize:11 }}>
                  Page-view tracking starts from the next login — samples recorded
                  before it existed carry no user and are not counted here.
                </span>
              </div>
            ) : (
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                  <thead>
                    <tr>{["User","Pages opened","Screens","Time on screen","Avg open","Videos","Changes","Logins","Last seen",""]
                      .map((h,i)=>(
                        <th key={i} style={{ textAlign: i===0?"left":"right", padding:"7px 10px",
                    ...
                    {activity.map(a => (
                      <Fragment key={a.username}>
                        <tr style={{ borderBottom:"1px solid #f1f5f9" }}>
                          <td style={{ padding:"8px 10px", fontWeight:700, color:"#0f172a" }}>{a.username}</td>
                          <td style={tdNum}>{a.pagesOpened}</td>
                          <td style={tdNum}>{a.distinctPages}</td>
                          <td style={{ ...tdNum, fontWeight:700, color:"#0f172a" }}>{fmtDur(a.dwellMs)}</td>
                          <td style={tdNum}>{a.avgOpenMs ? `${Math.round(a.avgOpenMs)} ms` : "—"}</td>
                          <td style={tdNum}>{a.videosWatched}</td>
                          <td style={{ ...tdNum, fontWeight:700,
                                        color: a.changesMade ? "#b45309" : "#cbd5e1" }}>{a.changesMade}</td>
                          <td style={tdNum}>{a.logins}</td>
                          <td style={{ ...tdNum, color:"#64748b" }}>{fmtRelative(a.lastSeen)}</td>
                          <td style={{ ...tdNum, paddingRight:10 }}>
                            <button onClick={()=>openDetail(a.username)}
                                    style={{ fontSize:10, fontWeight:700, padding:"3px 9px",
                                              borderRadius:99, border:"1.5px solid #e2e8f0",
                                              background:"#fff", color:"#475569", cursor:"pointer" }}>
                              {detailFor===a.username ? "hide" : "details"}
                            </button>
                          </td>
                        </tr>
                        {detailFor === a.username && (
                          <tr>
                            <td colSpan={10} style={{ background:"#f8fafc", padding:"12px 14px" }}>
                              {detailLoading ? <div className="spinner" style={{ margin:"10px auto" }}/> : (
                                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
                                  <div>
                                    <div style={hdSm}>Screens opened</div>
                                    {(detail.pages||[]).length === 0
                                      ? <div style={emptySm}>No page views recorded.</div>
                                      : (detail.pages||[]).map(p => (
                                        <div key={p.page} style={rowSm}>
                                          <span style={{ fontFamily:"monospace", fontSize:11, color:"#0f172a" }}>{p.page}</span>
                                          <span style={{ color:"#64748b" }}>
                                            {p.opens}× · <b style={{ color:"#0f172a" }}>{fmtDur(p.dwellMs)}</b>
                                          </span>
                                        </div>
                                      ))}
                                  </div>
                                  <div>
                                    <div style={hdSm}>Changes made</div>
                                    {(detail.changes||[]).filter(c=>c.action!=="AUTH_LOGIN").length === 0
                                      ? <div style={emptySm}>No changes in this window.</div>
                                      : (detail.changes||[]).filter(c=>c.action!=="AUTH_LOGIN").slice(0,25).map((c,i) => (
                                        <div key={i} style={rowSm}>
                                          <span style={{ fontWeight:700, fontSize:11, color:"#b45309" }}>{c.action}</span>
                                          <span style={{ color:"#64748b", fontSize:11 }}>
                                            {c.entityType||""}{c.entityId!=null?` #${c.entityId}`:""} · {c.ts?.slice(5,16)}
                                          </span>
                                        </div>
                                      ))}
                                  </div>
