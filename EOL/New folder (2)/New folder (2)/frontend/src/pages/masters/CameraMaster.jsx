...
export default function CameraMaster() {
  ...
  const loadVideoPath = async () => {
    try {
      ...
      setVideoPathInfo(r)
      setRetentionDays(r.retention_days ?? 2)
    } catch {}
  }
  ...
  const saveVideoPath = async () => {
    setSavingPath(true)
    try {
      await api.setVideoPath({ save_path: videoPath, retention_days: Number(retentionDays) })
    ...
    <div className="space-y-4">
      ...
      <div className="glass dark:bg-slate-800/50 dark:border-slate-700 p-4">
        ...
        <div className="flex gap-2 items-center mt-3">
          <label className="text-sm font-semibold text-gray-600 dark:text-slate-300 whitespace-nowrap">Video Retention (days)</label>
          <input type="number" min={1} max={365}
            className="input-field text-sm w-24"
            value={retentionDays}
            onChange={e => setRetentionDays(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') saveVideoPath() }} />
          <span className="text-xs text-gray-400">Last N days kept · NG clips kept permanently · TS rotates per shift</span>
        </div>
