...
def get_bypass_episodes(
    ...
    for r in rows:
        ...
        info["events"].append({"ts": ts, "is_pass": is_pass, "actual": actual,
                                "alert": r["alert_level"],
                                "acked": bool(r.get("acknowledged")),
                                "acked_at": r.get("acknowledged_at")})
    ...
    for py_no, info in by_py.items():
        ...
        for e in info["events"]:
            if not e["is_pass"]:
                ...
                last_nonpass_ts = None
        if cur_ep:
            # 2026-08-25 — trailing-episode never closed.
            # The collector emits NO 'PASS' event on recovery — it
            # AUTO-ACKS the event stream instead (rule B: no new
            # detection for 120 s ⇒ presumed recovered, the SAME signal
            # the dashboard tile + matrix use for "live bypass =
            # unacknowledged").  So a trailing episode whose py has
            # already been acknowledged is NOT ongoing — the bypass
            # cleared.  Close it at its recovery time instead of
            # painting "ongoing since …" forever.  Without this the
            # Failures page contradicted the "ALL SET OK" tile.
            last_e = info["events"][-1] if info["events"] else None
            if last_e and last_e.get("acked"):
                cur_ep["end_dt"] = last_e.get("acked_at") or last_nonpass_ts
            raw_episodes.append(cur_ep)  # end_dt None ⇒ still truly ongoing
...
_MAIL_CONFIG_KEYS = {
    ...
    # 2026-09-06 — Online Deviation "raised" email with one-click Approve/Reject
    # buttons. Blank falls back to the bypass_* Quality list (see quality.py).
    "deviation_to": ("DEVIATION_TO", None, "Online Deviation approval mail — To addresses (Quality approver; blank = use Bypass To)"),
    "deviation_cc": ("DEVIATION_CC", None, "Online Deviation approval mail — Cc addresses"),
    ...
    # 2026-08-14 — the same three-level chain the shift reports use.  The
    # hourly report previously had ONE To and ONE Cc, so the line's own people,
    # the zone head and the section head all had to share a single box and
    # everybody landed in the same field.  Blank = that level is not mailed, so
    # an installation that only ever filled hourly_to is unaffected.
    "hourly_zone_to":    ("HOURLY_ZONE_TO",    None, "Hourly slot report — Zone head To addresses"),
    "hourly_zone_cc":    ("HOURLY_ZONE_CC",    None, "Hourly slot report — Zone head Cc addresses"),
    "hourly_section_to": ("HOURLY_SECTION_TO", None, "Hourly slot report — Section head To addresses"),
    "hourly_section_cc": ("HOURLY_SECTION_CC", None, "Hourly slot report — Section head Cc addresses"),
}
...
def _get_mail_addrs(kind: str) -> tuple[list, list]:
    ...
    2026-08-14 — 'hourly' now merges three levels (line, zone head, section
    head) into the one pair SMTP needs.  Order is line → zone → section and
    duplicates are dropped, so an address listed twice is mailed once; an
    address in both To and Cc stays in To only, because being on To is the
    stronger placement and servers differ on how they treat the overlap.
    The other kinds keep their single pair — only the hourly report was asked
    for a chain.
    """
    levels = ("", "zone_", "section_") if kind == "hourly" else ("",)

    to, seen_to = [], set()
    for lv in levels:
        for e in (_lookup_mail_value(f"{kind}_{lv}to") or "").split(","):
            e = e.strip()
            if e and e.lower() not in seen_to:
                seen_to.add(e.lower()); to.append(e)

    cc, seen_cc = [], set()
    for lv in levels:
        for e in (_lookup_mail_value(f"{kind}_{lv}cc") or "").split(","):
            e = e.strip()
            k = e.lower()
            if e and k not in seen_to and k not in seen_cc:
                seen_cc.add(k); cc.append(e)
...
def _send_bypass_email_async(body: "EventIngest") -> None:
    ...
    line_name  = "Unknown line"   # never a number: the DB lookup below fills the
                                  # real name, and this seed can reach the e-mail
                                  # subject if that lookup throws.
    model_name = "—"
...
def sensor_health(
    ...
    with get_conn() as conn:
        ...
        try:
            ...
            pass
        if resolved_bit is not None:
            # 2026-08-22 — read the PUBLISHED per-model config first.
            # Maintenance Panel → PY Config edits `mes_py_config` and "Update
            # to Software" publishes it to `mes_py_config_live`, but this
            # endpoint only ever looked at the legacy `mes_py_assignments`
            # table.  That table was never migrated (41 rows for the whole
            # plant), so YNC-SS model 15 showed 8 sensors on the dashboard
            # while the operator's published list held 23.  Prefer the
            # published config for this line+model and keep the legacy query
            # as the fallback, so any line that was never published through
            # the new editor behaves exactly as before.
            src_rows = []
            from_published = False
            try:
                cur.execute("""
                    SELECT id            AS py_master_id,
                           sensing_bits  AS sensing_bits,
                           py_no         AS py_no,
                           py_name       AS py_name,
                           NULL::text    AS side,
                           d_bit         AS register_addr,
                           desired_value AS desired_value
                      FROM mes_py_config_live
                     WHERE line_id = %s AND model_number = %s
                     -- 2026-08-22 — no filter here on purpose.  Operator:
                     -- "list me 23 hai na to 23 hi dikhne chahiye dashboard
                     -- pe".  Sensor Health is the health board for the whole
                     -- configured list, so every published row for the running
                     -- model is returned.  Active / Not-Active still decides
                     -- which of them the collector ALARMS on — that gate lives
                     -- in check_py_bypass, not here.
                     ORDER BY py_seq, py_no
                """, (line_id, resolved_bit))
                src_rows = cur.fetchall()
                from_published = bool(src_rows)
            except Exception:
                src_rows = []
            if not src_rows:
                cur.execute("""
            ...
            for r in src_rows:
                ...
                if from_published:
                    # Whole published list is shown; nothing is filtered out.
                    applicable.append(dict(r))
                    continue
                # Legacy source only: skip PYs whose desired value for this
                # model is PASS / 0 / NULL — those don't apply to the model.
                if dv is None:
                    continue
                try:
                    if int(dv) == 0:
...
def _send_bypass_digest_now() -> None:
    ...
    for i, item in enumerate(seen.values(), 1):
        ...
        <tr style="background:{'#fff' if i%2 else '#f8fafc'};">
          ...
          <td style="padding:7px 12px;">{ev.get('line_name') or "Unknown line"}</td>
...
# Kick off the scheduler on module import — in the ONE process elected to run
# background work, so extra uvicorn workers do not each send the digest.
from bg_leader import is_leader as _bg_is_leader
if _bg_is_leader():
...
def _collect_slot_report(conn, line: dict, slot: dict, record_date) -> dict:
    ...
    return {
        "line_name":       line.get("line_name") or "Unknown line",
        "line_id":         line_id,
        "shift":           shift,
...
# ══════════════════════════════════════════════════════════════
# HOURLY SLOT REPORT — per-line recipients  (2026-08-14)
# ══════════════════════════════════════════════════════════════
# The shift report (mes_report_email_config) and the OEE drop alarm
# (mes_oee_alarm_config) are both keyed by line_id, so each line mails its own
# people.  The hourly slot report was the odd one out: a single global To/Cc
# got ONE digest covering every line, so a supervisor who owns one line was
# either mailed about 19 lines or not mailed at all.
#
# Same three levels as the shift report (line / zone head / section head).
# A line with no row here still appears in the GLOBAL digest, exactly as
# today — so this adds routing without taking any away.
class HourlyMailCfg(BaseModel):
    line_id:              int
    to_addresses:         str = ""
    cc_addresses:         str = ""
...
_HOURLY_CFG_READY = False


def _ensure_hourly_mail_table() -> None:
    global _HOURLY_CFG_READY
    if _HOURLY_CFG_READY:
        return
    try:
        with get_conn() as conn:
            conn.cursor().execute("""
                CREATE TABLE IF NOT EXISTS mes_hourly_mail_config (
                    line_id              INTEGER PRIMARY KEY,
                    to_addresses         TEXT NOT NULL DEFAULT '',
                    cc_addresses         TEXT NOT NULL DEFAULT '',
                    zone_to_addresses    TEXT NOT NULL DEFAULT '',
                    zone_cc_addresses    TEXT NOT NULL DEFAULT '',
                    section_to_addresses TEXT NOT NULL DEFAULT '',
                    section_cc_addresses TEXT NOT NULL DEFAULT '',
                    is_active            BOOLEAN NOT NULL DEFAULT TRUE,
                    updated_at           TIMESTAMPTZ DEFAULT NOW()
                )
            """)
            conn.commit()
        _HOURLY_CFG_READY = True
    except Exception as exc:
        print(f"[SLOT-REPORT] per-line mail table unavailable: {exc}")


def _merge_levels(cfg: dict) -> tuple:
    """line → zone → section into one (to, cc); dedup, To beats Cc."""
    to, seen_to = [], set()
    for k in ("to_addresses", "zone_to_addresses", "section_to_addresses"):
        for a in (cfg.get(k) or "").split(","):
            a = a.strip()
            if a and a.lower() not in seen_to:
                seen_to.add(a.lower()); to.append(a)
    cc, seen_cc = [], set()
    for k in ("cc_addresses", "zone_cc_addresses", "section_cc_addresses"):
        for a in (cfg.get(k) or "").split(","):
            a = a.strip(); low = a.lower()
            if a and low not in seen_to and low not in seen_cc:
                seen_cc.add(low); cc.append(a)
    return to, cc


def _hourly_line_configs() -> dict:
    """line_id → merged (to, cc) for every ACTIVE per-line row that has a To."""
    _ensure_hourly_mail_table()
    out = {}
    try:
        with get_conn() as conn:
            cur = dict_cursor(conn)
            cur.execute("SELECT * FROM mes_hourly_mail_config WHERE is_active")
            for r in cur.fetchall():
                to, cc = _merge_levels(r)
                if to:
                    out[int(r["line_id"])] = (to, cc)
    except Exception:
        pass
    return out


@router.get("/hourly-mail-config/", tags=["mail-config"])
def list_hourly_mail_config(user=Depends(get_current_user)):
    """Per-line recipients for the hourly slot report, joined to the line name
    so the admin table does not need a second round-trip."""
    _ensure_hourly_mail_table()
    with get_conn() as conn:
        cur = dict_cursor(conn)
        cur.execute("""
            SELECT c.*, l.line_code, l.line_name
              FROM mes_hourly_mail_config c
...
@router.put("/hourly-mail-config/", tags=["mail-config"])
def upsert_hourly_mail_config(body: HourlyMailCfg, admin=Depends(require_admin)):
    _ensure_hourly_mail_table()
    with get_conn() as conn:
        conn.cursor().execute("""
            INSERT INTO mes_hourly_mail_config
                (line_id, to_addresses, cc_addresses,
            ...
            ON CONFLICT (line_id) DO UPDATE SET
                ...
                updated_at           = NOW()
        """, (body.line_id, body.to_addresses, body.cc_addresses,
...
@router.delete("/hourly-mail-config/{line_id}", tags=["mail-config"])
def delete_hourly_mail_config(line_id: int, admin=Depends(require_admin)):
    """Drop a line's own recipients — it falls back to the global digest."""
    _ensure_hourly_mail_table()
    with get_conn() as conn:
        conn.cursor().execute(
            "DELETE FROM mes_hourly_mail_config WHERE line_id=%s", (line_id,))
        conn.commit()
    return {"ok": True}


def _send_slot_report(entries: list) -> None:
    """Route the just-completed slot to whoever owns each line.

    Lines with their own recipient row get their OWN mail containing just that
    line.  Everything left over goes out as the global digest it always was.
    """
    if not entries:
        return
    per_line = _hourly_line_configs()
    if per_line:
        routed = [e for e in entries if int(e.get("line_id", 0)) in per_line]
        rest   = [e for e in entries if int(e.get("line_id", 0)) not in per_line]
        for e in routed:
            to, cc = per_line[int(e["line_id"])]
            _send_slot_report_to([e], to, cc)
        entries = rest
        if not entries:
            return
    _send_slot_report_to(entries, None, None)


def _send_slot_report_to(entries: list, to_override, cc_override) -> None:
    """Render + send one slot mail.  Recipients default to the global config."""
    ...
    smtp_user = os.getenv("SMTP_USER", "")
    smtp_pass = os.getenv("SMTP_PASS", "")
    # Per-line recipients win; otherwise the global mail-config chain:
    # DB → env (HOURLY_TO / HOURLY_CC) → legacy env (SLOT_REPORT_EMAIL).
    # Edit via Admin Panel → Hourly Report Mail.
    if to_override:
        to_list, cc_list = to_override, (cc_override or [])
    else:
...
# Same as the digest above — one process only (see bg_leader.py).
if _bg_is_leader():
    _start_slot_worker()
