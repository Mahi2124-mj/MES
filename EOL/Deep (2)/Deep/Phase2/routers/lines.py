...
#   CYCLE_VIDEO_BASE_URL=http://192.168.30.50:5555
...
from auth import get_current_user, get_current_user_optional, require_admin, FLOOR_SCOPED_ROLES
...
# ── Which lines are actually PRODUCING right now ──────────────────────────
# 2026-08-03 — added for the Camera CMS.  The CMS was recording every bound
# camera 24/7: measured 31 continuous RTSP recorders while only 2 lines were
# running.  Those ~29 pointless streams competed with the ones that mattered,
# and the cameras/switch could not serve them all cleanly — so the useful
# recordings lost frames.  That is what operators saw as pixel break-up, and
# (once the recorder started stamping real arrival times) as a clip whose
# progress bar advanced while the picture sat frozen: the timeline was right,
# the frames were simply missing.
#
# The CMS polls this and records only cameras whose line has cycled recently,
# which cuts concurrent recorders to the handful that are actually needed.
# Cached briefly so a poll every minute costs nothing.
_PRODUCING_CACHE: dict = {"at": 0.0, "minutes": None, "lines": []}
...
def lines_producing(minutes: int = Query(15, ge=1, le=240),
    ...
    Returns line_name (what the CMS camera bindings key on) plus line_code/id.
    Fail-soft by design: a line whose ct_log can't be read is treated as
    PRODUCING, so a query problem can never silently stop its recording."""
    import time as _t
    now = _t.time()
    if (_PRODUCING_CACHE["lines"] and _PRODUCING_CACHE["minutes"] == minutes
            and now - _PRODUCING_CACHE["at"] < 25):
        return {"minutes": minutes, "lines": _PRODUCING_CACHE["lines"],
                "cached": True}

    out = []
    with get_conn() as conn:
        cur = dict_cursor(conn)
        cur.execute("SELECT id, line_code, line_name, db_table_name FROM mes_lines "
                    "WHERE COALESCE(is_active, TRUE) AND COALESCE(db_table_name,'') <> ''")
        rows = cur.fetchall() or []
        for r in rows:
            tbl = f"{r['db_table_name']}_ct_log"
            try:
                cur.execute("SET LOCAL statement_timeout = '4s'")
                cur.execute(
                    f"SELECT 1 FROM {tbl} WHERE ts > NOW() - INTERVAL '%s minutes' LIMIT 1"
                    % int(minutes))
                producing = cur.fetchone() is not None
            except Exception:
                conn.rollback()
                producing = True          # unreadable -> keep its cameras recording
            if producing:
                out.append({"id": r["id"], "line_code": r["line_code"],
                            "line_name": r["line_name"]})
    _PRODUCING_CACHE.update({"at": now, "minutes": minutes, "lines": out})
    return {"minutes": minutes, "lines": out, "cached": False}
...
class LineUpdate(BaseModel):
    ...
    # 2026-08-10 — line_code and plant_id are editable after creation.  The
    # admin panel used to lock all three of these the moment a line existed, so
    # a typo made during creation could only be fixed in SQL.  The panel now
    # unlocks them behind an explicit Edit button; these fields let that save
    # actually land.  db_table_name was already accepted here.
...
class MachineCreate(BaseModel):
    ...
    # ── Shift-wise capture (2026-08-06) ─────────────────────────
    # Same protocol the other machines already use: read the block off a
    # D-bit, then hand the PLC an L-bit so it clears itself before the
    # next shift.  Both optional — blank keeps the per-cycle behaviour.
    sa_shift_data_bit:   Optional[str] = None         # e.g. "D5300"
    sa_shift_reset_bit:  Optional[str] = None         # e.g. "L150"
    # ── Semi-Auto verdict → Final NG trace (2026-08-19, SEAT SLIDER) ──
    # On the Semi-Auto: which captured data register holds the station
    # verdict, and the two values that mean OK and NG.  On Final Inspection:
    # the bit to pulse when a part that was NG at Semi-Auto arrives.
    # All blank/NULL by default = feature off, so no line changes behaviour
    # until an admin fills these in.
    # 2026-08-24 — operator moved the Semi-Auto verdict from "one register
    # holding 1/2" to two plain bits: whichever is ON is the verdict.  The
    # older register fields stay in the model so existing rows keep loading.
    sa_ok_bit:           Optional[str] = None         # e.g. "M5701"
    sa_ng_bit:           Optional[str] = None         # e.g. "M5702"
    sa_ng_trigger_bit:   Optional[str] = None         # NG-side read trigger, e.g. "M5703"
    sa_result_register:  Optional[str] = None         # legacy
    sa_result_ok_value:  Optional[int] = None         # legacy
    sa_result_ng_value:  Optional[int] = None         # legacy
    fi_sa_ng_bit:        Optional[str] = None         # e.g. "M5800"
    fi_sa_ng_bit_hold_sec: Optional[float] = None     # default 2.0 s pulse
    fi_fetch_bit:        Optional[str] = None         # Final compare-fetch bit (SA-NG reject timing)
    # ── Bottleneck flag ─────────────────────────────────────────
    ...
    # ── Seat-Slider load-cell NG traceability (2026-07-16) ──────────
    # Opt-in per machine (blank = feature off).  Semi-Auto: lc_result_reg
    # holds the PLC's load-cell verdict (1=OK, 2=NG); lc_value_reg the actual
    # weight (logged with the NG barcode).  Final: qr_reg = the scanned QR
    # register; reject_bit = the bit the collector pulses ON 5s when the QR
    # matches an unresolved NG in mes_seatslider_ng.
    lc_result_reg:       Optional[str]  = None   # semi-auto: 1=OK / 2=NG
    lc_value_reg:        Optional[str]  = None   # semi-auto: weight register
    qr_reg:              Optional[str]  = None   # final: QR data register
    reject_bit:          Optional[str]  = None   # final: 5s-ON reject bit
...
@router.get("/part-trace")
def part_trace(code: str = Query(..., min_length=1, description="Exact part code"),
               user=Depends(get_current_user)):
    """Full traceability dossier for ONE part code, aggregated across every
    line: a summary (runs, OK/NG counts, stations, first/last seen, whether it
    was rejected at Final), the Semi-Auto / Final OK-NG verdicts, the load /
    force values captured, and every cycle run with its video handle.  Exact
    match — the caller passes a full serial (partial browse is /part-search)."""
    code = code.strip()
    with get_conn() as conn:
        cur = dict_cursor(conn)

        # 1) Quality verdicts — Semi-Auto + Final OK/NG + reject-bit written.
        quality = []
        try:
            cur.execute("""
                SELECT station, result, bit_written, raw_value, cycle_seq,
                       line_id, line_name, machine_name, shift_name,
                       to_char(ts,'YYYY-MM-DD HH24:MI:SS') AS ts,
                       to_char(record_date,'YYYY-MM-DD') AS record_date
                  FROM mes_sa_fi_quality_log
                 WHERE part_code = %s
                 ORDER BY ts
            """, (code,))
            quality = [dict(r) for r in cur.fetchall()]
        except Exception:
            conn.rollback()

        # 2) Load / data captures (Semi-Auto data log) — the register values.
        loads = []
        try:
            cur.execute("""
                SELECT d.sub_plc_id, p.machine_name, d.model_name, d.shift_name,
                       to_char(COALESCE(d.ts_plc, d.ts_server),
                               'YYYY-MM-DD HH24:MI:SS') AS ts,
                       d.data_values
                  FROM mes_submachine_data_log d
                  LEFT JOIN mes_plc_configs p ON p.id = d.sub_plc_id
                 WHERE d.part_code = %s
                 ORDER BY COALESCE(d.ts_plc, d.ts_server)
            """, (code,))
            for r in cur.fetchall():
                d = dict(r)
                d["machine_name"] = d.get("machine_name") or f"sub {d.get('sub_plc_id')}"
                loads.append(d)
        except Exception:
            conn.rollback()

        # 3) Every cycle run — main-line ct_logs (with video handle) + sub-machine ct_log.
        runs = []
        cur.execute("SELECT id, line_name, db_table_name FROM mes_lines "
                    "WHERE db_table_name IS NOT NULL AND db_table_name <> ''")
        seen = set()
        for ln in cur.fetchall():
            tbl = ln["db_table_name"] + "_ct_log"
            if tbl in seen:
                continue
            seen.add(tbl)
            cur.execute("SELECT to_regclass(%s) AS e", (tbl,))
            if not cur.fetchone()["e"]:
                continue
            cur.execute("SELECT 1 FROM information_schema.columns "
                        "WHERE table_name=%s AND column_name='is_ng'", (tbl,))
            ngcol = "is_ng" if cur.fetchone() else "FALSE AS is_ng"
            try:
                cur.execute(f"""
                    SELECT to_char(ts,'YYYY-MM-DD HH24:MI:SS') AS ts,
                           to_char(record_date,'YYYY-MM-DD') AS record_date,
                           shift_name, ct_value, cycle_seq, {ngcol}
                      FROM {tbl}
                     WHERE part_code = %s
                ...
                for r in cur.fetchall():
                    d = dict(r)
                    runs.append({
                        "kind": "FINAL", "station": ln["line_name"],
                        "line_id": ln["id"], "ts": d["ts"],
                        "record_date": d["record_date"], "shift_name": d["shift_name"],
                        "ct_value": float(d["ct_value"]) if d["ct_value"] is not None else None,
                        "cycle_seq": d["cycle_seq"], "is_ng": bool(d.get("is_ng")),
                        "has_video": d["cycle_seq"] is not None,
                    })
            except Exception:
                conn.rollback()
        # sub-machine ct_log (auxiliary stations)
        try:
            cur.execute("""
                SELECT s.sub_plc_id, p.machine_name, s.line_id,
                       to_char(COALESCE(s.ts_end, s.ts_start),
                               'YYYY-MM-DD HH24:MI:SS') AS ts,
                       to_char(s.record_date,'YYYY-MM-DD') AS record_date,
                       s.shift_name, s.ct_seconds, s.cycle_seq, s.is_ng
                  FROM mes_submachine_ct_log s
                  LEFT JOIN mes_plc_configs p ON p.id = s.sub_plc_id
                 WHERE s.part_code = %s
                 ORDER BY COALESCE(s.ts_end, s.ts_start) LIMIT 500
            """, (code,))
            for r in cur.fetchall():
                d = dict(r)
                runs.append({
                    "kind": "SUB",
                    "station": d.get("machine_name") or f"sub {d['sub_plc_id']}",
                    "line_id": d["line_id"], "ts": d["ts"],
                    "record_date": d["record_date"], "shift_name": d["shift_name"],
                    "ct_value": float(d["ct_seconds"]) if d["ct_seconds"] is not None else None,
                    "cycle_seq": d["cycle_seq"], "is_ng": bool(d.get("is_ng")),
                    "has_video": False,
                })
        except Exception:
        ...
        runs.sort(key=lambda r: r["ts"] or "")

        # 4) Remarks / NG notes for this part.
        remarks = []
        for _tbl, _txtcol in (("mes_ng_remarks", "remark"),
                              ("mes_cycle_comments", "comment")):
            try:
                cur.execute("SELECT to_regclass(%s) AS e", (_tbl,))
                if not cur.fetchone()["e"]:
                    continue
                cur.execute(f"""
                    SELECT to_char(created_at,'YYYY-MM-DD HH24:MI:SS') AS ts,
                           {_txtcol} AS text
                      FROM {_tbl} WHERE part_code = %s
                     ORDER BY created_at LIMIT 100
                """, (code,))
                for r in cur.fetchall():
                    if (r.get("text") or "").strip():
                        remarks.append({"ts": r["ts"], "text": r["text"], "src": _tbl})
            except Exception:
                conn.rollback()

        # 5) Summary rollup.
        semi_ok  = sum(1 for q in quality if q["station"] == "SEMI"  and q["result"] == "OK")
        semi_ng  = sum(1 for q in quality if q["station"] == "SEMI"  and q["result"] == "NG")
        final_ng = sum(1 for q in quality if q["station"] == "FINAL" and q["result"] == "NG")
        rejected = any(q["station"] == "FINAL" and q.get("bit_written") for q in quality)
        run_ng   = sum(1 for r in runs if r["is_ng"])
        all_ts   = [r["ts"] for r in runs if r["ts"]] + [q["ts"] for q in quality if q["ts"]]
        summary = {
            "part_code":  code,
            "total_runs": len(runs),
            "stations":   sorted({r["station"] for r in runs}),
            "semi_ok":    semi_ok,
            "semi_ng":    semi_ng,
            "final_ng":   final_ng,
            "run_ng":     run_ng,
            "rejected_at_final": rejected,
            "first_seen": min(all_ts) if all_ts else None,
            "last_seen":  max(all_ts) if all_ts else None,
            "found":      bool(quality or loads or runs),
        }
        return {"summary": summary, "quality": quality, "loads": loads,
                "runs": runs, "remarks": remarks}
...
def get_historical_data(
    ...
    try:
        with get_conn() as conn:
            ...
            # 2026-06-19 — OT slot manual-plan overlay.  The collector creates the
            # OT hour bucket with plan_pieces=0, so its stored _plan is always 0.
            # The production admin sets a manual OT target (mes_shift_configs.
            # ot_plan); overlay it here so the OT slot shows a real plan / variance
            # / efficiency — just like a normal shift slot.  Compute-on-read only
            # (collector never writes the OT plan).
            try:
                cur.execute(
                    "SELECT ot_plan FROM mes_shift_configs WHERE line_id = %s AND shift_name = %s",
                    (line_id, shift_name),
                )
                _otp_row = cur.fetchone()
                _ot_plan = (_otp_row or {}).get("ot_plan")
                if _ot_plan:
                    cur.execute(
                        "SELECT db_column_prefix FROM mes_hourly_slots "
                    ...
                    for _s in cur.fetchall():
                        _p = _s.get("db_column_prefix") or ""
                        if _p.endswith("_ot") and f"{_p}_actual" in result:
                            _act = result.get(f"{_p}_actual", 0) or 0
                            result[f"{_p}_plan"]     = _ot_plan
                            result[f"{_p}_variance"] = _act - _ot_plan
                    result["ot_plan"] = _ot_plan
            except Exception as _oe:
                print(f"[HISTORICAL] OT plan overlay skipped: {_oe}")
...
def import_excel_data(
    ...
    with get_conn() as conn:
        ...
        if existing:
            ...
            # 2026-09-16 — write the REAL line name, never "Line <id>".  This
            # column is persisted and then read back by every report, export,
            # PDF and e-mail, so the old f"Line {line_id}" permanently poisoned
            # imported shifts with a number instead of e.g. "Y17-SS".
            cur.execute("SELECT line_name, line_code FROM mes_lines WHERE id = %s", (line_id,))
            _lrow = cur.fetchone() or {}
            _line_name = (_lrow.get("line_name") or _lrow.get("line_code") or "").strip()
            if not _line_name:
                raise HTTPException(400, f"Line {line_id} has no name configured — "
                                         f"set it in Admin → Production Lines before importing")
            cur.execute(f"""
                INSERT INTO {table}
                ...
                RETURNING id
            """, (record_date, shift_name, _line_name, ok_count, ng_count, _shift_plan, _shift_plan, 0))
...
def list_lines(plant_id: Optional[int] = None, user=Depends(get_current_user)):
    ...
    with get_conn() as conn:
        cur = dict_cursor(conn)
        # 2026-07-31 — scope by ASSIGNMENT, not by role name.  This used to
        # restrict only role='operator', so a 'production'/'department' login
        # tied to one line (e.g. the SeatSlider account) still pulled all 17
        # lines — wasted queries, wasted polling, and the dashboard opened on
        # someone else's line.  Now ANY user that has rows in
        # mes_operator_lines sees exactly those; a user with NO assignment is
        # unrestricted exactly as before (admins are never assignable), so
        # this cannot narrow an existing login by accident.
        _scoped = False
        if user["role"] != "admin":
            cur.execute("SELECT 1 FROM mes_operator_lines WHERE admin_id = %s LIMIT 1",
                        (user["id"],))
            _scoped = cur.fetchone() is not None
            # A floor role (operator/production/section_incharge/production_incharge)
            # with NO assignment sees NOTHING — it must be scoped to be useful, and
            # "unassigned = all" was leaking every line to e.g. a bare operator.
            if not _scoped and user["role"] in FLOOR_SCOPED_ROLES:
                return []
        if _scoped:
            # Assigned-line user: only their lines
            cur.execute("""
        ...
        #
        # 2026-08-10 — HEARTBEAT is the real liveness signal, not collector_pid.
        # This block used to decide purely on is_process_alive(collector_pid),
        # but that column is only ever written by provision_line() — collectors
        # launched by start_everything.sh leave it NULL.  So alive was False for
        # every normally-started line, and this loop actively rewrote the
        # collector_status that collector_engine had just set to 'running'.
        # Result: the admin panel showed all 18 lines "stopped" while 16 were
        # happily polling their PLCs, and the two writers fought on every poll.
        # mes_collector_locks.heartbeat_at is refreshed by EVERY collector
        # (measured 0-9 s fresh), so trust that, and keep the pid check as a
        # second opinion for the provision-started ones.
        _hb = {}
        try:
            cur.execute("""
                SELECT line_id,
            ...
            """)
            _hb = {r["line_id"]: float(r["age_s"] or 1e9) for r in cur.fetchall()}
        except Exception as exc:
            # No lock table / transient error — fall back to the pid check alone.
            print(f"[LINES] collector heartbeat lookup failed: {exc}")

        # Generous: collectors beat every few seconds, so a minute of silence is
        # a real outage, not a slow poll.
        _HB_STALE_S = 60.0

        for row in rows:
            stored_status = row.get("collector_status")
            pid = row.get("collector_pid")
            alive = is_process_alive(pid) or _hb.get(row["id"], 1e9) < _HB_STALE_S
...
def get_cycle_video(
    ...
    ng:        Optional[int] = Query(None,
                  description="1 = the Alarm row for this seq, 0 = the OK row. "
                              "Alarm and OK rows can share a cycle_seq."),
    ...
    with get_conn() as conn:
        ...
        # 2026-09-17 — MIDNIGHT ROLLOVER.  `date or today` broke every video the
        # moment the clock passed 00:00: B shift runs THROUGH midnight, so a
        # cycle produced at 00:02 still carries the shift's record_date of the
        # PREVIOUS day.  Pinning the lookup to today's date matched no row and
        # the endpoint 404'd every clip until the shift ended ("12 baje date
        # change hui, saari video band").  When the caller does not specify a
        # date we now accept the last two days and take the most recent matching
        # cycle, which resolves the running shift correctly on either side of
        # midnight.  An explicit ?date= still pins exactly as before.
        record_date = date or None
        shift_clause = ""
        if record_date:
            date_clause = "record_date = %s"
            params = [record_date, cycle_seq]
            if shift:
                shift_clause = "AND shift_name = %s"
                params.insert(1, shift)
        else:
            date_clause = "record_date >= (CURRENT_DATE - INTERVAL '1 day')"
            params = [cycle_seq]
            if shift:
                shift_clause = "AND shift_name = %s"
                params.insert(0, shift)
        ...
        # 2026-08-11 — an Alarm row and an OK row can share a cycle_seq, and this
        # only kept the latest ts, so one dot could serve the other's clip.  When
        # the caller passes ng=0/1 we pick the row it actually clicked; without
        # it the old behaviour is untouched.
        _ng_clause = ""
        _params2 = list(params)
        if ng is not None:
            _ng_clause = "AND COALESCE(is_ng, FALSE) = %s "
            _params2.append(bool(ng))
        cur.execute(
            f"SELECT ts, ct_value, part_code, is_ng, record_date FROM {tbl_log} "
            f"WHERE {date_clause} {shift_clause} AND cycle_seq = %s "
            f"{_ng_clause}"
            f"ORDER BY ts DESC LIMIT 1",
            _params2,
        )
        hit = cur.fetchone()
        # Use the row's OWN record_date from here on (archive lookup + clip
        # naming): across midnight it is the shift's date, not today's.
        if hit and hit.get("record_date"):
            record_date = str(hit["record_date"])
        elif not record_date:
            # No row matched — keep the old default so every later query still
            # has a concrete date to bind.
            record_date = datetime.now().strftime("%Y-%m-%d")

        # 2026-08-12 — ARCHIVE HIT for the MAIN line.  The sub-machine route has
        # served pre-rendered clips off disk since yesterday, but this route did
        # not — so the Management dashboard (which shows Final Inspection, i.e.
        # the main line) still paid a full transcode on every click and was the
        # one screen that never got faster.  Checked before any CMS work; a miss
        # falls through to exactly the old path.
        if hit:
            try:
                from routers.clip_archive import find as _arch_find, serve as _arch_serve
                _hit_arch = _arch_find("line", line_id, record_date, cycle_seq,
                                       bool(hit.get("is_ng")))
            except Exception:
                _hit_arch = None
            if _hit_arch:
                return _arch_serve(_hit_arch, request)
        ...
        if tr and tr.get("ts"):
            ...
            # 2026-08-11 — same stoppage clamp as the sub-machine route (see
            # routers/submachines.py): a cycle spanning a break can be 30+ min,
            # which CMS grinds on until the proxy timeout and then 404s.  120 s
            # is far above any real CT here, so normal cycles are untouched.
            _clip_dur     = max(3.0, _cycle_dur_s + 1.0)
            # 2026-08-11 — operator spec: an Alarm clip shows the Alarm AND the
            # cycle after it as ONE continuous video.  This log stores only the
            # cycle END (`ts`), so extending means pushing _ts_end out to the
            # NEXT row's ts and growing the duration by that gap.  Next row is
            # found by TIME because Alarm rows reuse the OK cycle's seq.
            if hit.get("is_ng"):
                cur.execute(
                    f"SELECT ts FROM {tbl_log} "
                    f"WHERE record_date = %s AND ts > %s "
                    f"ORDER BY ts ASC LIMIT 1",
                    (record_date, _ts_end),
                )
                _nx = cur.fetchone()
                if _nx and _nx["ts"] and _nx["ts"] > _ts_end:
                    _gap = (_nx["ts"] - _ts_end).total_seconds()
                    _ts_end    = _nx["ts"]
                    _clip_dur += _gap
            _clip_cap     = float(os.environ.get("CLIP_MAX_SECONDS", "120") or 120)
            if _clip_cap > 0 and _clip_dur > _clip_cap:
                _clip_dur = _clip_cap
            _ts_start     = _ts_end - _td(seconds=_clip_dur)
            # 2026-07-01 — Resolve THIS line's OWN main camera instead of a
            # hardcoded default.  The old `cam_panasonic_default` made EVERY
            # line pull the .115 (YNC) feed — e.g. YCA-SS Final Inspection
            # (.96) wrongly showed .115 instead of its bound .116 camera.
            # Prefer the main PLC's nf2_camera_id (pinned via
            # mes_lines.dashboard_plc_id, else parent_plc_id IS NULL); fall
            # back to the main PLC's IP so the CMS resolves it via
            # camera_config_bindings.  Never another line's camera.
            cur.execute(
                "SELECT NULLIF(TRIM(pc.nf2_camera_id),'') AS cam, pc.plc_ip "
                "FROM mes_plc_configs pc JOIN mes_lines l ON l.id = pc.line_id "
                "WHERE pc.line_id = %s AND ("
                "  (l.dashboard_plc_id IS NOT NULL AND pc.id = l.dashboard_plc_id) "
                "  OR (l.dashboard_plc_id IS NULL AND pc.parent_plc_id IS NULL)) "
                "ORDER BY (pc.parent_plc_id IS NULL) DESC LIMIT 1",
                (line_id,),
            )
            _cr  = cur.fetchone() or {}
            _cam = (_cr.get("cam") or "").strip()
            _sel = (f"camera_id={_cam}" if _cam
                    else f"plc_ip={(_cr.get('plc_ip') or '').strip()}")
            _qs = (f"{_sel}"
            ...
            try:
                ...
                pass
            try:
                # 2026-08-10 (r2) — stream=True REMOVED.  requests.get() builds a
                # throwaway Session and closes it on return; with stream=True the body
                # is still unread at that point, so when we later touch r.content the
                # connection is handed back to an already-closed pool and the socket FD
                # is never released.  That was ~13 leaked sockets/min (clip_prefetch
                # calls this endpoint for every active line every 60 s).  Both call
                # sites here buffer r.content anyway, so streaming bought nothing.
                r = requests.get(upstream, headers=fwd, timeout=15)
            except Exception as exc:
                # Network blip — fall through to by-part if part_code exists.
                r = None
                _last_exc = exc
            # 2026-08-10 — SOCKET LEAK FIX.  `stream=True` keeps the upstream
            # connection checked out of urllib3's pool until the body is read
            # or close() is called.  The success path below reads r.content, but
            # this >=400 fall-through did neither, so every 404 from the CMS
            # leaked one socket.  clip_prefetch hits this endpoint every 60 s
            # for every active line, and ~7 of those 404 each round — that is
            # the 8,500 stranded sockets / 8.9 GB RSS the API grew to in 48 h.
            ...
            if r is not None and r.status_code < 400:
                ...
                # 2026-07-31 — a cycle's clip never changes, so let the browser
                # (and a Cloudflare cache rule) reuse it instead of re-pulling it
                # across the site's single internet uplink on every replay.
                ...
                print(f"[CYCLE-VIDEO] line={line_id} cycle_seq={cycle_seq} "
                      ...
                      flush=True)
                # 2026-07-31 — buffer the (small, 1-20MB) clip and return a
                # self-terminating Response with a real Content-Length +
                # Connection: close.  The old StreamingResponse(iter_content)
                # left the client/tunnel connection open ~30s after the last
                # byte (framing never closed), tying up a connection per click
                # through Cloudflare.  Box has 233 GB RAM; buffering is instant.
    ...
    try:
        r = requests.get(upstream, headers=fwd_headers, timeout=15)   # see note above
    except Exception as exc:
        raise HTTPException(502, f"Upstream unreachable: {exc}")

    # 2026-08-10 — close the streamed upstream before raising, else the socket
    # stays checked out for the life of the process (see the fix note above).
    if r.status_code >= 400:
        _detail = (f"Video not found for part_code={part_code}"
                   if r.status_code == 404 else f"Upstream error: {r.text[:200]}")
        _code = r.status_code
        try: r.close()
        except Exception: pass
        raise HTTPException(_code, _detail)
    ...
    resp_headers["Access-Control-Expose-Headers"] = "X-Video-Source, X-Video-Offset"
    resp_headers["Cache-Control"] = "public, max-age=3600"   # immutable per cycle
    print(f"[CYCLE-VIDEO] line={line_id} cycle_seq={cycle_seq} "
          f"by-part fallback offset={ng_offset_sec:.1f}s pc={part_code}",
          flush=True)

    resp_headers["Connection"] = "close"   # buffered, self-terminating (see above)
    return Response(
        content=r.content,
        status_code=r.status_code,
        media_type=resp_headers.get("Content-Type", "video/mp4"),
...
def get_line_realtime(line_id: int, user=Depends(get_current_user_optional)):
    ...
    with get_conn() as conn:
        ...
        # per poll (~150ms each on the 192.168.30.10 DB).
        ...
        # collection). Querying a missing table raises psycopg2 UndefinedTable →
        # HTTP 500 on EVERY dashboard poll, which floods the log with tracebacks
        # and ties up a pool connection in a failed transaction + rollback each
        # time — that pool pressure slows /realtime for ALL lines (the whole
        # dashboard feels like "no data"). Fail fast + clean with a 404 instead:
        # a cheap catalog lookup, no error, and the frontend already renders a
        # non-200 line as an empty tile. Nothing is created or modified here.
        cur.execute("SELECT to_regclass(%s) AS reg", (table,))
        if not (table and cur.fetchone()["reg"]):   # RealDictCursor → access by key
            raise HTTPException(404, f"line {line_id} not provisioned (missing table {table})")
        ...
        # 2026-08-12 — SHIFT IS OVER, BOARD STAYS UP.  Everything above only
        # looks at NON-completed rows, so the instant a shift closed this
        # returned the empty "UNKNOWN" structure and the whole dashboard went
        # blank — precisely when the shift is being reviewed.  Hold the last
        # completed shift instead; the only blank period is the few minutes
        # before the next shift starts (see routers/wallboard.py).
        if not data:
            try:
                from routers.wallboard import _in_preshift_blank
                _blank = _in_preshift_blank(line_id, conn)
            except Exception:
                _blank = False
            if not _blank:
                cur.execute(f"""
        ...
        # 2026-08-12 — ...AND THE GAP SHIFT COUNTS AS OVER TOO.  The block above
        # only fires when nothing is open, but between two shifts a GAP_* row is
        # open and empty, so this endpoint happily served its zeros for the
        # whole gap (YNC, 17:15-18:30) and the dashboard looked blank anyway.
        # Hold the production shift that just finished instead.
        try:
            from routers.wallboard import _hold_last_production_shift
            data = _hold_last_production_shift(table, data, line_id, conn)
        except Exception as exc:
            print(f"[LINES] gap hold skipped: {exc}")
        if not data:
        ...
        # 2026-09-05 — OEE quality fallback. On register-mode / non-per-cycle
        # lines the collector's OEE uses a per-cycle OK counter that stays 0, so
        # it writes quality_oee = 0 → overall_oee = 0 even though the line has
        # real OK/NG plus valid availability & performance. The OEE tile then
        # shows "—". When OEE is 0 but the line actually produced parts,
        # recompute quality + overall from the SAME OK/NG already displayed
        # (ok/(ok+ng) — exactly the collector's cycle-mode formula). This only
        # fills a missing metric; no part count is read or changed, and lines
        # that already have a real OEE are left untouched.
        try:
            if not float(data.get("overall_oee") or 0) and float(data.get("ok_count") or 0) > 0:
                _ok = float(data.get("ok_count") or 0)
                _ng = float(data.get("ng_count") or 0)
                _av = float(data.get("availability") or 0)
                _pf = float(data.get("performance") or 0)
                _q  = (_ok / (_ok + _ng) * 100) if (_ok + _ng) > 0 else 100.0
                data["quality_oee"] = round(_q, 2)
                data["overall_oee"] = round(_av * _pf * _q / 10000, 2)
        except Exception:
            pass
        ...
        if data.get("shift_name"):
            cur.execute(
                """SELECT ot_start_time, ot_end_time, ot_plan,
            ...
            if ot_row:
                ...
                data["ot_plan"]       = ot_row.get("ot_plan")   # manual OT target (pcs); None if unset
        ...
        # ── ANDON-based breakdown + OEE (DISPLAY-ONLY, andon-covered lines) ──
        # Operator (2026-08-30): on lines that have physical andon hardware the
        # "breakdown" loss = the andon Maintenance+Toolroom call durations, and
        # OEE is recomputed from it.  Collector/counting is NOT touched — this
        # only rewrites the numbers we SHOW, and only for andon-covered lines.
        # Fully guarded: any failure leaves the collector's values as-is.
        try:
            from routers.andon import (andon_line_set, recompute_oee_with_andon,
                                       andon_open_call, _norm as _anorm)
            _sn = data.get("shift_name")
            _rd = data.get("record_date")
            if _sn and _rd and not str(_sn).upper().startswith(("GAP", "UNKNOWN")):
                ...
                if _lname and _anorm(_lname) in andon_line_set():
                    ...
                    if _sc and _sc.get("start_time") and _sc.get("end_time"):
                        ...
                        _rdate = _rd if hasattr(_rd, "year") else _dt.fromisoformat(str(_rd)).date()
                        _start = _dt.combine(_rdate, _sc["start_time"])
                        _end   = _dt.combine(_rdate, _sc["end_time"])
                        if _sc["xm"] or _end <= _start:
                            _end += _td(days=1)
                        ...
                        if _res:
                            ...
                            if _res["availability"] is not None:
                                ...
                                data["performance"]  = _res["performance"]
                                data["overall_oee"]  = _res["overall_oee"]
                                data["oee_grade"]    = _res["oee_grade"]
        except Exception as _abx:
            print(f"[LINES] andon breakdown override skipped: {_abx}")

        # 2026-09-15 — SUB-ASSEMBLY plan fix. On sub-assembly lines the collector
        # does NOT maintain shift_plan_completed (the running "plan so far"), so
        # the top Plan + hourly TOTAL showed 0 while the per-hour plans were
        # correct. Compute it here from the current shift's hour_*_plan windows
        # (past slots full + current slot pro-rated), exactly how normal lines
        # ramp. Scoped to the Sub-Assembly zone and only when the collector value
        # is 0/behind, so no other line is affected. Display-only, no DB write.
        try:
            if (data.get("shift_plan_completed") or 0) <= 0 and data.get("shift_name") \
               and not str(data.get("shift_name")).upper().startswith(("GAP", "UNKNOWN")):
                cur.execute("SELECT z.zone_name FROM mes_lines l "
                            "LEFT JOIN mes_zones z ON z.id = l.zone_id WHERE l.id=%s",
                            (line_id,))
                _zr = cur.fetchone()
                # Sub-Assembly AND Loop Pipe are register-mode lines whose
                # collector doesn't maintain shift_plan_completed (both showed
                # top Plan / hourly TOTAL = 0). Same hour_*_plan compute for both.
                if _zr and (_zr.get("zone_name") or "").strip().lower() in ("sub-assembly", "loop pipe"):
                    cur.execute("""SELECT start_time, end_time,
                                          COALESCE(crosses_midnight,false) AS xm
                                     FROM mes_shift_configs
                                    WHERE line_id=%s AND shift_name=%s""",
                                (line_id, data["shift_name"]))
                    _sc2 = cur.fetchone()
                    if _sc2 and _sc2.get("start_time") and _sc2.get("end_time"):
                        import re as _re2
                        from datetime import datetime as _dt2
                        _st = _sc2["start_time"]; _et = _sc2["end_time"]
                        s_start = _st.hour * 60 + _st.minute
                        s_end   = _et.hour * 60 + _et.minute
                        window  = (s_end - s_start) % 1440
                        if _sc2["xm"] or window == 0:
                            window = window or 1440
                        _now = _dt2.now()
                        now_rel = ((_now.hour * 60 + _now.minute) - s_start) % 1440
                        total = 0.0
                        for _k, _v in list(data.items()):
                            _m = _re2.match(r"^hour_(\d{2})(\d{2})_(\d{2})(\d{2})_plan$", _k)
                            if not _m or not _v:
                                continue
                            _sh, _sm, _eh, _em = (int(g) for g in _m.groups())
                            _s0 = _sh * 60 + _sm
                            _e0 = _eh * 60 + _em
                            _dur = (_e0 - _s0) % 1440 or 60
                            _srel = (_s0 - s_start) % 1440
                            if _srel >= window:        # belongs to the OTHER shift
                                continue
                            if now_rel >= _srel + _dur:
                                _frac = 1.0
                            elif now_rel <= _srel:
                                _frac = 0.0
                            else:
                                _frac = (now_rel - _srel) / _dur
                            total += float(_v) * _frac
                        if total > 0:
...
def update_line(line_id: int, body: LineUpdate, admin=Depends(require_admin)):
    ...
    with get_conn() as conn:
        try:
            ...
            # (plant_id, line_code) is UNIQUE.  Say which field clashed instead
            # of letting the panel show a bare 500.
            raise HTTPException(
                409, f"Line code '{updates.get('line_code')}' already exists in "
                     f"this plant — pick a different code.")
        # 2026-07-16 — keep is_production in sync with active_shifts so the
        # PLAN (which sums is_production shifts) automatically reflects which
        # shifts the line actually runs.  Un-ticking a shift in Active Shifts
        # now drops it from the plan; re-ticking restores it.  GAP shifts are
        # never touched (they stay non-production).
        if "active_shifts" in updates:
            active = [s.strip() for s in (updates["active_shifts"] or "").split(",") if s.strip()]
            conn.cursor().execute("""
                UPDATE mes_shift_configs
                   SET is_production = (shift_name = ANY(%s))
                 WHERE line_id = %s AND shift_name NOT LIKE 'GAP%%'
            """, (active, line_id))
        conn.cursor().execute("""
            INSERT INTO mes_audit_log (action, entity_type, entity_id, details,
                                       user_id, username)
...
@router.get("/{line_id}/model-ct")
def get_model_ct(line_id: int, user=Depends(get_current_user_optional)):
    """Per-model ideal cycle time for a line — drives the model-aware
    Fullscreen (plan/estimation follow the operator-selected model).
    Read-only; empty list when no per-model CT is configured."""
    with get_conn() as conn:
        cur = dict_cursor(conn)
        cur.execute("""
            SELECT model_number, model_name, ideal_ct
            FROM mes_model_mappings
            WHERE line_id = %s AND ideal_ct IS NOT NULL
            ORDER BY model_number
        """, (line_id,))
        return [{"model_number": r["model_number"], "model_name": r["model_name"],
                 "ideal_ct": float(r["ideal_ct"])} for r in cur.fetchall()]


class ModelCTItem(BaseModel):
    model_number: int
    ideal_ct:     Optional[float] = None
    model_name:   Optional[str]   = None


@router.put("/{line_id}/model-ct")
def put_model_ct(line_id: int, body: List[ModelCTItem], admin=Depends(require_admin)):
    """Set per-model ideal cycle time (admin — Models tab, YWD-SS model-aware
    plan).  Upserts mes_model_mappings.ideal_ct per model_number.  ideal_ct
    None clears it (model falls back to the line CT)."""
    with get_conn() as conn:
        cur = conn.cursor()
        for it in body:
            ct = it.ideal_ct if (it.ideal_ct and it.ideal_ct > 0) else None
            cur.execute("""UPDATE mes_model_mappings SET ideal_ct = %s
                           WHERE line_id = %s AND model_number = %s""",
                        (ct, line_id, it.model_number))
            if cur.rowcount == 0 and it.model_name:
                cur.execute("""INSERT INTO mes_model_mappings (line_id, model_number, model_name, ideal_ct)
                               VALUES (%s, %s, %s, %s)""",
                            (line_id, it.model_number, it.model_name, ct))
    return {"ok": True, "count": len(body)}


def _model_group(name: str) -> str:
    """Collapse concrete model names into a CT-band group.  INR RH / INR LH
    share one cycle time (25.7s) so they count as one plan-rate; OTR is the
    other (14.42s).  Used only for the model-aware plan view (YWD-SS)."""
    n = (name or "").upper()
...
@router.get("/{line_id}/model-plan")
def get_model_plan(line_id: int,
                   date: Optional[str] = Query(None),
                   shift: Optional[str] = Query(None),
                   user=Depends(get_current_user_optional)):
    """Model-aware plan for a line, derived READ-ONLY from the per-cycle CT
    log (nothing on the collector changes).

    Each cycle in <db_table_name>_ct_log is labelled to the nearest configured
    per-model ideal CT (mes_model_mappings.ideal_ct) — INR ~25.7s vs OTR
    ~14.42s cluster ~2x apart, so classification is robust.  A light
    majority-window smoother removes single-cycle flips.  From the resulting
    model segments we compute: changeover events, a model-aware hourly plan
    (each slot's working-time split by which model actually ran, ÷ that
    model's CT), and the changeover's impact on the shift plan.

    Fully reload-proof (reads persisted ct_log), scoped to whichever line the
    caller asks for — the dedicated FullscreenYWD screen uses it for YWD-SS."""
    rec_date = date or _date.today().isoformat()
    ...
    with get_conn() as conn:
        ...
        if not lr:
            raise HTTPException(404, "Line not found")
        ct_tbl = f"{lr['db_table_name']}_ct_log"
        cur.execute("SELECT to_regclass(%s) AS reg", (f"public.{ct_tbl}",))
        if not cur.fetchone()["reg"]:
            return {"enabled": False, "reason": "no ct_log", "segments": [],
                    "changeovers": [], "hourly": [], "totals": None}

        # configured per-model CT → distinct bands {ct, group}
        cur.execute("""SELECT model_name, ideal_ct FROM mes_model_mappings
                       WHERE line_id = %s AND ideal_ct IS NOT NULL""", (line_id,))
        band_by_ct = {}
        for r in cur.fetchall():
            ct = round(float(r["ideal_ct"]), 2)
            band_by_ct.setdefault(ct, _model_group(r["model_name"]))
        bands = [{"ct": ct, "group": g} for ct, g in band_by_ct.items()]
        if len(bands) < 2:
            return {"enabled": False, "reason": "need >=2 per-model CT", "segments": [],
                    "changeovers": [], "hourly": [], "totals": None}
        ct_of_group = {}
        for b in bands:
            ct_of_group.setdefault(b["group"], b["ct"])

        # resolve shift (default = shift with most cycles today)
        if not shift:
            cur.execute(f"""SELECT shift_name, count(*) c FROM {ct_tbl}
                            WHERE record_date = %s GROUP BY shift_name
                            ORDER BY c DESC LIMIT 1""", (rec_date,))
            sr = cur.fetchone()
            shift = sr["shift_name"] if sr else "A"

        # hourly slots for this shift (working_minutes excludes breaks)
        cur.execute("""SELECT slot_label, start_time, end_time, crosses_midnight,
                              working_minutes, plan_pieces, db_column_prefix, slot_order
                       FROM mes_hourly_slots
                       WHERE line_id = %s AND shift_name = %s
                       ORDER BY slot_order""", (line_id, shift))
        slots = cur.fetchall()

        # per-cycle CT stream for the day+shift
        cur.execute(f"""SELECT ts, ct_value, is_ng FROM {ct_tbl}
                        WHERE record_date = %s AND shift_name = %s
    ...
    base = datetime.fromisoformat(rec_date)
    def _slot_bounds(s):
        st, en = s["start_time"], s["end_time"]
        ds = base.replace(hour=st.hour, minute=st.minute, second=0, microsecond=0)
        de = base.replace(hour=en.hour, minute=en.minute, second=0, microsecond=0)
        if de <= ds:
            de += timedelta(days=1)
        return ds, de

    now = datetime.now()
    shift_end = max((_slot_bounds(s)[1] for s in slots), default=now)
    shift_start = min((_slot_bounds(s)[0] for s in slots), default=now)

    # ── classify each cycle to nearest band, then majority-window smooth ──
    def _nearest(v):
        return min(bands, key=lambda b: abs(v - b["ct"]))["group"]
    raw = [(r["ts"], _nearest(float(r["ct_value"])), bool(r["is_ng"])) for r in rows]
    W = 2  # +/-2 cycle window
    smoothed = []
    for i in range(len(raw)):
        window = [raw[j][1] for j in range(max(0, i-W), min(len(raw), i+W+1))]
        # majority vote
        g = max(set(window), key=window.count)
        smoothed.append((raw[i][0], g))

    # ── run-length encode into segments ──
    segments = []
    for ts, g in smoothed:
        if segments and segments[-1]["group"] == g:
            segments[-1]["end_ts"] = ts
            segments[-1]["cycles"] += 1
        else:
            segments.append({"group": g, "ct": ct_of_group.get(g),
                             "start_ts": ts, "end_ts": ts, "cycles": 1})

    # ── drop spurious short runs ──
    # End-of-shift model-test cycles and MC frame-desync blips create tiny
    # 1-5 cycle flips that would otherwise be reported as real changeovers.
    # Absorb any run below MIN_SEG cycles into the run before it (relabel to
    # that model), then re-merge adjacent same-model runs.  A genuine model
    # change lasts far longer than this, so only noise is removed.
    MIN_SEG = 15
    _clean = []
    for seg in segments:
        if _clean and (seg["cycles"] < MIN_SEG or _clean[-1]["group"] == seg["group"]):
            _clean[-1]["end_ts"] = seg["end_ts"]
            _clean[-1]["cycles"] += seg["cycles"]
        else:
            _clean.append(dict(seg))
    # a short FIRST run gets folded into the next real run
    if len(_clean) >= 2 and _clean[0]["cycles"] < MIN_SEG:
        _clean[1]["start_ts"] = _clean[0]["start_ts"]
        _clean[1]["cycles"] += _clean[0]["cycles"]
        _clean.pop(0)
    segments = _clean

    # first segment is assumed to have run from shift start
    if segments:
        segments[0]["start_ts"] = min(segments[0]["start_ts"], shift_start)

    cur_group = segments[-1]["group"] if segments else None

    # ── effective full-shift timeline (past segments + projected current model) ──
    eff = [dict(g=s["group"], ct=s["ct"],
                a=s["start_ts"], b=segments[i+1]["start_ts"] if i+1 < len(segments) else min(now, shift_end))
           for i, s in enumerate(segments)]
    if cur_group and now < shift_end:
        eff.append(dict(g=cur_group, ct=ct_of_group.get(cur_group), a=now, b=shift_end))

    def _overlap(a1, b1, a2, b2):
        lo, hi = max(a1, a2), min(b1, b2)
        return max(0.0, (hi - lo).total_seconds())

    # ── changeovers (group flips) ──
    changeovers = []
    for i in range(1, len(segments)):
        if segments[i]["group"] != segments[i-1]["group"]:
            changeovers.append({"from": segments[i-1]["group"], "to": segments[i]["group"],
                                "at": segments[i]["start_ts"].strftime("%H:%M"),
                                "ct": segments[i]["ct"]})

    # ── model-aware hourly ──
    hourly = []
    plan_full = 0.0
    by_group_work = {}   # group -> working_seconds
    for s in slots:
        ds, de = _slot_bounds(s)
        wall = (de - ds).total_seconds() or 1.0
        wm_sec = (float(s["working_minutes"] or 0)) * 60.0
        slot_plan = 0.0
        grp_overlap = {}
        for e in eff:
            ov = _overlap(ds, de, e["a"], e["b"])
            if ov <= 0 or not e["ct"]:
                continue
            frac = ov / wall
            wsec = wm_sec * frac
            slot_plan += wsec / e["ct"]
            grp_overlap[e["g"]] = grp_overlap.get(e["g"], 0.0) + ov
            by_group_work[e["g"]] = by_group_work.get(e["g"], 0.0) + wsec
        # actual OK cycles that fall in this slot
        act = sum(1 for r in rows if ds <= r["ts"] < de and not r["is_ng"])
        dom = max(grp_overlap, key=grp_overlap.get) if grp_overlap else None
        # if no model data at all for this slot, fall back to the flat plan
        if slot_plan <= 0 and (s["plan_pieces"] or 0):
            slot_plan = float(s["plan_pieces"])
        plan_full += slot_plan
        hourly.append({
            ...
            "group": dom,
            "plan_flat": s["plan_pieces"],
            "plan": round(slot_plan),
            "actual": act,
            "started": ds <= now,
        })

    # ── totals + changeover impact ──
    total_work_sec = sum((float(s["working_minutes"] or 0)) * 60.0 for s in slots)
    # plan up to now = sum of each slot's past-portion (segments ∩ [slot_start, now])
    plan_now = 0.0
    for s in slots:
        ds, de = _slot_bounds(s)
        if ds > now:
        ...
        cap = min(de, now)
        for e in eff:
            ov = _overlap(ds, cap, e["a"], e["b"])
            if ov > 0 and e["ct"]:
                plan_now += (wm_sec * ov / wall) / e["ct"]

    actual_ok = sum(1 for r in rows if not r["is_ng"])
    first_group = segments[0]["group"] if segments else cur_group
    baseline_ct = ct_of_group.get(first_group)
    baseline = (total_work_sec / baseline_ct) if baseline_ct else 0
    if_all = {g: round(total_work_sec / c) for g, c in ct_of_group.items()}
    by_group = [{"group": g, "ct": ct_of_group.get(g),
                 "working_min": round(sec/60), "plan": round(sec/ct_of_group[g]) if ct_of_group.get(g) else 0}
                for g, sec in by_group_work.items()]
    ...
    return {
        ...
        "current_ct": ct_of_group.get(cur_group) if cur_group else None,
        "segments": [{"group": s["group"], "ct": s["ct"],
                      "start": s["start_ts"].strftime("%H:%M"),
                      "end": (segments[i+1]["start_ts"] if i+1 < len(segments) else min(now, shift_end)).strftime("%H:%M"),
                      "cycles": s["cycles"]} for i, s in enumerate(segments)],
        "changeovers": changeovers,
        "hourly": hourly,
        "totals": {
            "actual": actual_ok,
            "plan_now": round(plan_now),
            "plan_full": round(plan_full),
            "variance": actual_ok - round(plan_now),
            "baseline_single": {"group": first_group, "plan": round(baseline)},
            "if_all": if_all,
            "by_group": by_group,
            "impact": round(plan_full - baseline),
        },
    }
...
def provision(line_id: int, admin=Depends(require_admin)):
    # Same lock guard as restart: a line that was just stopped still holds a
    # warm lock for ~30 s and the new collector would refuse to start.
    _wait_for_lock_release(line_id)
    try:
...
def _wait_for_lock_release(line_id: int, timeout_s: float = 50.0) -> None:
    """Block until this line's collector lock is gone or stale.

    collector_engine refuses to start while another collector holds a lock with
    a heartbeat newer than _HEARTBEAT_STALE_AFTER_SEC (30 s) — a guard against
    two collectors fighting over one PLC.  A restart therefore cannot simply
    stop-then-start: the old process dies instantly but its lock row stays warm
    for another half minute, and the new one exits with "Another collector is
    already running".  Wait it out instead of failing the button.
    """
    import time as _t
    ...
    while _t.time() < deadline:
        with get_conn() as conn:
            ...
            cur.execute(
                ...
                "FROM mes_collector_locks WHERE line_id = %s", (line_id,))
            row = cur.fetchone()
        if not row or float(row["age"] or 0) > 31:
            return
        _t.sleep(2)
...
# ── per-line CMS (camera) restart ───────────────────────────────────────────
# 2026-09-17 — operator: "jaise har line ka collector restart option hai,
# vaise hi CMS me bhi de do har line ka."
#
# CMS is ONE process serving every camera, so there is no per-line CMS to
# restart — restarting it would interrupt all 20 lines to fix one.  What a
# line actually needs is ITS cameras' recorders restarted, and CMS already
# has the mechanism: POST /api/internal/video-line with enabled=false kills
# that line's recorders (plc_monitor._kill_cam), and enabled=true lets the
# watchdog respawn them on its next tick.  This drives that pair, so the
# button behaves like the collector one without touching CMS code.
CMS_BASE = os.environ.get("CMS_BASE", "http://127.0.0.1:5555")


def _cms_video_line(line_name: str, enabled: bool) -> dict:
    import requests
    r = requests.post(f"{CMS_BASE}/api/internal/video-line",
                      json={"line": line_name, "enabled": enabled}, timeout=30)
    r.raise_for_status()
    return r.json()


@router.post("/{line_id}/cms-restart")
def cms_restart(line_id: int, admin=Depends(require_admin)):
    """Restart just this line's camera recorders."""
    import time as _t
    with get_conn() as conn:
        cur = dict_cursor(conn)
        cur.execute("SELECT line_name FROM mes_lines WHERE id = %s", (line_id,))
        row = cur.fetchone()
    if not row or not row.get("line_name"):
        raise HTTPException(404, "Line not found")
    name = row["line_name"]

    try:
        off = _cms_video_line(name, False)
    except Exception as e:
        raise HTTPException(502, f"CMS did not accept the stop: {e}")

    stopped = (off.get("data") or off).get("stopped_recorders", 0)
    # Let the camera drop its RTSP session before reconnecting.  Single-session
    # cameras refuse a second login while the first is still open, which is how
    # a "restart" ends up leaving a line with no recording at all.
    _t.sleep(8)
    try:
        _cms_video_line(name, True)
    except Exception as e:
        # Re-enable is the half that must not be skipped — a line left disabled
        # records nothing until somebody notices.
        raise HTTPException(
            502, f"Recorders stopped but re-enable failed for {name}: {e} — "
                 f"re-enable it from Admin or run the CMS restart")
    return {"ok": True, "line": name, "stopped_recorders": stopped,
            "note": "Recorders restarted — they reconnect within ~30 s"}
...
def add_machine(line_id: int, body: MachineCreate, admin=Depends(require_admin)):
    ...
    with get_conn() as conn:
        ...
        cur.execute("""
            INSERT INTO mes_plc_configs
                (line_id, machine_name, plc_ip, plc_port, protocol,
                 ...
                 sa_result_register, sa_result_ok_value, sa_result_ng_value,
                 fi_sa_ng_bit, fi_sa_ng_bit_hold_sec, fi_fetch_bit,
        ...
        """, (line_id, body.machine_name, body.plc_ip, body.plc_port, body.protocol,
              ...
              (body.sa_shift_data_bit or None), (body.sa_shift_reset_bit or None),
              bool(body.is_bottleneck),
              (body.lc_result_reg or None), (body.lc_value_reg or None),
              (body.qr_reg or None), (body.reject_bit or None),
              (body.sa_result_register or None), body.sa_result_ok_value,
              body.sa_result_ng_value,
              (body.fi_sa_ng_bit or None), body.fi_sa_ng_bit_hold_sec,
              (body.fi_fetch_bit or None),
              (body.sa_ok_bit or None), (body.sa_ng_bit or None),
              (body.sa_ng_trigger_bit or None)))
    ...
    with get_conn() as conn:
        ...
        conn.cursor().execute("""
            UPDATE mes_plc_configs SET
                ...
                sa_shift_data_bit=%s, sa_shift_reset_bit=%s,
                is_bottleneck=%s,
                lc_result_reg=%s, lc_value_reg=%s, qr_reg=%s, reject_bit=%s,
                sa_result_register=%s, sa_result_ok_value=%s, sa_result_ng_value=%s,
                fi_sa_ng_bit=%s, fi_sa_ng_bit_hold_sec=%s, fi_fetch_bit=%s,
                sa_ok_bit=%s, sa_ng_bit=%s, sa_ng_trigger_bit=%s,
                updated_at=NOW()
            WHERE id=%s
...
_SEMI_AUTO_SCHEMA_READY = False


def _ensure_semi_auto_schema(conn) -> None:
    """Idempotent migration for Semi-Auto data-capture on sub-machines.
    Adds optional columns to mes_plc_configs + creates the
    mes_submachine_data_log table with appropriate indexes.

    2026-08-17 — RUNS ONCE PER PROCESS, not on every endpoint hit.
    The old docstring claimed it was "safe to call on every endpoint hit —
    Postgres skips the work if already present".  That is true of the WORK and
    false of the LOCK: `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` still takes an
    ACCESS EXCLUSIVE lock on mes_plc_configs even when every column is already
    there.  Normally it is granted instantly and nobody notices.

    On 2026-08-17 two clip-lookup connections went idle-in-transaction while
    holding a lock on that table.  This ALTER queued behind them for an
    ACCESS EXCLUSIVE — and because the Postgres lock queue is FIFO, every
    ordinary `SELECT * FROM mes_plc_configs` that arrived afterwards queued
    behind the ALTER, even though the readers were compatible with each other.
    146 queries blocked, 194 connections, and every line's dashboard went dark.

    The table is the hottest one in the system (each dashboard reads it per
    line), so it must not be handed an exclusive lock request per request.
    The guard mirrors _ENERGY_PER_PART_COL_READY directly above.
    """
    global _SEMI_AUTO_SCHEMA_READY
    if _SEMI_AUTO_SCHEMA_READY:
        return
    try:
        cur = conn.cursor()
        # Never queue for the lock.  If something else holds mes_plc_configs we
        # would rather this migration fail and retry on the next call than
        # become the head of a queue that stalls every reader behind it.
        ...
        cur.execute("""
            ALTER TABLE mes_plc_configs
              ...
              -- 2026-08-06 — SHIFT-WISE Semi-Auto capture (operator spec:
              -- "jaise baaki machines mein D-bit se data lete aur agli shift
              -- se pehle reset karte hain, waise hi semi auto mein bhi").
              -- sa_shift_data_bit : the D-bit whose rising edge means "the
              --   shift's accumulated data block is ready to be read".  Read
              --   once per shift instead of once per cycle.
              -- sa_shift_reset_bit: the L-bit the collector pulses AFTER the
              --   shift's data has been stored and BEFORE the next shift
              --   starts, so the PLC zeroes that block for the new shift.
              --   Mirrors the existing register-mode shift_reset_bit (L110)
              --   pattern, but scoped to the Semi-Auto data block.
        ...
        conn.commit()
        cur.close()
        _SEMI_AUTO_SCHEMA_READY = True
    except Exception as exc:
        try: conn.rollback()
        except Exception: pass
        # Flag stays False so a genuine failure is retried — but a lock timeout
        # now costs 3 s on one request instead of stalling every reader.
...
class OTConfigEntry(BaseModel):
    ...
    ot_plan:       Optional[int] = None   # manual OT target (pcs) for the shift's OT window
...
def get_cycle_extremes(
    ...
    with get_conn() as conn:
        ...
        # 2026-09-14 — Guard unprovisioned lines (per-line table never created).
        # The shift-resolve query below reads {base} directly (outside the
        # try/except further down), so a missing table raised UndefinedTable →
        # HTTP 500 on every 15s poll for lines 37/39/40/41. Return empty here.
        cur.execute("SELECT to_regclass(%s) AS reg", (base,))
        if not (base and cur.fetchone()["reg"]):
            return {"min": None, "max": None, "shift": shift,
...
def save_ot_config(line_id: int, body: List[OTConfigEntry], user=Depends(get_current_user)):
    ...
    with get_conn() as conn:
        cur = conn.cursor()
        for entry in body:
            # Coerce ot_plan to a non-negative int (or NULL).  A blank field
            # from the UI arrives as None → stored NULL (no OT target set).
            ot_plan = entry.ot_plan
            if ot_plan is not None:
                try:
                    ot_plan = max(0, int(ot_plan))
                except (TypeError, ValueError):
                    ot_plan = None
            cur.execute("""
                UPDATE mes_shift_configs
            ...
            """, (entry.ot_start_time or None, entry.ot_end_time or None,
                  ot_plan, line_id, entry.shift_name))
        conn.commit()
        cur.close()
    return {"ok": True}
...
def set_ot_active(line_id: int, body: OTActiveBody, user=Depends(get_current_user)):
    ...
    with get_conn() as conn:
        cur = conn.cursor()
        # Validate: when activating, the shift must be a REAL production shift
        # of THIS line (not a GAP, not an arbitrary string).  Without this the
        # collector silently ignored a bad value (it guards with `in shifts`)
        # but we'd persist garbage in mes_lines.ot_active_shift.
        if shift is not None:
            cur.execute("""
                SELECT 1 FROM mes_shift_configs
                WHERE line_id = %s AND shift_name = %s
                  AND is_production = true AND shift_name NOT LIKE 'GAP%%'
            """, (line_id, shift))
            if not cur.fetchone():
                raise HTTPException(400, f"'{shift}' is not a production shift on this line")
        cur.execute(
        ...
        conn.commit()
        cur.close()
    return {"ok": True, "ot_active_shift": shift}
...
# Set once the table + all migration columns are confirmed present, so the
# schema DDL below runs at most ONCE per process instead of on every request.
_cycle_comments_ready = False

def _ensure_cycle_comments_table(conn):
    """Create the comments table + migrate columns, at most ONCE per process.

    2026-09-05 — Fix intermittent HTTP 500 on the comment endpoints
    (Comments History page + the dashboard video comment box). This used to run
    `ALTER TABLE mes_cycle_comments ADD COLUMN IF NOT EXISTS ...` on EVERY call
    (every comment read AND write). `ADD COLUMN` needs an ACCESS EXCLUSIVE lock
    even when the column already exists, so while operators were commenting and
    wallboards were polling, the ALTER could not get the lock and was cancelled
    with `LockNotAvailable: canceling statement due to lock timeout` → 500.

    Now: skip entirely once the schema is confirmed (process-level flag), and
    even on the first call only ALTER columns that are genuinely missing
    (checked with a cheap, lock-free catalog query). A warm production DB does
    zero table-rewriting DDL here. A transient lock hiccup is swallowed (the
    table already exists) so it never fails the request."""
    global _cycle_comments_ready
    if _cycle_comments_ready:
        return
    try:
        cur = conn.cursor()
        cur.execute("""
        ...
        """)
        # shift + machine columns so Comments History shows the shift + machine
        # name. Only ALTER the columns that are actually missing — this avoids
        # taking an ACCESS EXCLUSIVE lock on a hot table when they already exist.
        cur.execute(
            "SELECT column_name FROM information_schema.columns "
            "WHERE table_name = 'mes_cycle_comments'")
        have = {r[0] for r in cur.fetchall()}
        for col, typ in (("shift_name", "TEXT"), ("machine_id", "INTEGER"),
                         ("machine_name", "TEXT"), ("record_date", "DATE"),
                         ("leader_name", "TEXT")):   # 2026-09-15 — leader tagged per cycle comment
            if col not in have:
                cur.execute(f"ALTER TABLE mes_cycle_comments ADD COLUMN {col} {typ}")
        conn.commit()
        _cycle_comments_ready = True
    except Exception:
        # Never let a transient DDL/lock hiccup fail the request — the table and
        # its columns already exist in production. Clear the aborted transaction
        # so the caller's real query runs; retry the migration next request.
        try: conn.rollback()
        except Exception: pass
...
def list_cycle_comments(
    ...
    2026-09-16 — SHIFT SCOPING.  `part_code` is `cycle_<seq>_<date>`, which does
    NOT carry the shift, so A-shift cycle #10 and B-shift cycle #10 of the SAME
    day shared one comment thread and each showed the other's notes (operator:
    "A shift ki #10 aur B shift ki #10 same nahi hai").  A cycle number restarts
    every shift, so the caller now passes the CYCLE's own shift and we scope the
    thread to it.  Omitting `shift` keeps the old unscoped behaviour so any
    caller that has not been updated still works."""
    safe_pc = (part_code or "").strip()
    if not safe_pc:
        raise HTTPException(400, "part_code is required")
    safe_shift = (shift or "").strip() or None
    with get_conn() as conn:
        _ensure_cycle_comments_table(conn)
        cur = dict_cursor(conn)
        if safe_shift:
            # 2026-09-17 — `OR shift_name IS NULL` keeps the 239 comments written
            # before this column existed (28 May - 08 Jun) readable.  Scoping the
            # thread to a shift is what stops an A-shift note appearing on the
            # B-shift cycle that happens to carry the same number, but a plain
            # `shift_name = %s` also hides every legacy row from BOTH shifts —
            # the notes were still in the table, just unreachable from the UI.
            # Un-shifted rows cannot be attributed to one shift, so they show in
            # either; that is the only option that does not lose them.
            cur.execute(
                "SELECT id, comment, author, leader_name, created_at "
                "FROM mes_cycle_comments "
                "WHERE line_id = %s AND part_code = %s "
                "  AND (shift_name = %s OR shift_name IS NULL) "
                "ORDER BY created_at ASC, id ASC",
                (line_id, safe_pc, safe_shift),
            )
        else:
            cur.execute(
                "SELECT id, comment, author, leader_name, created_at "
...
def add_cycle_comment(
    ...
    # Author = the LOGIN ID of whoever added the comment.
    # 2026-09-14 — `user` is a DICT (get_current_user returns id/username/role),
    # so the old getattr() never matched and every comment was saved as the
    # literal "operator". Use dict access so the actual username is stored.
    author = (
        (user.get("username") if isinstance(user, dict) else getattr(user, "username", None))
        or (user.get("name") if isinstance(user, dict) else getattr(user, "name", None))
        or "operator"
    )
    ...
    # 2026-09-15 — the leader this cycle's remark is attributed to. Defaults on
    # the UI to the line's shift-assigned leader (mes_leader_shift_alloc); the
    # operator can override it per-comment from the dropdown next to the box.
    leader_name = (str(body.get("leader_name") or "").strip() or None)
    _mid = body.get("machine_id")
    try:
        machine_id = int(_mid) if _mid not in (None, "", 0, "0") else None
    except Exception:
        machine_id = None
    # 2026-09-16 — STAMP THE CYCLE'S SHIFT, NOT THE CLOCK'S.  This used to take
    # whatever shift was running when the comment was SAVED, so reviewing an
    # A-shift cycle during B shift stamped the note "B" — and because the comment
    # thread key (`cycle_<seq>_<date>`) carries no shift, A-shift cycle #10 and
    # B-shift cycle #10 of the same day shared one thread and showed each other's
    # notes.  A cycle number restarts every shift, so the caller now sends the
    # cycle's OWN shift (and optionally its date) and we store that.  Falls back
    # to the running shift when the caller doesn't send one (older clients).
    body_shift = (str(body.get("shift") or "").strip() or None)
    body_date  = (str(body.get("record_date") or "").strip() or None)
    with get_conn() as conn:
        _ensure_cycle_comments_table(conn)
        shift_name, record_date = _current_shift(line_id, conn)
        if body_shift:
            shift_name = body_shift
        if body_date:
            record_date = body_date
        cur = dict_cursor(conn)
        cur.execute(
            "INSERT INTO mes_cycle_comments "
            "  (line_id, part_code, comment, author, "
            "   shift_name, machine_id, machine_name, record_date, leader_name) "
...
def save_ng_process_remark(
    ...
    # 2026-09-14 — store the LOGIN ID (username). The old "user_id" key does not
    # exist on the user dict (keys are id/username/role), so created_by saved NULL.
    author = (user.get("username") if isinstance(user, dict) else
              getattr(user, "username", None)) or "unknown"
...
def get_comments_history(
    ...
    rows = []
    # 2026-09-17 — validate the dates BEFORE they reach Postgres.  They were
    # passed straight through as strings, so `?date_from=notadate` came back
    # as a 500 (psycopg2 InvalidDatetimeFormat) — a client mistake reported
    # as a server crash, which sends whoever is on call hunting for a broken
    # query.  Found by pmaudit/chaos.py feeding malformed input everywhere.
    ...
    with get_conn() as conn:
        ...
        cur.execute(
            f"SELECT id, line_id, part_code, comment, author, leader_name, "
        ...
        # ── 3. Cycle time per row ───────────────────────────────────
        # The comment/remark tables don't store CT, so resolve it from the cycle
        # the part_code points at: main-line "cycle_<seq>_<date>" → the line's
        # own ct_log; sub-machine "M<mid>-C<seq>-<date>" → mes_submachine_ct_log.
        # 2026-09-16 — BATCHED (was N+1).  This used to run ONE ct_log query per
        # comment row: at limit=50 that is 50+ round-trips and made this the
        # slowest call in the app (measured 1.5-2.0 s, every other page endpoint
        # is 10-70 ms).  Now every row's key is parsed first, bucketed by the
        # table it needs, and each table is read in ONE query — a couple of
        # queries total regardless of how many rows are returned.
        import re as _re
        _tbl_by_line = {}
        cur.execute("SELECT id, db_table_name FROM mes_lines "
                    "WHERE db_table_name IS NOT NULL AND db_table_name <> ''")
        for _lr in cur.fetchall():
            _tbl_by_line[_lr["id"]] = _lr["db_table_name"]
        _ct_tbl_ok = {}
        def _ct_ok(tbl):
            if tbl not in _ct_tbl_ok:
                cur.execute("SELECT to_regclass(%s) AS e", (tbl,))
                _ct_tbl_ok[tbl] = bool(cur.fetchone()["e"])
            return _ct_tbl_ok[tbl]

        # The lookup is keyed on (cycle_seq, record_date, SHIFT).  The shift is
        # essential, not cosmetic: cycle_seq restarts every shift, so one
        # (seq, date) matches up to 5 ct_log rows (A / B / GAP) whose CTs are
        # wildly different (measured 7.53 / 22.34 / 349.19 s for one key).  The
        # old `LIMIT 1` picked an ARBITRARY one, so the Cycle Time column could
        # show another shift's cycle.  Grouping by (date, shift) also lets the
        # query ride the existing (record_date, shift_name) index — there is no
        # index on cycle_seq, so filtering on it alone would seq-scan.
        _main_groups = {}        # (tbl, date, shift) -> {cycle_seq}
        _sub_groups  = {}        # sub_plc_id -> {cycle_seq}
        _row_keys    = []        # parallel to `rows`
        for _row in rows:
            pc = str(_row.get("part_code") or "")
            key = None
            m = _re.match(r"^cycle_(\d+)_(\d{4}-\d{2}-\d{2})", pc)
            if m:
                base = _tbl_by_line.get(_row.get("line_id"))
                tbl = f"{base}_ct_log" if base else None
                if tbl:
                    _seq  = int(m.group(1))
                    _dt   = m.group(2)
                    _sh   = _row.get("shift_name") or None
                    key = ("main", tbl, _dt, _sh, _seq)
                    _main_groups.setdefault((tbl, _dt, _sh), set()).add(_seq)
            else:
                # 2026-09-19 — keyed on the DATE (and shift) too.  A sub-machine's
                # cycle_seq restarts every shift, so (sub_plc_id, cycle_seq) alone
                # matched up to 451 rows across 27 dates and the CT shown for an
                # NG remark came from an arbitrary day.  The part code carries the
                # date ("M95-C4055-2026-09-16"); the row carries the shift.
                m = _re.match(r"^M(\d+)-C(\d+)-(\d{4}-\d{2}-\d{2})?", pc)
                if m:
                    _mid, _seq = int(m.group(1)), int(m.group(2))
                    _dt = m.group(3) or _row.get("record_date")
                    _sh = _row.get("shift_name") or None
                    key = ("sub", _mid, _dt, _sh, _seq)
                    _sub_groups.setdefault((_mid, _dt, _sh), set()).add(_seq)
            _row_keys.append(key)

        # One indexed query per (table, date, shift) group instead of per row.
        # Each hit is (cycle time, when the cycle ran).  The second is the
        # video's own capture time — the page used to show only the time the
        # COMMENT was typed, which can be an hour after the cycle.
        _main_ct = {}
        for (_tbl, _dt, _sh), _seqs in _main_groups.items():
            if not _seqs or not _ct_ok(_tbl):
                continue
            try:
                if _sh:
                    cur.execute(
                        f"SELECT cycle_seq, ct_value, ts FROM {_tbl} "
                        f"WHERE record_date = %s AND shift_name = %s "
                        f"  AND cycle_seq = ANY(%s) AND ct_value IS NOT NULL",
                        (_dt, _sh, list(_seqs)))
                else:
                    cur.execute(
                        f"SELECT cycle_seq, ct_value, ts FROM {_tbl} "
                        f"WHERE record_date = %s "
                        f"  AND cycle_seq = ANY(%s) AND ct_value IS NOT NULL",
                        (_dt, list(_seqs)))
                for d in cur.fetchall():
                    _main_ct.setdefault((_tbl, _dt, _sh, int(d["cycle_seq"])),
                                        (d["ct_value"], d["ts"]))
            except Exception:
                conn.rollback()
        _sub_ct = {}
        if _sub_groups and _ct_ok("mes_submachine_ct_log"):
            # Rides idx (sub_plc_id, record_date, shift_name, cycle_seq), and the
            # date lets the planner prune to one monthly partition — the old
            # date-less query touched every partition.
            for (_mid, _dt, _sh), _seqs in _sub_groups.items():
                try:
                    _q = ("SELECT cycle_seq, ct_seconds, ts_start FROM mes_submachine_ct_log "
                          "WHERE sub_plc_id = %s AND cycle_seq = ANY(%s) "
                          "  AND ct_seconds IS NOT NULL")
                    _p = [_mid, list(_seqs)]
                    if _dt:
                        _q += " AND record_date = %s"
                        _p.append(_dt)
                    if _sh:
                        _q += " AND shift_name = %s"
                        _p.append(_sh)
                    cur.execute(_q, _p)
                    for d in cur.fetchall():
                        _sub_ct.setdefault((_mid, _dt, _sh, int(d["cycle_seq"])),
                                           (d["ct_seconds"], d["ts_start"]))
                except Exception:
                    conn.rollback()

        # Attach the resolved CT and capture time back onto each row.
        #   video_ts — when the cycle ran, i.e. when its video was captured
        #   ts_local — when the comment was typed
        # Both are plain plant-clock "YYYY-MM-DD HH:MM:SS" strings, so the page
        # and the exports split them into Date / Time without any timezone
        # arithmetic on whatever clock the viewing laptop happens to have.
        def _clock(v):
            try:
                return v.strftime("%Y-%m-%d %H:%M:%S") if v else None
            except Exception:
                return None
        for _row, key in zip(rows, _row_keys):
            ctv, vts = None, None
            try:
                hit = None
                if key and key[0] == "main":
                    hit = _main_ct.get((key[1], key[2], key[3], key[4]))
                elif key and key[0] == "sub":
                    hit = _sub_ct.get((key[1], key[2], key[3], key[4]))
                if hit is not None:
                    if hit[0] is not None:
                        ctv = round(float(hit[0]), 2)
                    vts = _clock(hit[1])
            except Exception:
                ctv, vts = None, None
            _row["cycle_time"] = ctv
            _row["video_ts"] = vts
            _t = _row.get("ts")
            _row["ts_local"] = _t[:19].replace("T", " ") if _t else None

    # Newest first, by WHEN THE CYCLE RAN — the Date / Time columns show the
    # capture time now, and ordering by comment time made them read out of
    # sequence (a comment typed at 12:01 about an 11:13 cycle sat above one
    # typed at 12:00 about 11:58).  A row whose cycle was not found falls back
    # to its comment time.  The comment time is the tie-break, and it matters:
    # WallboardLeft takes the FIRST row per part_code as that cycle's latest
    # note, so within one cycle the newest comment must still come first.
    rows.sort(key=lambda x: (x.get("video_ts") or x.get("ts_local") or "",
                             x.get("ts_local") or ""), reverse=True)
...
@router.get("/{line_id}/comments-summary")
def get_comments_summary(
    line_id:    int,   # 0 = all lines (mirrors /{line_id}/comments-history)
    date_from:  Optional[str] = Query(None, description="YYYY-MM-DD"),
    date_to:    Optional[str] = Query(None, description="YYYY-MM-DD"),
    shift_name: Optional[str] = Query(None),
    user=Depends(get_current_user_optional),
    ...
    # 2026-09-17 — validate the dates BEFORE they reach Postgres.  They were
    # passed through as raw strings, so `?date_from=notadate` came back as a
    # 500 (psycopg2 InvalidDatetimeFormat) — a client mistake reported as a
    # server crash, which sends whoever is on call hunting for a broken query.
    # Found by pmaudit/chaos.py feeding malformed input at every parameter.
    for _label, _val in (("date_from", date_from), ("date_to", date_to)):
        if _val:
            try:
                datetime.strptime(_val, "%Y-%m-%d")
                ...
                raise HTTPException(400, f"{_label} must be YYYY-MM-DD")
2026-09-16 — Headline numbers for the window the Comments History page is
    showing: how many parts ran, how many of those were OVER TARGET (cycle time
    above the line's ideal_cycle_time), and how many carried a comment — each
    with its share.  Operator: "total duration me kitne total part the, kitne
    overtarget the, kitne comment dale, with percent".

    Parts + over-target come from each line's `*_ct_log` (counted with the
    (record_date, shift_name) index, so it stays fast over long ranges);
    comments are counted from mes_cycle_comments over the same window.
    line_id = 0 aggregates every line."""
    from datetime import date as _date, timedelta as _td
    if not date_from:
        date_from = str(_date.today() - _td(days=7))
    if not date_to:
    ...
    per_line    = []
    with get_conn() as conn:
        conn.autocommit = True          # read-only; a missing ct_log must not
        try:                            # poison the loop over the other lines
            cur = dict_cursor(conn)
            if line_id and line_id > 0:
                ...
                cur.execute("SELECT id, line_name, db_table_name, ideal_cycle_time "
                            "FROM mes_lines WHERE db_table_name IS NOT NULL "
                            "AND db_table_name <> '' ORDER BY id")
            _lines = cur.fetchall() or []
            for _l in _lines:
                base = (_l.get("db_table_name") or "").strip()
                # identifier guard — the table name is interpolated into SQL
                if not base or not re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", base):
                    continue
                tbl = base + "_ct_log"
                try:
                    cur.execute("SELECT to_regclass(%s) AS e", (tbl,))
                    if not (cur.fetchone() or {}).get("e"):
                        continue
                    ideal = float(_l.get("ideal_cycle_time") or 0)
                    _w = "record_date BETWEEN %s AND %s"
                    _p = [date_from, date_to]
                    if shift_name:
                        _w += " AND shift_name = %s"
                        _p.append(shift_name)
                    cur.execute(
                        f"SELECT count(*) AS total, "
                        f"       count(*) FILTER (WHERE ct_value > %s) AS over "
                        f"FROM {tbl} WHERE {_w} AND ct_value IS NOT NULL",
                        [ideal] + _p)
                    r = cur.fetchone() or {}
                    t = int(r.get("total") or 0)
                    o = int(r.get("over") or 0)
                    total_parts += t
                    over_target += o
                    if t:
                        per_line.append({
                            "line_id":   _l["id"],
                            "line_name": _l["line_name"],
                            "ideal_cycle_time": ideal or None,
                            "parts": t, "over_target": o,
                            "over_target_pct": round(o * 100.0 / t, 2),
                        })
                except Exception:
                    continue

            # Comments over the same window (same source the table lists).
            _cw = "created_at::date BETWEEN %s AND %s"
            _cp = [date_from, date_to]
            if line_id and line_id > 0:
                _cw += " AND line_id = %s"
                _cp.append(line_id)
            if shift_name:
                _cw += " AND shift_name = %s"
                _cp.append(shift_name)
            comments = 0
            try:
                cur.execute(f"SELECT count(*) AS n FROM mes_cycle_comments WHERE {_cw}", _cp)
                comments = int((cur.fetchone() or {}).get("n") or 0)
            except Exception:
                comments = 0
        finally:
    ...
    return {
        ...
        "over_target_pct":  round(over_target * 100.0 / total_parts, 2) if total_parts else 0.0,
        "comments":         comments,
        "commented_pct":    round(comments * 100.0 / total_parts, 2) if total_parts else 0.0,
        "per_line":         per_line,
    }


@router.get("/{line_id}/comments-pareto")
def get_comments_pareto(
    line_id:    int,   # path: 0 = all lines (mirrors /{line_id}/comments-history)
    date_from:  Optional[str] = Query(None, description="YYYY-MM-DD"),
    date_to:    Optional[str] = Query(None, description="YYYY-MM-DD"),
    shift_name: Optional[str] = Query(None),
    machine_id: Optional[int] = Query(None, description="restrict to one machine"),
    zone_id:    Optional[int] = Query(None, description="restrict to one zone"),
    user=Depends(get_current_user_optional),
):
    """COMMENT-BASED Pareto: ranks the actual comment / NG-remark TEXT by how
    often it repeats, so the MAJOR vs MINOR problems stand out. Scope it with the
    page filters — a specific machine, line or zone (line_id=0 = all lines),
    plus date + shift. Descending with cumulative %. Read-only.

    Bars = the problem text; count = how many times it was recorded."""
    from datetime import date as _date, timedelta as _td
    if not date_from:
        date_from = str(_date.today() - _td(days=7))
    if not date_to:
    ...
    counts: dict = {}    # lower(text) -> count   (case-insensitive merge)
    labels: dict = {}    # lower(text) -> display spelling
    with get_conn() as conn:
        cur = dict_cursor(conn)
        # Zone filter → its line ids.
        zone_lines = None
        if zone_id:
            cur.execute("SELECT id FROM mes_lines WHERE zone_id = %s", (zone_id,))
            zone_lines = [r["id"] for r in cur.fetchall()] or [-1]
        ...
        def _add(rows_):
            for r in rows_:
                ...
                if not disp:
                    continue
                k = disp.lower()   # "Bin change" and "Bin Change" are one problem
                counts[k] = counts.get(k, 0) + int(r["n"])
                labels.setdefault(k, disp)

        # per-cycle comments — grouped by the comment text
        w = "created_at::date BETWEEN %s AND %s AND btrim(comment) <> ''"
        p = [date_from, date_to]
        if line_id:
            w += " AND line_id = %s";   p.append(line_id)
        if shift_name:
            w += " AND shift_name = %s"; p.append(shift_name)
        if machine_id:
            w += " AND machine_id = %s"; p.append(machine_id)
        if zone_lines is not None:
            w += " AND line_id = ANY(%s)"; p.append(zone_lines)
        try:
            cur.execute(f"SELECT mode() WITHIN GROUP (ORDER BY btrim(comment)) AS t, "
                        f"       COUNT(*) AS n "
                        f"FROM mes_cycle_comments WHERE {w} "
                        f"GROUP BY LOWER(btrim(comment))", p)
            _add(cur.fetchall())
        except Exception:
            conn.rollback()

        # per-machine NG remarks — grouped by the remark text
        w2 = "record_date BETWEEN %s AND %s AND btrim(remark_text) <> ''"
        p2 = [date_from, date_to]
        if line_id:
            w2 += " AND line_id = %s";   p2.append(line_id)
        if shift_name:
            w2 += " AND shift_name = %s"; p2.append(shift_name)
        if machine_id:
            w2 += " AND machine_id = %s"; p2.append(machine_id)
        if zone_lines is not None:
            w2 += " AND line_id = ANY(%s)"; p2.append(zone_lines)
        try:
            cur.execute(f"SELECT mode() WITHIN GROUP (ORDER BY btrim(remark_text)) AS t, "
                        f"       COUNT(*) AS n "
                        f"FROM mes_ng_process_remarks WHERE {w2} "
                        f"GROUP BY LOWER(btrim(remark_text))", p2)
            _add(cur.fetchall())
        except Exception:
            conn.rollback()

    items = sorted(({"name": labels[k], "count": v} for k, v in counts.items()),
                   key=lambda x: -x["count"])
    total = sum(x["count"] for x in items)
    run = 0
    for it in items:
        run += it["count"]
        it["pct"]     = round(it["count"] * 100.0 / total, 1) if total else 0.0
        it["cum_pct"] = round(run * 100.0 / total, 1)          if total else 0.0
    return {"date_from": date_from, "date_to": date_to,
            "total": total, "distinct": len(items), "items": items}
...
def list_ng_parts_for_slot(
    ...
    with get_conn() as conn:
        ...
        # 2026-09-14 — Guard lines whose per-line ct_log was never created
        # (sub-assembly lines 33-41). Querying {tbl}_ct_log raised
        # UndefinedTable → HTTP 500 every time the operator clicked an NG cell.
        # Return an empty list instead so the popup opens cleanly with no rows.
        cur.execute("SELECT to_regclass(%s) AS reg", (f"{tbl}_ct_log",))
        if not cur.fetchone()["reg"]:
            return {"line_id": line_id, "date": date, "slot_label": slot_label,
                    "count": 0, "rows": []}
...
def _mail_clip_bytes(line_id, r):
    ...
    if cyc is not None:
        try:
            ...
            url = (f"http://127.0.0.1:8080/api/lines/{line_id}"
...
def send_ng_mail_to_quality(
    ...
    with get_conn() as conn:
        ...
        line_name = lr.get("line_name") or "Unknown line"
        tbl       = lr["db_table_name"]
        ideal     = float(lr["ideal_cycle_time"]) if lr.get("ideal_cycle_time") else None
