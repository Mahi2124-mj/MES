...
def add_plc(payload: Dict, base_dir: Optional[str] = None) -> Tuple[bool, str, Optional[str]]:
    ...
    ok_bit = str(payload.get("ok_bit", "")).strip()
    ng_bit = str(payload.get("ng_bit", "")).strip()
    # Build bit_address from ok_bit + ng_bit for backwards compat with PlcMonitor
    bits = [b for b in [ok_bit, ng_bit] if b]
    bit_addr = payload.get("bit_address", "") or ",".join(bits)
    new_plc = {
        "id": plc_id,
        "ip": str(payload.get("ip", "")).strip(),
        "port": int(payload.get("port", 502)),
        "bit_address": str(bit_addr).strip(),
        "ok_bit": ok_bit,
        "ng_bit": ng_bit,
...
def update_plc(plc_id: str, updates: Dict, base_dir: Optional[str] = None) -> Tuple[bool, str]:
    ...
    for p in plcs:
        if str(p.get("id")) == str(plc_id):
            ...
            if "ok_bit" in updates: p["ok_bit"] = str(updates["ok_bit"]).strip()
            if "ng_bit" in updates: p["ng_bit"] = str(updates["ng_bit"]).strip()
            # Rebuild bit_address from ok_bit + ng_bit
            if "ok_bit" in updates or "ng_bit" in updates:
                bits = [b for b in [p.get("ok_bit",""), p.get("ng_bit","")] if b]
                p["bit_address"] = ",".join(bits) if bits else p.get("bit_address","")
            elif "bit_address" in updates:
                p["bit_address"] = str(updates["bit_address"]).strip()
