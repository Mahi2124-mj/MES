...
from recorder import (
    DEFAULT_METADATA_CSV, ensure_metadata_file, open_rtsp_capture,
    append_cycle_metadata, get_next_cycle_number, get_resolved_videos_dir,
)
...
# ─── PLC edge webhook → per-part clip (driven by the MES collector) ─
#
# The MES collector (Phase2/collector_engine.py) fires _emit_edge_webhook
# on EVERY committed part — L108/OK and L109/NG — in BOTH bit-count and
# register-count (D1001) modes.  It POSTs the exact wall-clock instant
# here so we can cut THAT ONE part's video.
#
# Why the collector pushes (instead of us polling the PLC): the
# Mitsubishi accepts only a couple of TCP clients on :5002; a second
# poller steals ~half the rising edges from the collector.  So the
# collector is the single source of truth and this route is a passive
# receiver.  (This is also why plc_poller.py stays disabled.)
#
# Each edge is the END of a part, so consecutive edges delimit it:
#     part-N video = [previous edge time, this edge time]
# We extract that exact window from the camera's continuously-rolling
# .ts (RecordingManager) with ffmpeg -c copy — instant, no re-encode,
# no cv2, no 1-second cycle_state poll — and register it as a normal
# per-cycle mp4 row in cycles.csv, so the existing dashboard shows it
# exactly like every other machine's clip.  A missed/duplicate edge no
# longer stretches one clip across several parts (the "dual/triple
# combined video" the operator reported).

# machine_id -> {"epoch_ms": int, "end_epoch": float}
_edge_last: Dict[str, Dict[str, Any]] = {}
_edge_lock = threading.Lock()        # guards _edge_last (dedup + window)
_edge_csv_lock = threading.Lock()    # serialises cycle-number + csv append


def _resolve_edge_binding(line_id: Any) -> "Dict[str, Any] | None":
    """Map a collector edge (which carries line_id, not machine_id) to a
    camera binding.  Preference: a binding explicitly tagged with this
    line_id; else the only binding when exactly one exists; else None.
    To disambiguate when multiple cameras exist, add "line_id": <n> to
    the relevant binding in camera_config_bindings.json."""
    ...
    if line_id is not None and str(line_id).strip():
        ...
        for b in bindings:
            if str(b.get("line_id", "")).strip() == lid:
                return b
    if len(bindings) == 1:
        return bindings[0]
    return None
...
def _machine_meta(machine_id: str) -> Dict[str, str]:
    for m in all_machines_flat(BASE_DIR):
    ...
    return {"machine_name": "", "line_name": "", "zone_name": ""}


def _edge_file_path(videos_root: str, machine_id: str, fname: str):
    """(absolute_path, csv_file_path) honouring the same in-tree-vs-
    external-HDD convention recorder.py uses, so /api/video resolves it."""
...
def _extract_edge_clip(machine_id: str, camera_id: str, status: str,
                       start_epoch: float, end_epoch: float,
                       epoch_ms: int, tag: str) -> None:
    """Background worker: cut [start,end] from the camera's rolling .ts
    and append a cycles.csv row.  Best-effort — logs and returns on any
    problem so a bad clip never blocks the next part."""
    try:
        # Let the rolling .ts flush the tail of this part before cutting.
        time.sleep(1.5)
        rec = RecordingManager.get(BASE_DIR).get_recorder(camera_id)
        info = rec.current_recording_info() if rec else None
        if not info:
            print(f"[PLC-EDGE] no active .ts for camera {camera_id} — "
                  f"is the recorder running? skip", flush=True)
            return
        ts_file, ts_start_ep = info
        if not ts_file or not os.path.exists(ts_file):
            print(f"[PLC-EDGE] .ts missing ({ts_file}) — skip", flush=True)
            return
        ss = max(0.0, start_epoch - ts_start_ep)
        to = max(0.0, end_epoch - ts_start_ep)
        if to - ss < 0.20:
            print(f"[PLC-EDGE] window too small ss={ss:.2f} to={to:.2f} — skip",
                  flush=True)
            return
        videos_root = get_resolved_videos_dir()
        tmp_abs, _tmp_rel = _edge_file_path(
            videos_root, machine_id, f"_edge_{epoch_ms}.mp4")
        cmd = [
            FFMPEG_EXE if os.path.exists(FFMPEG_EXE) else "ffmpeg",
            "-y",
            "-ss", f"{max(0.0, ss - 0.5):.3f}",   # 0.5s pre-roll for keyframe
            "-to", f"{to + 0.5:.3f}",
            "-i", ts_file,
            "-c", "copy",
            "-movflags", "+faststart",
        ...
        try:
            ...
            print(f"[PLC-EDGE] ffmpeg failed: "
                  f"{exc.stderr.decode(errors='ignore')[:200]}", flush=True)
            return
        except subprocess.TimeoutExpired:
            print("[PLC-EDGE] ffmpeg timed out", flush=True)
            return
        # Reserve the cycle number + publish the row atomically so two
        # workers (or a fast OK/NG pair) never collide on cycle_N.mp4.
        meta = _machine_meta(machine_id)
        with _edge_csv_lock:
            ensure_metadata_file(CSV_PATH)
            cyc = get_next_cycle_number(CSV_PATH)
            final_abs, final_rel = _edge_file_path(
                videos_root, machine_id, f"cycle_{cyc}.mp4")
            os.replace(tmp_abs, final_abs)
            append_cycle_metadata(
                CSV_PATH, cyc,
                datetime.fromtimestamp(start_epoch),
                datetime.fromtimestamp(end_epoch),
                final_rel,
                machine_id   = machine_id,
        ...
        print(f"[PLC-EDGE] {status} cycle {cyc} {machine_id} "
              f"{to - ss:.1f}s -> {final_rel}", flush=True)
    except Exception as exc:
        print(f"[PLC-EDGE] worker error: {exc}", flush=True)


@app.post("/api/plc-edge")
def plc_edge():
    """Receive a per-part edge ping from the MES collector and cut that
    part's clip.  Unauthenticated by design — an internal localhost
    webhook (same trust level as /api/health); the collector cannot
    carry a user JWT.  Returns 200 immediately so the collector's retry
    loop never fires a duplicate for a slow handler."""
    payload = request.get_json(silent=True) or {}
    bit    = str(payload.get("bit", "")).strip().upper()
    status = str(payload.get("status", "")).strip().upper()
    if status not in ("OK", "NG"):
        status = "OK" if bit == "L108" else "NG" if bit == "L109" else ""
    if status not in ("OK", "NG"):
        return _json_ok(message="ignored (no OK/NG status)")
    ...
    if not epoch:
        try:
            epoch = float(payload.get("epoch_ms") or 0) / 1000.0
        except (TypeError, ValueError):
            epoch = 0.0
    if not epoch:
        epoch = time.time()
    try:
        epoch_ms = int(payload.get("epoch_ms") or round(epoch * 1000))
    except (TypeError, ValueError):
        epoch_ms = int(round(epoch * 1000))

    binding = _resolve_edge_binding(payload.get("line_id"))
    if not binding:
        return _json_ok(message="no camera binding for this line — "
                                "set one in Camera Config")
    machine_id = str(binding.get("machine_id", "")).strip()
    camera_id  = str(binding.get("camera_id", "")).strip()
    if not machine_id or not camera_id:
        return _json_ok(message="binding missing machine_id/camera_id")
    try:
        target = float(binding.get("target_time") or 30)
    except (TypeError, ValueError):
        target = 30.0

    with _edge_lock:
        prev = _edge_last.get(machine_id)
        if prev and epoch_ms <= prev["epoch_ms"]:
            # Same edge re-delivered by the collector's retry loop, or an
            # out-of-order replay — ignore so we don't double-cut.
            return _json_ok(message="duplicate edge ignored")
        if prev and prev["end_epoch"] < epoch:
            start_epoch = prev["end_epoch"]       # true part-to-part boundary
        else:
            start_epoch = epoch - target          # first part: bounded window
        _edge_last[machine_id] = {"epoch_ms": epoch_ms, "end_epoch": epoch}

    pc = str(payload.get("part_code") or "").strip()
    mn = str(payload.get("model_name") or "").strip()
    tag = status + (f" pc={pc}" if pc else "") + (f" {mn}" if mn else "")

    threading.Thread(
        target=_extract_edge_clip,
        args=(machine_id, camera_id, status, start_epoch, epoch, epoch_ms, tag),
        daemon=True, name=f"plc-edge-{status}",
    ).start()
    return _json_ok({"status": status, "machine_id": machine_id}, "edge accepted")
