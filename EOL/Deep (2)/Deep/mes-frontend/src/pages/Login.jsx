...
export default function Login() {
  ...
  useEffect(() => {
    if (token) navigate("/", { replace: true });   // "/" resolves to the first page this user may open
  }, [token]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/", { replace: true });   // "/" resolves to the first page this user may open
    } catch (err) {
    ...
    <>
      <style>{`
        ...
        @media (max-width: 640px) {
          /* Stack the branding ABOVE the form (instead of hiding it) so all
             details show on a phone. */
          /* The desktop root is a fixed 100vh box that CENTERS the card with
             overflow:hidden. On a phone the stacked card (branding + form) is
             taller than 100vh, so centering + hidden clipped the TOP (company
             name) and bottom. Let the root grow and scroll instead. */
          .login-root { height: auto; min-height: 100vh; align-items: stretch;
                        overflow-y: auto; overflow-x: hidden; }
          .login-card { flex-direction: column; width: 100vw; max-width: 100vw;
                        border-radius: 0; min-height: 100vh; }
          .login-left  { width: 100%; flex-shrink: 0;
                         padding: calc(env(safe-area-inset-top, 0px) + 26px) 26px 16px; }
          .login-right { padding: 22px 26px 36px; }
          .login-headline, .login-sub-headline { font-size: 30px; margin-bottom: 2px; }
          .login-sub-headline { margin-bottom: 14px; }
          .login-desc { font-size: 11.5px; }
        }
      `}</style>
