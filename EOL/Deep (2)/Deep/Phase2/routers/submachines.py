...
# 2026-08-11 — 30 s was long enough for the operator to stare at a spinner
# and still end up with a 404: a stoppage-sized window (measured 31 min) makes
# CMS grind for the full timeout and then fail.  Normal clips answer in ~0.3 s,
# and even a 120 s capped extract renders in a few seconds, so 15 s only ever
# cuts short the requests that were going to fail anyway — and the player shows
# its clean "no video for this cycle" instead of hanging.
_CLIP_PROXY_TIMEOUT = float(os.environ.get("CLIP_PROXY_TIMEOUT", "15") or 15)

# 2026-08-11 — CLAMP STOPPAGE-SIZED WINDOWS.
#
# A cycle is the wall-clock gap between two parts, so a stoppage becomes one
# huge "cycle": measured 431 rows in a day over 5 min, worst 46 min (the first
# part after a break spans the whole break).  Asking for a 31-minute clip made
# CMS grind the full proxy timeout and then 404 — the operator waited and got
# nothing.
#
# The cap is 120 s, far above every real cycle here (longest normal CT ~25 s),
# so an ordinary cycle is NEVER touched and the "video length must match cycle
# length" spec still holds.  Only stoppage-sized windows clamp.
#
# We keep the END of the window, not the start: the window is
# [previous part -> this part], so the part actually being made is at ts_end.
# Trimming from the front shows the machine standing idle; trimming to the last
# 120 s shows the production that closed the cycle, which is what the operator
# opened the clip for.
#
# Done here rather than inside CMS on purpose — the CMS process owns the live
# camera recorders, and restarting it wipes the rolling .ts buffer (measured
# 199 GB -> 4.6 GB, i.e. hours of reviewable footage gone).  Clamping in the
# proxy needs only an API restart, and because the pre-warmer calls this same
# route the CMS cache key stays identical either way.
...
# 2026-05-29 - Same standalone-mode flag as lines.py.
...
def cycle_video(
    ...
    ng:        Optional[int] = Query(None,
                  description="1 = the Alarm row for this seq, 0 = the OK row. "
                              "Alarm and OK rows can share a cycle_seq, so the "
                              "caller says which dot was clicked."),
    ...
    if not CMS_INTEGRATION_ENABLED:
        raise HTTPException(404, "Video integration disabled (CMS standalone mode)")
    ...
    with get_conn() as conn:
        sub = _check_sub_belongs_to_line(sub_id, conn)
        cur = dict_cursor(conn)
        # 2026-08-11 — an Alarm row and an OK row can carry the SAME cycle_seq
        # (the collector writes the Alarm with its own 60 s review window), and
        # this lookup only kept the later ts_end — so clicking one dot could
        # serve the other one's clip.  `ng` lets the caller name the row it
        # clicked; omitted, the old behaviour is preserved exactly.
        cur.execute("""
            SELECT ts_start, ts_end, COALESCE(is_ng, FALSE) AS is_ng, record_date
            FROM mes_submachine_ct_log
            WHERE sub_plc_id = %s AND cycle_seq = %s
              AND (%s::int IS NULL OR COALESCE(is_ng, FALSE) = (%s::int = 1))
            ORDER BY ts_end DESC LIMIT 1
        """, (sub_id, cycle_seq, ng, ng))
        row = cur.fetchone()
        if not row:
            raise HTTPException(404, "Cycle not found")
        ts_start = row["ts_start"]
        ts_end   = row["ts_end"]
        # 2026-08-11 — operator spec: an Alarm clip must show the Alarm AND the
        # cycle that follows it, as ONE continuous video ("ng + next ki clip ek
        # saath aaye").  Reviewing a reject is about what the machine did next,
        # and the Alarm's own window ends the instant the bit fired.  The next
        # row is found by TIME, not by cycle_seq — Alarm rows reuse the seq of
        # the OK cycle, so seq+1 is not reliably "the next one".
        _extended_to = None
        if row["is_ng"]:
            cur.execute("""
                SELECT ts_end FROM mes_submachine_ct_log
                WHERE sub_plc_id = %s AND record_date = %s AND ts_end > %s
                ORDER BY ts_end ASC LIMIT 1
            """, (sub_id, row["record_date"], ts_end))
            _nxt = cur.fetchone()
            if _nxt and _nxt["ts_end"] > ts_end:
                _extended_to = _nxt["ts_end"]
                ts_end = _nxt["ts_end"]
        # 2026-08-11 — ARCHIVE HIT: over-target and Alarm clips are rendered in
        # the background and kept on the video volume, so the common click is a
        # plain disk read instead of a 2.7 s transcode.  Checked before any CMS
        # work; a miss falls through to exactly the old path.
        try:
            from routers.clip_archive import find as _arch_find, serve as _arch_serve
            _hit = _arch_find("sub", sub_id, row["record_date"], cycle_seq,
                              bool(row["is_ng"]))
        except Exception:
            _hit = None
        if _hit:
            return _arch_serve(_hit, request)

        _win_s   = (ts_end - ts_start).total_seconds()
        _clipped_from = None
        if _CLIP_MAX_SECONDS > 0 and _win_s > _CLIP_MAX_SECONDS:
    ...
    try:
        r = requests.get(upstream, params=params,
                         headers=fwd_headers,
                         stream=True, timeout=_CLIP_PROXY_TIMEOUT)
    except Exception as exc:
        raise HTTPException(502, f"Upstream unreachable: {exc}")

    # 2026-08-10 — close the streamed upstream before raising; a stream=True
    # response abandoned mid-flight never returns its socket to the pool.
    if r.status_code >= 400:
        _detail, _code = f"Upstream: {r.text[:200]}", r.status_code
        try: r.close()
    ...
    if _extended_to:
        resp_headers["X-Clip-Includes-Next"] = "1"
    if _clipped_from:
        # Let the player say "31 min window, last 120 s" instead of silently
        # showing a shorter clip than the labelled cycle time.
        resp_headers["X-Clip-Truncated"] = f"{_clipped_from:.0f}"
        resp_headers["X-Clip-Duration"]  = f"{_CLIP_MAX_SECONDS:.0f}"
    resp_headers["Pragma"]        = "no-cache"

    # 2026-08-10 — this path DOES need stream=True (it relays the body), so
    # instead of dropping it we guarantee the upstream is closed when the
    # generator finishes OR the browser aborts mid-video (seek / modal close),
    # which is what left sockets stranded before.
    def _relay():
        try:
