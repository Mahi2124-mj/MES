...
from auth import get_current_user, require_admin, FLOOR_SCOPED_ROLES
...
# ══════════════════════════════════════════════════════════════
# SCHEMA — per-zone colour (2026-08-14)
# ══════════════════════════════════════════════════════════════
# Zones had no colour of their own, so every screen that groups by zone drew
# the same grey heading and the only way to tell SEAT SLIDER from Recliner at
# a glance was to read the text.  One nullable column; NULL means "no colour
# chosen" and callers fall back to their existing styling, so nothing changes
# for a zone until someone picks a colour for it.
_SCHEMA_READY = False


@once
def _ensure_schema() -> None:
    """Add mes_zones.color if this deploy predates it.  Once per process."""
    ...
    try:
        with get_conn() as conn:
            conn.cursor().execute(
                "ALTER TABLE mes_zones ADD COLUMN IF NOT EXISTS color VARCHAR(9)")
            conn.commit()
    except Exception:
        # Never take the router down over a cosmetic column.
        pass
    _SCHEMA_READY = True


# Hex only.  The value is interpolated straight into a CSS colour on the
# frontend, so refusing anything that is not #RGB/#RRGGBB/#RRGGBBAA keeps
# arbitrary text out of a style attribute.
...
def _clean_color(value: Optional[str]) -> Optional[str]:
    ...
    v = value.strip()
    if v == "":
        return None                      # explicit clear → back to no colour
    if not _HEX_RE.match(v):
        raise HTTPException(400, f"color must be a hex value like #1e40af, got {value!r}")
    return v.lower()
...
def list_zones(plant_id: Optional[int] = None, user=Depends(get_current_user)):
    """Return active zones, optionally filtered by plant. Includes line count.

    Scoped by ASSIGNMENT (mirrors routers/lines.list_lines): a non-admin user
    with rows in mes_operator_lines sees ONLY the zones that contain at least
    one line assigned to them, and line_count reflects just those lines. Admins
    and users with NO assignment stay unrestricted, so this can never narrow an
    existing login by accident."""
    _ensure_schema()
    with get_conn() as conn:
        ...
        if user["role"] != "admin":
            ...
            if not _scoped and user["role"] in FLOOR_SCOPED_ROLES:
                return []          # floor role, no assignment → sees no zones
        if _scoped:
            params = [user["id"]]
            plant_clause = ""
            if plant_id:
                plant_clause = "AND z.plant_id = %s"
            ...
            cur.execute(f"""
                ...
                JOIN mes_lines l ON l.zone_id = z.id AND l.is_active = true
                JOIN mes_operator_lines ol ON ol.line_id = l.id AND ol.admin_id = %s
                WHERE z.is_active = true {plant_clause}
...
def create_zone(body: ZoneCreate, admin=Depends(require_admin)):
    ...
    with get_conn() as conn:
        ...
        cur.execute("""
            INSERT INTO mes_zones (plant_id, zone_code, zone_name, description, color)
...
def update_zone(zone_id: int, body: ZoneUpdate, admin=Depends(require_admin)):
    ...
    # Colour is the one field you must be able to UNSET.  The filter above drops
    # None (= "field not sent"), so an empty string is what "clear the colour"
    # looks like on the wire; _clean_color turns that into SQL NULL and rejects
    # anything that is not a hex value.
    if "color" in updates:
        updates["color"] = _clean_color(updates["color"])
    if not updates:
...
def get_zone_lines(zone_id: int, user=Depends(get_current_user)):
    """Return lines in this zone, scoped by ASSIGNMENT (mirrors list_lines):
    a non-admin user with mes_operator_lines rows sees only THEIR lines in the
    zone; admins / unassigned users see all lines in it."""
    with get_conn() as conn:
        cur = dict_cursor(conn)
        _scoped = False
        if user["role"] != "admin":
            ...
            _scoped = cur.fetchone() is not None
            if not _scoped and user["role"] in FLOOR_SCOPED_ROLES:
                return []          # floor role, no assignment → sees no lines
        if _scoped:
            cur.execute("""
                ...
                WHERE l.zone_id = %s AND ol.admin_id = %s
                ORDER BY l.line_code
            """, (zone_id, user["id"]))
        else:
            cur.execute("""
...
class OTUpdate(BaseModel):
    ot_enabled:  bool
    ot_end_time: Optional[str] = None
    ot_plan:     Optional[int] = None   # optional zone-wide manual OT target (pcs)
...
def toggle_zone_shift_ot(zone_id: int, shift_name: str, body: OTUpdate, admin=Depends(require_admin)):
    """Enable / disable overtime for ONE shift across ALL active lines in a zone.

    2026-06-19 — Rewritten so zone OT actually WORKS.  Previously this only wrote
    mes_shift_configs.ot_enabled (a flag the collector never reads) and never set
    mes_lines.ot_active_shift (the real OT switch the collector keys on), so a
    zone OT toggle did nothing in production.  Now, for every active *production*
    line+shift in the zone:
      • ON  → ot_enabled=true, ot_end_time=<body>, ot_plan=<body or keep>, and
              mes_lines.ot_active_shift = <shift>  (the collector's actual switch).
              OT window start defaults to the shift's end_time via the collector
              (ot_start_time left as-is → fallback = shift_end).
      • OFF → ot_enabled=false, and mes_lines.ot_active_shift cleared — but ONLY
              where it currently points at THIS shift, so we never clobber OT
              that a different shift has active.
    """
    with get_conn() as conn:
        lines = _all_lines(zone_id, conn)
        if not lines:
        ...
        for line in lines:
            lid = line["id"]
            # OT only applies to a real production shift on this line.
            cur.execute(
                "SELECT is_production FROM mes_shift_configs "
                "WHERE line_id = %s AND shift_name = %s",
                (lid, shift_name),
            )
            sc = cur.fetchone()
            if not sc or not sc.get("is_production"):
                continue
            if body.ot_enabled:
                cur.execute("""
                    UPDATE mes_shift_configs
                     ...
                     WHERE line_id = %s AND shift_name = %s
                """, (body.ot_end_time or None, body.ot_plan, lid, shift_name))
                cur.execute(
                ...
                cur.execute(
                    "UPDATE mes_shift_configs SET ot_enabled = false "
                    "WHERE line_id = %s AND shift_name = %s",
                    (lid, shift_name),
                )
                cur.execute(
                    "UPDATE mes_lines SET ot_active_shift = NULL, updated_at = NOW() "
                    "WHERE id = %s AND ot_active_shift = %s",
                    (lid, shift_name),
                )
            affected += 1
        conn.cursor().execute("""
        ...
        """, (zone_id, f"shift={shift_name} ot={body.ot_enabled} end={body.ot_end_time} lines={affected}"))
    return {"ok": True, "lines_affected": affected}
