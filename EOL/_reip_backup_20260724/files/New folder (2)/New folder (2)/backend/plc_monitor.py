...
def _camera_recording_allowed(camera_id: str, base_dir: str, bindings=None) -> bool:
    """True if this camera should be recording: master video ON and its line
    not in the per-line disabled set."""
...
class PlcMonitor:
        ...
        with self._cam_spawn_lock:
            ...
            cmd = [
                ...
                *_live_flags,
                ...
                # 2026-07-01 — width is CLAMPED to [1280, 1920]: small sub-streams
                # upscale to 1280, but anything wider than 1920 is DOWNSCALED to
                # 1920 so it fits H.264 Level 4.0.  A high-res MAIN stream (e.g. the
                # .116 Final Inspection cam = 2304x1296 HEVC) previously kept its
                # native 2304 width and NVENC rejected it ("Invalid Level") → 0-byte
                # .ts → the machine fell back to the default camera's feed.  The cap
                # lets every PLC-bound camera record its OWN stream, TV-safe.
                ...
                # 2026-07-22 — CPU-SATURATION FIX.  This box has NO GPU
                # (ASPEED BMC only) so _pick_live_encoder falls back to
                # software libx264, and 45+ simultaneous HEVC-2304 decodes +
                # lanczos scale + unsharp sharpen pegged all 64 cores (load
                # avg 65, ffmpeg ~41 cores).  Saturated streams fell behind,
                # the stall-watchdog killed ffmpeg, and recordings came out as
                # 2-3 s fragments with gaps ("video kabhi 2-3 sec, kabhi nahi").
                # Dropped the cosmetic unsharp sharpen and switched the scale
                # to fast_bilinear — measured ~52 % less CPU per stream, same
                # file size — while KEEPING the essential normalization the
                # sharpen was bundled with: downscale to <=1920 (H.264 Level
                # 4.0 for TV decoders) + format=yuv420p (tv-range, so HEVC
                # 2304 Panasonic cams stay browser/TV-playable).
                "-vf",
                "scale='min(max(iw,1280),1920)':'-2':flags=fast_bilinear,"
                "format=yuv420p",
                "-color_range", "tv",
                ...
                # silently ignore.
                # 2026-07-22 — 20 -> 15 fps as part of the CPU-saturation fix
                # above: 25 % fewer frames through the (GPU-less) HEVC decode +
                # libx264 encode, still smooth for shop-floor cycle review.
                # -g/-keyint_min tracked to 15 so a keyframe still lands every
                # 1 s and `-c copy` clip extraction keeps its +/-1 s accuracy.
                "-r", "15",
                "-g", "15",           # keyframe every 15 frames = 1 s
                "-keyint_min", "15",
    ...
    def _detect_write_start(self, cam: Dict, ts_file: str) -> None:
        ...
        STALL_TIMEOUT = 5.0   # seconds with no file growth → stalled
    ...
    def _extract_cycle(
        ...
        if (result_rc == 0 and (0 < size_actual < min_required or not dur_ok)) or result_rc != 0:
            ...
            retry_cmd = [
                ...
                *_concat_in,                 # concat demuxer when stitching TS
                "-i", _eff_src,              # single TS or concat listfile
                "-ss", f"{_eff_ss:.3f}",     # output seek = frame-accurate
                "-t", f"{duration:.3f}",
