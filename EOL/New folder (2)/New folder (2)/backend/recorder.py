...
def build_rtsp_candidates(rtsp_url: str) -> list:
    ...
    # Auth must come from the URL itself; no plaintext fallback in code.
...
PART_LOG_COLUMNS = [
    "cycle_number", "part_name", "part_code", "cycle_time_sec",
    "start_time", "end_time", "video_path",
]
PART_LOG_CSV = "part_log.csv"


def _ensure_part_log(base_dir: str) -> str:
    """Ensure part_log.csv exists with header, return its absolute path."""
    path = os.path.join(base_dir, PART_LOG_CSV)
    if not os.path.exists(path):
        with open(path, "w", newline="", encoding="utf-8") as f:
            csv.writer(f).writerow(PART_LOG_COLUMNS)
    return path
...
def append_cycle_metadata(
    ...
    # Also write to part_log.csv — clean CSV with just part/cycle/video info
    base_dir = os.path.dirname(os.path.abspath(csv_path))
    part_log_path = _ensure_part_log(base_dir)
    with open(part_log_path, "a", newline="", encoding="utf-8") as f:
        csv.writer(f).writerow([
            cycle_number,
            machine_name,         # part_name
            part_code,
            duration_seconds,     # cycle_time_sec
            start_dt.strftime("%Y-%m-%dT%H:%M:%S"),
            end_dt.strftime("%Y-%m-%dT%H:%M:%S"),
            relative_file_path,   # video_path
        ])
...
class CycleRecorder:
    def __init__(self, rtsp_url: str, metadata_csv: str, videos_dir: str):
        ...
        self.videos_dir = (
            videos_dir if os.path.isabs(videos_dir)
            else os.path.join(self.base_dir, videos_dir)
        )
    ...
    def start_cycle(self, frame_shape, fps: float):
        ...
        file_rel = os.path.join(DEFAULT_VIDEOS_DIR, file_name).replace("\\", "/")
