...
function HistoryTable({ rows, onCloseTicket, onViewTicket, slipThresholdMin = 10 }) {
  ...
  return (
    <div style={{
      ...
      {/* 2026-06-16 — cap to ~5 rows then scroll INSIDE the box, so a 2-day
          backlog (30+ slips) no longer stretches the whole page.  The column
          header stays pinned (sticky) while the rows scroll. */}
...
export function ClosureFormModal({ ticket, mode, phase = "maintenance", onClose, onSave, token }) {
  ...
  //      injected header band ("Historical Data" / "192.168.30.185:5656/…").
