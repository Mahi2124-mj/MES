...
def list_items(line_id: Optional[int] = None, user=Depends(get_current_user)):
    ...
    with get_conn() as conn:
        ...
        # 2026-09-16 — join mes_lines so each item carries its LINE NAME.  The
        # 5S Items table used to render "Line #7" because the payload had no
        # name in it at all; line_name is NULL for plant-wide items (line_id
        # IS NULL), which the UI shows as "ALL".
        if line_id is not None:
            cur.execute("""SELECT i.*, l.line_name FROM mes_5s_items i
                       LEFT JOIN mes_lines l ON l.id = i.line_id
                            WHERE i.is_active = TRUE
                              AND (i.line_id IS NULL OR i.line_id = %s)
                            ORDER BY i.display_order, i.id""", (line_id,))
        else:
            cur.execute("""SELECT i.*, l.line_name FROM mes_5s_items i
                       LEFT JOIN mes_lines l ON l.id = i.line_id
                            WHERE i.is_active = TRUE
                            ORDER BY i.pillar, i.display_order, i.id""")
        return cur.fetchall()
