    ...
    for cfg in cfgs:
        ...
        try:
            ...
            try:
                ...
                send_to_line(line_id, f"OEE drop — {line_name}",
                             f"Overall OEE {oee:.1f}% (target {threshold:.0f}%) for "
                             f"{int(elapsed_min)} min · Shift {shift}.",
                             url="/dashboard", tag="oee_drop")
            except Exception as _ex:
                print("[OEE-ALARM] inbox notify failed:", str(_ex)[:120])
            with get_conn() as conn:
                c3 = conn.cursor()
