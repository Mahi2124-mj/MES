...
# A "default camera" placeholder used only on a fresh install when no
# cameras.json exists.  Credentials come from env so plaintext doesn't sit
# in source code.  If the env vars aren't set we leave them empty — the
# user must add a real camera through the UI before any streaming works.
DEFAULT_CAMERA = {
...
def _build_rtsp_url(camera: Dict[str, object]) -> str:
    ...
    password = str(camera.get("password", "admin123"))
...
def update_camera(
    ...
    """Update mutable fields on an existing camera.  Allowed keys:
    name, ip, port, username, password, path.  Empty values are ignored
    (so the user can change just the name, leaving creds untouched)."""
    payload = load_config(base_dir)
    cameras = payload.get("cameras", [])
    target = None
    for cam in cameras:
    ...
    name = str(updates.get("name", "")).strip()[:80]
    ip   = str(updates.get("ip", "")).strip()[:64]
    user = str(updates.get("username", "")).strip()
    pwd  = str(updates.get("password", "")).strip()
    ...
    if name:
        target["name"] = name
    if ip:
        # Ensure no duplicate IP+path collision with another camera
        new_path = _normalize_path(str(path)) if path is not None else str(target.get("path", ""))
        for cam in cameras:
            if str(cam.get("id", "")) == str(camera_id):
                continue
            if str(cam.get("ip", "")) == ip and str(cam.get("path", "")) == new_path:
                return False, "Another camera already uses this IP + path"
        target["ip"] = ip
    if user:
    ...
    if pwd:
        target["password"] = pwd
    if path is not None:
        target["path"] = _normalize_path(str(path))
    if port is not None:
        try:
