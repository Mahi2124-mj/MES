...
export default function MaintenanceHistorical() {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const histQ = new URLSearchParams({ ...Object.fromEntries(winQ), limit: "10000" });
      ...
      // 2026-06-18 — sourced from the breakdown MASTER (mes_breakdown_log:
      // FY25-26 base + manual) instead of mes_breakdowns.  Same param names +
      // response shape, so the rest of the page renders unchanged.  The old
      // live auto-slips stay separate in mes_breakdowns (different FY, by design).
      const [s, h] = await Promise.all([
        api.get(`/api/breakdowns/log/stats?${winQ.toString()}`,  token).catch(() => ({})),
        api.get(`/api/breakdowns/log?${histQ.toString()}`, token).catch(() => ({})),
      ]);
      setStats({
    ...
    <>
      <style>{`
        ...
        @media (max-width: 600px) {
          .mh-daterange { flex-wrap: wrap; }
          .mh-daterange input.mh-input { width: 120px !important; min-width: 0; flex: 1 1 110px; }
        }
      ...
      <div className="mh-root">
        ...
        <div className="mh-body">
          ...
          <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center",
            ...
            <div className="mh-daterange" style={{ display:"flex", alignItems:"center", gap: 6 }}>
