...
import { ArrowLeft, Save, Plus, Pencil, Camera as CameraIcon, Cpu, Activity, Lock } from 'lucide-react'
...
export default function MachineDetail() {
  ...
  // Inline "Edit camera" panel — edits the currently-assigned camera in place
  const [editCamOpen, setEditCamOpen] = useState(false)
  const [editCam, setEditCam] = useState(EMPTY_CAMERA)
  const [savingEditCam, setSavingEditCam] = useState(false)
  ...
  // Open the edit panel pre-filled with the currently-assigned camera.
  // username/password start blank — leaving them blank keeps the existing
  // (encrypted) credentials, which are never shown back for security.
  const openEditCamera = () => {
    const cam = cameras.find(c => String(c.id) === String(form.nf2_camera_id))
    if (!cam) { toast.error('Select a camera to edit first'); return }
    setEditCam({
      name:     cam.name || '',
      ip:       cam.ip || '',
      port:     cam.port || 554,
      username: '',
      password: '',
      path:     cam.path || '/h264/ch1/main/av_stream',
    })
    setNewCamOpen(false)
    setEditCamOpen(true)
  }
  ...
  const saveEditCamera = async () => {
    if (!editCam.name.trim() || !editCam.ip.trim()) {
      toast.error('Camera name and IP are required')
      return
    }
    setSavingEditCam(true)
    try {
      await api.updateCamera(form.nf2_camera_id, {
        name: editCam.name.trim(),
        ip:   editCam.ip.trim(),
        port: Number(editCam.port) || 554,
        path: editCam.path.trim() || '/h264/ch1/main/av_stream',
        // Only send credentials if the admin actually typed new ones.
        ...(editCam.username.trim() ? { username: editCam.username } : {}),
        ...(editCam.password.trim() ? { password: editCam.password } : {}),
      })
      const refreshed = await api.getCameras()
      setCameras(refreshed)
      ...
      <Section title="Camera" icon={CameraIcon}>
        <div className="space-y-3">
          <div className="flex gap-2 items-end">
            ...
            <button type="button" onClick={openEditCamera} disabled={!form.nf2_camera_id}
              className="btn-secondary inline-flex items-center gap-1 whitespace-nowrap disabled:opacity-40 disabled:cursor-not-allowed">
              <Pencil size={14} /> Edit
            </button>
            <button type="button" onClick={() => { setEditCamOpen(false); setNewCamOpen(o => !o) }}
          ...
          {newCamOpen && (
            <div className="rounded-lg border border-dashed border-blue-300 dark:border-blue-700 bg-blue-50/30 dark:bg-blue-900/10 p-3 space-y-3">
              ...
              <div className="grid md:grid-cols-2 gap-2">
                ...
                <Field label="IP *"><input className="input-field font-mono" value={newCam.ip} onChange={e => setNewCam(c => ({ ...c, ip: e.target.value }))} placeholder="192.168.30.118" /></Field>
          ...
          {editCamOpen && (
            <div className="rounded-lg border border-dashed border-amber-300 dark:border-amber-700 bg-amber-50/30 dark:bg-amber-900/10 p-3 space-y-3">
              <p className="text-xs font-semibold text-amber-700 dark:text-amber-300">Edit camera — leave Username / Password blank to keep the existing (encrypted) credentials</p>
              <div className="grid md:grid-cols-2 gap-2">
                <Field label="Name *"><input className="input-field" value={editCam.name} onChange={e => setEditCam(c => ({ ...c, name: e.target.value }))} placeholder="e.g. Cell-3 Front" /></Field>
                <Field label="IP *"><input className="input-field font-mono" value={editCam.ip} onChange={e => setEditCam(c => ({ ...c, ip: e.target.value }))} placeholder="192.168.30.118" /></Field>
                <Field label="Port"><input type="number" className="input-field font-mono" value={editCam.port} onChange={e => setEditCam(c => ({ ...c, port: e.target.value }))} /></Field>
                <Field label="RTSP path"><input className="input-field font-mono" value={editCam.path} onChange={e => setEditCam(c => ({ ...c, path: e.target.value }))} /></Field>
                <Field label="Username (blank = keep)"><input className="input-field" value={editCam.username} onChange={e => setEditCam(c => ({ ...c, username: e.target.value }))} placeholder="unchanged" /></Field>
                <Field label="Password (blank = keep)"><input type="password" className="input-field" value={editCam.password} onChange={e => setEditCam(c => ({ ...c, password: e.target.value }))} placeholder="unchanged" /></Field>
              </div>
              <div className="flex justify-end gap-2">
                <button className="btn-secondary text-sm" onClick={() => { setEditCamOpen(false); setEditCam(EMPTY_CAMERA) }}>Cancel</button>
                <button className="btn-primary text-sm" onClick={saveEditCamera} disabled={savingEditCam}>
                  {savingEditCam ? 'Saving...' : 'Save Changes'}
                </button>
              </div>
            </div>
          )}
