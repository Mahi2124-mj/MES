...
def submachine_clip():
    ...
    # Render only if this exact window isn't already cached.  Render to a
    # unique work file, then os.replace() onto the keyed path so a concurrent
    # reader never sees a half-written clip.
    if not (os.path.exists(_out_path) and os.path.getsize(_out_path) > 0):
        _wfd, _work = _tempfile.mkstemp(prefix="cycclip_", suffix=".part.mp4", dir=_tmp_root)
        os.close(_wfd)

        # The ORIGINAL single-TS extract — kept as the GUARANTEED fallback so
        # this change can never make a clip worse than before.
        _single_cmd = [
            ffmpeg, "-y",
            "-fflags",     "+genpts+discardcorrupt",
            "-err_detect", "ignore_err",
            ...
            "-an",
            "-avoid_negative_ts", "make_zero",
            "-movflags", "+faststart",         # regular moov w/ the REAL duration
            "-f", "mp4",
            _work,
        ]
        ...
        try:
            ...
            if len(_cyc) >= 2:                  # cycle spans >1 TS file -> stitch
                ...
                with os.fdopen(_lfd, "w", encoding="utf-8") as _lf:
                    for _, _p in _cyc:
                        _lf.write("file '%s'\n" % _p.replace("\\", "/").replace("'", "'\\''"))
                _soff = max(0.0, (ts_naive_start - _first).total_seconds())
        ...
        _cmds = _cmds + [_single_cmd]

        # ffmpeg can exit non-zero on a benign trailing-packet warning yet still
        # write a playable clip, so success = "non-empty file exists".  Try the
        # stitch first; on empty/failed output fall through to the single-file.
        for _try_cmd in _cmds:
            try:
                subprocess.run(_try_cmd, stdout=subprocess.DEVNULL,
                               stderr=subprocess.DEVNULL, timeout=30)
            except Exception:
                pass   # timeout / spawn error -- try next candidate
