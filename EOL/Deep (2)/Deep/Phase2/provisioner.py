...
def _build_collector_script(line_cfg: dict) -> str:
    ...
    line_cfg keys:
      ...
      recliner_processes (list) — if present, a multi-process recliner line
    """
    # Recliner lines (N processes per machine, per-process cameras, machine
    # total = sum) run the ReclinerEngine profile instead of the base engine.
    if line_cfg.get("recliner_processes"):
        return _build_recliner_script(line_cfg)
...
def _build_recliner_script(line_cfg: dict) -> str:
    """ReclinerEngine collector — machine total = SUM of the per-process OK/NG
    registers; the single main OK/NG is blanked (recliner has no one main
    count).  Status/model/shift/break config is identical to a normal line."""
    ln, tbl, lid = line_cfg["line_name"], line_cfg["table_name"], line_cfg["line_id"]
    pip, pprt    = line_cfg["plc_ip"], line_cfg["plc_port"]
    st_a, md_a   = line_cfg["status_addr"], line_cfg["model_addr"]
    ict, mct     = line_cfg["ideal_ct"], line_cfg["max_ct"]

    models_literal = repr({m["model_number"]: m["model_name"] for m in line_cfg["models"]})
    status_literal = repr({s["status_code"]: {"name": s["status_name"], "loss": s.get("loss_type")} for s in line_cfg["status_map"]})
    breaks_literal = repr([{"start": b["start_time"], "end": b["end_time"], "name": b["break_name"]} for b in line_cfg["breaks"]])
    shifts_literal = repr({
        s["shift_name"]: {"start": s["start_time"], "end": s["end_time"],
                          "plan": s["total_plan"], "crosses_midnight": s["crosses_midnight"]}
        for s in line_cfg["shifts"] if s.get("is_production", False)
    })
    procs_literal = repr([
        {"process_no": p["process_no"], "ok_register": p.get("ok_register"),
         "ng_register": p.get("ng_register"), "count_mode": p.get("count_mode") or "register"}
        for p in line_cfg["recliner_processes"]
    ])

    return f'''#!/usr/bin/env python3
"""
AUTO-GENERATED RECLINER COLLECTOR — {ln}
Line ID    : {lid}
Table      : {tbl}
Generated  : {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}

Multi-process recliner line: machine total = sum of all processes.
DO NOT EDIT MANUALLY — regenerate via admin panel if config changes.
"""
...
from recliner_engine import ReclinerEngine
...
CONFIG = {{
    ...
    "breaks":     {breaks_literal},
    "shifts":     {shifts_literal},
    "recliner_processes": {procs_literal},
}}

if __name__ == "__main__":
    engine = ReclinerEngine(CONFIG)
    engine.run()
'''
...
def provision_line(line_id: int) -> dict:
    ...
    with get_conn() as conn:
        ...
        # Recliner processes (multi-process lines) — empty for normal lines.
        cur.execute(
            "SELECT process_no, process_name, count_mode, ok_register, ng_register "
            "FROM mes_recliner_processes WHERE line_id = %s AND is_active "
            "ORDER BY process_no",
            (line_id,)
        )
        recliner_procs = [dict(r) for r in cur.fetchall()]
        ...
        line_cfg = {
            ...
            "recliner_processes": recliner_procs,
        }
