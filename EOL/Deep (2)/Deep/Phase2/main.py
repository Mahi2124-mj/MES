...
from routers.prod_breakdown_slips import router as prod_bd_slips_router
from routers.escalation import router as escalation_router
from routers.timer_config import router as timer_config_router
from routers.hierarchy import router as hierarchy_router
from routers.leaders import router as leaders_router
from routers.panel import router as panel_router
from routers.device_registry import router as device_registry_router
from routers.push import router as push_router
from routers.hourly_sync      import start_hourly_sync
from routers.clip_browser   import router as clip_browser_router
from routers.peff            import router as peff_router
...
from routers.py_stations      import router as py_stations_router
from routers.py_config         import router as py_config_router
from routers.recliner         import router as recliner_router
from routers.sixsigma         import router as sixsigma_router
from routers.binfilling       import router as binfilling_router
from routers.comments_export  import router as comments_export_router
from routers.faults           import router as faults_router
from routers.logs           import router as logs_router
from routers.video_coverage import router as video_coverage_router
from routers.py_bypass import router as py_bypass_router
from routers.pm_status        import pm_status_router
...
app = FastAPI(
    ...
    version     = "2.0.0",
    # 2026-06-18 hardening: no Swagger / ReDoc / openapi schema in production.
    docs_url    = None,
    redoc_url   = None,
    openapi_url = None,
)
...
# ── Static files ───────────────────────────────────────────────
# 2026-07-05 SECURITY FIX: previously mounted the backend SOURCE dir at
# /static, which served /static/.env, /static/database.py, /static/auth.py
# etc. unauthenticated (live API keys + DB password + JWT secret leaked over
# HTTP).  Now serve ONLY a dedicated static/ subfolder of public assets
# (logo + icons).  Nothing else in the source tree is reachable via /static.
try:
...
app.add_middleware(
    CORSMiddleware,
    # 2026-06-18 hardening: restrict to known MES origins instead of "*".
    # The app is same-origin behind Caddy, so normal use is unaffected — this
    # just stops arbitrary external sites from scripting authenticated calls.
    allow_origins     = [
        "https://mes.tbdi.in", "https://app.tbdi.in",
        "https://192.168.1.181", "https://tbdi-mes.taild0ff20.ts.net",
        "https://100.79.110.97",
        "http://localhost:5656", "http://127.0.0.1:5656", "http://192.168.30.185:5656",
    ],
...
app.include_router(ui_timing_router)  # UI wait-time telemetry -> Waiting Time page
app.include_router(shift_compile_router)  # Shift Compile — line-leader shift review + sign-off
app.include_router(andon_router)          # Andon History — read-only view over maintenance_db.andon_history
app.include_router(prod_bd_slips_router)   # Production Breakdown Slip — production fills the andon auto-slip (maintenance_db)
app.include_router(escalation_router)      # Shift-end NG/alarm escalation — per-zone person chain + My Escalations
app.include_router(timer_config_router)    # Timer / Alerts config — inactivity, OEE, shift-close window, manpower alert timings
app.include_router(hierarchy_router)       # My Team — juniors down the escalation leg + activeness + leader line counts
app.include_router(leaders_router)         # Leader Allocation — assign leaders to lines + capability factor
app.include_router(panel_router)            # Panel-IP → auto-open dashboard mapping
app.include_router(device_registry_router) # Device registry — which phones/tablets/browsers run the app + version
app.include_router(push_router)            # Web Push (VAPID) — native phone notifications for escalations
app.include_router(clip_browser_router)  # back-day clip browser -> Historical > Video Archive
app.include_router(peff_router)          # PEFF check-sheet auto-fill feed (machines/operators/hourly)
...
app.include_router(py_stations_router)          # Poka-Yoke stations (Zone→Line→Machine code + station PY)
app.include_router(py_config_router)            # NEW model-aware PY config (line×machine×model×PY → desired) — parallel to old
app.include_router(recliner_router)             # Recliner zone: per-machine processes + per-process cameras
app.include_router(sixsigma_router)             # 6 Sigma: Ball Guide (Seat Slider) dual-camera clip review + 40-day retention
app.include_router(binfilling_router)           # Bin Filling: reverse-proxy the standalone BinVision app (:8090) as a native MES page
app.include_router(comments_export_router)       # Comments History → real .xlsx (POST the rows on screen)
app.include_router(faults_router)               # Fault Config: per-machine fault list (bit/register), zone/line/machine wise
app.include_router(pm_status_router)            # Per-line PM due/overdue status from the maintenance yearly plan (read-only)
app.include_router(logs_router)                  # Log Viewer: filtered tail of every log, by text / level / HTTP code
app.include_router(video_coverage_router)        # Video Coverage: clip per cycle + reason, read-only Video Agent
app.include_router(py_bypass_router)             # PY bypass: mail approve/reject, auto deviation, machine bit


# ══════════════════════════════════════════════════════════════════════
#  Background work runs in EXACTLY ONE API process
# ══════════════════════════════════════════════════════════════════════
# 2026-09-17 — Every startup hook below launches a background worker: the
# clip archiver, the manpower / kanban / OEE / loss-line alarm sweeps, the
# PM mail sender, the network + machine-master pollers, the schema
# migrations.  With `workers=1` that was safe by accident.
#
# Measured under 30 simulated supervisors: the single API process pins at
# ~104% of ONE core while 31 of the box's 64 cores sit idle, and even
# /api/auth/me — a JWT decode, no DB — takes seconds.  Raising the worker
# count is the fix for that, but naively raising it would ALSO run every
# worker above once PER process: clips rendered N times, N copies of each
# alert notification, N mails for the same PM, N migration races.
#
# So a Postgres session-level advisory lock elects one leader.  The lock is
# held on a dedicated connection for the life of the process and is released
# by the server the moment that process dies, so a crashed leader is replaced
# on the next boot with nothing to clean up.  Non-leaders serve requests only.
#
# MES_BACKGROUND=0 turns background work off in this process entirely — used
# when running a second instance for measurement, so it cannot double up on
# the live one.
# The election itself lives in bg_leader.py so the routers that start their own
# workers at IMPORT time (breakdown_mail, poka_yoke) can ask the same question
# without importing main and creating a cycle.
from bg_leader import is_leader as _background_leader


def bg_startup(fn):
    """Startup hook that must run in exactly one API process."""
    import functools

    @app.on_event("startup")
    @functools.wraps(fn)
    def _wrapped():
        if not _background_leader():
            return
        return fn()
    return _wrapped


# 2026-08-11 — pre-render + keep the clips operators actually open (over-target
# and Alarm cycles).  Everything else still renders on demand.  Best-effort: if
# the archive volume is missing the worker just logs and stays off.
@bg_startup
def _ensure_sa_fi_quality_schema():
    """SEAT SLIDER Semi-Auto↔Final trace: log table + per-machine config cols.

    Additive and idempotent, so a fresh DB gets the feature without a manual
    migration.  Its own try/except means a locked mes_plc_configs delays this
    one table, never the API boot.
    """
    try:
        from routers.sa_fi_quality import ensure_schema as _safi_schema
        _safi_schema()
...
@bg_startup
def _start_clip_archive():
    try:
        from routers.clip_archive import start as _clip_archive_start
        _clip_archive_start()
    except Exception as exc:
        print(f"[CLIP-ARCHIVE] not started: {exc}")
...
@bg_startup
def _start_hourly_sync():
    """Fill hour_*_actual from the ct_log for register-only lines (Loop Pipe)
    whose edge path can't feed the hourly buckets.  Counting untouched."""
    try:
        start_hourly_sync()
    except Exception as exc:
        print(f"[HOURLY-SYNC] failed to start: {exc}")
...
# ── Loss-line alarm background worker ────────────────────────────
@bg_startup
def _start_loss_line_alarm():
    """Inbox alert when a line stays in continuous BREAKDOWN (open andon call)
    during an active shift for >= LOSS_LINE_SUSTAIN_MIN minutes."""
    try:
        from loss_line_alarm import start as start_loss_line
        start_loss_line()
    except Exception as exc:
...
# ── Weld Monitor test-data generator ─────────────────────────────
@bg_startup
def _start_weld_test_gen():
    """Synthetic robot-weld readings (station 'Robot-1', source='test').

    2026-08-03 — now OFF by default.  The real feed is live: weld_poller.py
    reads the RC-01 robot's current shunt off the recliner-zone analog card
    over Modbus TCP, so the placeholder data is no longer needed (and mixing
    invented numbers into a quality screen is worse than an empty one).
    Set WELD_TEST_GEN=1 to bring the generator back for a UI demo."""
    if os.environ.get("WELD_TEST_GEN", "0") != "1":
        return
    try:
        import threading
        threading.Thread(target=weld_test_worker, daemon=True, name="weld-testgen").start()
    except Exception as exc:
        print(f"[WELD] failed to start test gen: {exc}")


# ── Weld Monitor LIVE feed (RC-01 robot, recliner zone) ──────────
@bg_startup
def _start_weld_poller():
    """Real weld current from the 8-channel analog card at 192.168.31.59
    (Modbus TCP), channel 8, 60 mV = 600 A.  One row per weld into
    mes_weld_log with source='live'.  Kill switch: WELD_POLLER=0."""
    try:
        from weld_poller import start_weld_poller
        start_weld_poller()
    except Exception as exc:
...
@bg_startup
def _start_clip_prefetch():
    """Pre-render the newest cycle clips of producing lines so a dot-click
    hits a finished file instead of paying ffmpeg extraction time."""
    try:
        from clip_prefetch import start_clip_prefetch
        start_clip_prefetch()
    except Exception as exc:
        print(f"[CLIP-PREFETCH] failed to start: {exc}")


# ── Switch-network SNMP poller ───────────────────────────────────
@bg_startup
def _start_network_poller():
    """Read-only SNMP poll of the seat-slider switch ring → /api/network/status."""
    try:
        start_network_poller()
    except Exception as exc:
        print(f"[NET-POLL] failed to start: {exc}")


@bg_startup
def _start_machine_master_poller():
    """ICMP-ping every machine + camera IP → /api/machine-master (green/red)."""
    try:
        start_machine_master_poller()
    except Exception as exc:
        print(f"[MMASTER] failed to start: {exc}")


@bg_startup
def _start_py_bypass_worker():
    """PY bypass cases: mail the quality team with Approve/Reject buttons,
    remind and escalate, create the deviation on approve, and close the case
    (and the machine bit) when the PY is OK again.  Reads the poka-yoke events
    table; the PY module itself is untouched.  PYB_WORKER=0 disables it."""
    try:
        from routers.py_bypass import start as _pyb_start
        _pyb_start()
    except Exception as exc:
        print(f"[PYB] failed to start: {exc}")


@bg_startup
def _start_py_sensor_fault_sync():
    """A PY sensor stuck past the threshold on a producing line is written to
    maintenance_db.maintenance_py_sensor_faults for the maintenance dashboard,
    and the row is removed as soon as the sensor works again.  Read-only on the
    poka-yoke side; PY_FAULT_SYNC=0 disables it."""
    try:
        from py_sensor_fault_sync import start as _py_fault_start
        _py_fault_start()
    except Exception as exc:
        print(f"[PY-FAULT] failed to start: {exc}")
...
def run_migrations():
    ...
    migrations = [
        ...
        # 2026-06-19 — Removed the dead per-line OT columns ot_start_a / ot_end_a /
        # ot_start_b / ot_end_b.  Nothing reads or writes them anymore (the old
        # /ot-config pair that used them was deleted; the canonical OT window now
        # lives in mes_shift_configs.ot_start_time/ot_end_time — see lines.py).
        # We no longer auto-create them on fresh DBs.  Any existing orphan
        # columns are harmless and left in place (cannot be auto-dropped safely).
        ...
        # 2026-06-19 — Manual OT target (pcs) per shift.  OT output has no
        # auto-computed plan (the OT hour bucket is created with plan_pieces=0),
        # so the production admin sets the OT target by hand in AdminPanel →
        # Lines → OT Config.  Read endpoints overlay this onto the OT slot's
        # plan/variance/efficiency at read time (collector stays untouched).
        "ALTER TABLE mes_shift_configs ADD COLUMN IF NOT EXISTS ot_plan INTEGER",
        # 2026-06-19 — ot_enabled flag used by the zone-wide OT toggle.  The
        # collector keys OT off mes_lines.ot_active_shift (the real switch); this
        # column is the zone editor's bookkeeping mirror so its shift upsert /
        # OT toggle SQL no longer references a non-existent column (was a 500).
        "ALTER TABLE mes_shift_configs ADD COLUMN IF NOT EXISTS ot_enabled BOOLEAN DEFAULT false",
...
def export_data(
    ...
    if format == "xlsx":
        # 2026-08-24 — operator reference layout: ONE sheet per shift, each a
        # transposed hourly block (Date/Shift/Zone/Line header, hours as
        # columns, then Plan / Actual / Cummulative Actual / Gap, then Losses).
        # Shared renderer with the shift-report download.  CSV below keeps the
        # flat wide layout (a 2-D block does not map to CSV).
        try:
            from openpyxl import Workbook
        except ImportError:
            raise HTTPException(500, "openpyxl not installed")
        from routers.reports import render_hourly_shift_sheet

        with get_conn() as _c:
            _cur2 = dict_cursor(_c)
            _cur2.execute("SELECT shift_name, ot_plan FROM mes_shift_configs "
                          "WHERE line_id=%s", (line_id,))
            _ot_map = {r["shift_name"]: r["ot_plan"] for r in _cur2.fetchall()}

        wb = Workbook()
        wb.remove(wb.active)                       # drop the default empty sheet
        _used = set()
        for rec in records:
            # Skip the between-shift GAP rows — only real A / B shifts get a
            # sheet (the flat CSV path still lists every row, unchanged).
            if str(rec.get("shift_name") or "").upper().startswith("GAP"):
                continue
            rec = dict(rec)
            rec["_line_id"]      = line_id
            rec["_report_date"]  = rec.get("record_date")
            rec["_report_shift"] = rec.get("shift_name")
            rec["_zone_name"]    = line.get("zone_name") or ""
            rec["line_name"]     = rec.get("line_name") or line["line_name"]
            rec["_ot_plan"]      = _ot_map.get(rec.get("shift_name"))
            _t = f"{rec.get('record_date')} {rec.get('shift_name') or ''}".strip()
            for _bad in "[]:*?/\\":
                _t = _t.replace(_bad, "-")
            _t = (_t or "Shift")[:31]
            _base = _t; _n = 1
            while _t in _used:
                _n += 1; _t = f"{_base[:27]}~{_n}"
            _used.add(_t)
            render_hourly_shift_sheet(wb.create_sheet(title=_t), rec)

        if not wb.sheetnames:                      # range held only GAP rows
            wb.create_sheet(title="No shift data")
        buf = io.BytesIO()
        wb.save(buf)
        buf.seek(0)
        ...
        # Multi-page PDF: one page per real shift (GAP rows skipped), same
        # hourly grid + total-loss Pareto as the xlsx.  Shared figure builder
        # with the single-shift download.
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        from matplotlib.backends.backend_pdf import PdfPages
        from routers.reports import _hourly_pdf_figure
        ...
        with PdfPages(buf) as pdf:
            ...
            for rec in records:
                ...
                rec["line_name"]     = rec.get("line_name") or line["line_name"]
                rec["_ot_plan"]      = _ot_map.get(rec.get("shift_name"))
                fig = _hourly_pdf_figure(rec)
                pdf.savefig(fig, bbox_inches="tight")
                plt.close(fig)
                pages += 1
            if pages == 0:
                fig = plt.figure(figsize=(11.7, 8.3))
                fig.text(0.5, 0.5, "No shift data for this range", ha="center")
...
def _get_ai_schema_info():
    ...
    try:
        with get_conn() as conn:
            ...
            # 2026-09-14 — Only list lines whose shift table ACTUALLY exists. A few
            # mes_lines rows point at a db_table_name that was never created; listing
            # them made the model emit `SELECT ... FROM <missing>` → "relation does
            # not exist" and a useless answer. to_regclass() returns NULL when absent.
            lines = []
            for l in all_lines:
                cur.execute("SELECT to_regclass(%s) AS r", (l["db_table_name"],))
                if cur.fetchone()["r"]:
                    lines.append(l)
            schema_info = "LINES (each line has its OWN shift table — query FROM that table):\n"
            for l in lines:
                schema_info += f"  {l['line_name']} -> {l['db_table_name']}\n"
            schema_info += (
                "\nEACH LINE TABLE = one row per (record_date, shift_name). Columns:\n"
                "  record_date (date), shift_name ('A'=day, 'B'=night; IGNORE 'GAP_%' rows),\n"
                "  shift_plan (planned qty), shift_plan_completed (produced qty on most lines),\n"
                "  ok_count (produced qty on register lines e.g. Loop Pipe; 0 on the rest),\n"
                "  ng_count (rejects), overall_oee, availability, performance, quality_oee (all %),\n"
                "  operating_status, cycle_time_actual, ct_avg_20, total_loss (seconds),\n"
                "  loss_breakdown_seconds, loss_quality_seconds, loss_setup_seconds,\n"
                "  loss_material_seconds, loss_others_seconds, loss_speed_seconds,\n"
                "  loss_change_over_seconds, current_model_name, current_model_number.\n"
                "PRODUCED qty = GREATEST(ok_count, shift_plan_completed); rejects = ng_count.\n"
                "These tables have NO line_id — choose the correct table from LINES above.\n"
                "POKA (ALL lines in one table, keyed by line_id — join mes_lines on id):\n"
                "  mes_poka_yoke_events(line_id, rule_type, alert_level, detected_at, "
                "acknowledged, plc_value, context_json);\n"
                "  mes_poka_yoke_rules(line_id, poka_yoke_name, bit, value)\n"
                "OTHER: mes_lines(id, line_name, db_table_name, zone_id, plant_id), "
                "mes_plants, mes_zones"
            )
...
async def ai_chat(request: Request, user=Depends(get_current_user)):
    ...
    # 2026-09-14 — OFFLINE-FIRST chatbot. Answer straight from the DB with a
    # rule-based intent parser (ai_offline.py) — no external LLM, zero cost, works
    ...
    if os.getenv("AI_OFFLINE", "1") != "0":
        try:
            ...
            print(f"[AI] offline error: {e}")   # fall through to the LLM path
    ...
    system_prompt = (
        ...
        "- SELECT only. Use record_date='YYYY-MM-DD' and shift_name='A' or 'B'.\n"
        "- To answer about a line, SELECT FROM that line's own table (see LINES).\n"
        "  e.g. SELECT shift_plan, ok_count, ng_count, shift_plan_completed, overall_oee\n"
        "       FROM <db_table_name> WHERE record_date='YYYY-MM-DD' AND shift_name='A';\n"
        "- Produced qty = GREATEST(ok_count, shift_plan_completed); rejects = ng_count.\n"
        "- Exclude GAP rows: add shift_name IN ('A','B') for production totals.\n"
        "- Across many lines? UNION ALL per line table, or query each — never invent a\n"
        "  combined table; there is none.\n"
    ...
    try:
        # 2026-08-27 — anthropic 0.39 + httpx 0.28 crash: the lib passes the
        # now-removed `proxies` kwarg to httpx.Client, so Anthropic() init threw
        # `TypeError: got an unexpected keyword argument 'proxies'` and EVERY
        # /api/ai/chat 500'd (the floating chatbot looked dead).  Supplying our
        # own http_client skips that internal path — key + model are fine.
        import httpx as _httpx
        # NB: give the client a generous timeout — httpx.Client() defaults to
        # only 5 s, which the Haiku call + tool loop can exceed, and anthropic
        # then surfaces the httpx read-timeout as "Connection error." (a 500).
        # anthropic's own default is 10 min; 90 s is plenty for the chat.
        client = anthropic.Anthropic(
            api_key=os.getenv("ANTHROPIC_API_KEY"),
            http_client=_httpx.Client(timeout=90.0),
        )
        msgs   = []
...
if __name__ == "__main__":
    ...
    print("=" * 65)
    # Kept in step with start_everything.sh and restart_api.py, which are the
    # two paths that actually launch the API in production (both go through
    # `-m uvicorn … --workers`).  This branch only runs if someone starts the
    # API with `python main.py` by hand.  See bg_leader.py for why more than
    # one worker is safe.
    uvicorn.run("main:app", host="0.0.0.0", port=8080, reload=False,
                workers=int(os.environ.get("MES_API_WORKERS", "4")))