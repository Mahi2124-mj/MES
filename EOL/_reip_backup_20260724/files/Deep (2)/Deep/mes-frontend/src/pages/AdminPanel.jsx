      ...
      {subPage === 1 && (
        <Card>
          ...
          {/* ── Seat-Slider load-cell NG traceability (2026-07-16) ─────
              Opt-in. Semi-Auto: result reg (1=OK/2=NG) + weight reg →
              logs NG barcode+weight to the zone NG list, clears on OK.
              Final: QR reg + reject bit (pulses ON 5s on NG-barcode match). */}
          <div style={{ borderTop:"1px solid #e2e8f0", marginTop:20, paddingTop:20 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              <h4 style={{ fontSize:13, fontWeight:700, color:"#0f172a", margin:0 }}>Load-cell NG (Seat Slider)</h4>
              <span style={{ fontSize:10, color:"#64748b" }}>Optional — blank = off. Semi-Auto: result+weight · Final: QR+reject bit.</span>
            </div>
            {(() => {
              const nm = (plcForm.machine_name || "").toUpperCase();
              const isSemi  = nm.includes("SEMI");
              const isFinal = nm.includes("FINAL") || (plcForm.parent_plc_id == null && !isSemi);
              return (
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))", gap:"10px 16px" }}>
                  {isSemi && <FF label="LC Result Reg (Semi-Auto)">
                    <input style={mini} type="text" placeholder="1=OK / 2=NG"
                           value={plcForm.lc_result_reg||""}
                           onChange={e=>setPlcForm(f=>({...f, lc_result_reg:e.target.value.trim().toUpperCase()}))}/>
                  </FF>}
                  {isSemi && <FF label="LC Value Reg (Semi-Auto)">
                    <input style={mini} type="text" placeholder="weight — optional"
                           value={plcForm.lc_value_reg||""}
                           onChange={e=>setPlcForm(f=>({...f, lc_value_reg:e.target.value.trim().toUpperCase()}))}/>
                  </FF>}
                  {isFinal && <FF label="QR Reg (Final)">
                    <input style={mini} type="text" placeholder="QR data register"
                           value={plcForm.qr_reg||""}
                           onChange={e=>setPlcForm(f=>({...f, qr_reg:e.target.value.trim().toUpperCase()}))}/>
                  </FF>}
                  {isFinal && <FF label="Reject Bit (Final)">
                    <input style={mini} type="text" placeholder="5s ON on NG match"
                           value={plcForm.reject_bit||""}
                           onChange={e=>setPlcForm(f=>({...f, reject_bit:e.target.value.trim().toUpperCase()}))}/>
                  </FF>}
                  {!isSemi && !isFinal && <div style={{ fontSize:11, color:"#94a3b8" }}>
                    Applies to Semi-Auto (result + weight) or Final (QR + reject bit) machines — machine name should contain "Semi" or "Final".
                  </div>}
                </div>
              );
            })()}
            <div style={{ fontSize:10, color:"#94a3b8", marginTop:8 }}>
              Semi-Auto NG (result=2) → barcode + weight logged to the zone NG list; OK (result=1) clears it.
              Final: scanned QR matched against the NG list → Reject Bit pulses ON 5s if the part is still NG.
            </div>
          </div>
