...
# 2026-07-21 — was 4 s.  Under heavy video-recorder load the CMS Flask
# worker is thread-starved, so this outbound call to the MES can take
# several seconds to complete even though the MES itself answers fast.
# A 4 s cap made "Save Camera Binding" time out AFTER the MES had already
# written the row — the endpoint then returned 502 and skipped the local
# binding sync, leaving the DB updated but no camera_config_bindings entry
# (so no video).  15 s absorbs that jitter.
_TIMEOUT_S = 15
