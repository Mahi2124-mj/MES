...
_NF2_ZONE_COL_READY = False


@once
def _ensure_nf2_zone_col(conn) -> None:
    """Add mes_lines.nf2_zone_name once per process.

    2026-08-14 — nf2_line_name could already override the LINE name, but the
    ZONE was always taken from mes_zones.  That is wrong for the lines whose
    machines are filed under a different zone in the machine master: the YWD /
    2UA lines sit in zone 'YWD/2UA' here while their machines live under
    'RECLINER' and 'TWO_UA' there, so no line name could ever have rescued them.
    This is the zone half of the same override.
    """
    global _NF2_ZONE_COL_READY
    if _NF2_ZONE_COL_READY:
        return
    try:
        conn.cursor().execute(
            "ALTER TABLE mes_lines ADD COLUMN IF NOT EXISTS nf2_zone_name VARCHAR(80)")
        conn.commit()
    except Exception:
        pass
    _NF2_ZONE_COL_READY = True
...
def _resolve_nf2_line(conn, line_id: int):
    ...
    Resolution order:
      1. mes_lines.nf2_zone_name / nf2_line_name (admin-set explicit override)
      2. fuzzy: first-token of mes_lines.line_name + zone match  → return that
      3. fall back to the raw mes_lines.line_name
    """
    _ensure_nf2_zone_col(conn)
    cur = dict_cursor(conn)
    cur.execute("""
        SELECT l.line_name, l.nf2_line_name, l.nf2_zone_name, z.zone_name
          FROM mes_lines l
    ...
    # The zone override applies to every branch below, including the fuzzy
    # match — a line pointed at another zone must search THAT zone.
    zone_name = (row.get("nf2_zone_name") or row["zone_name"] or "")
