...
export function useCameraFrame(cameraId, refreshMs = 2000) {
  ...
  useEffect(() => {
    ...
    // MJPEG stream via SAME-ORIGIN /live_feed → Caddy proxies to Flask :5555
    // with flush_interval -1 (no buffering → smooth continuous stream).
    // The old absolute http://127.0.0.1:5000 was dead (Flask is on 5555) AND
    // blocked as mixed-content on the HTTPS :8443 portal → forced the laggy
