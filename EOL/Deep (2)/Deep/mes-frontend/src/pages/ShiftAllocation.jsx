...
export default function ShiftAllocation() {
  ...
  const saveLeader = async (lid) => {
    ...
    setLeaderBusy(true);
    try {
      await api.post("/api/leaders/shift-alloc",
        { line_id: Number(lineId), shift_date: date, shift_name: shift,
          leader_id: lid ? Number(lid) : null }, token);
      setToastV(lid ? "Line leader assigned ✓" : "Line leader cleared");
      setTimeout(() => setToastV(null), 2500);
    } catch (e) { setToastV("Leader save failed"); setTimeout(() => setToastV(null), 3000); }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setLines(ls || []);
        if (!lineId && ls?.length) {
          // Restore the last line the user worked on (survives reload); the
          // list is already scoped to the user's assigned lines by the API.
          let saved = null;
          try { saved = localStorage.getItem("mes_sa_last_line"); } catch {}
          const found = saved && ls.find(l => String(l.id) === String(saved));
          setLineId(found ? found.id : ls[0].id);
        }
      } catch (e) { showToast(`Init failed: ${e.message}`, "err"); }
    })();
  }, [token]);

  // Remember the selected line across reloads.
  useEffect(() => {
    if (lineId) { try { localStorage.setItem("mes_sa_last_line", String(lineId)); } catch {} }
  }, [lineId]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      (allocs || []).forEach(a => {
        ...
        if (slot) { const i = slot.findIndex(x => x === null); if (i >= 0) slot[i] = a.operator_id; }
      });

      // Carry-forward: a day with no allocation yet gets the crew from the most
      // recent same-shift day earlier THIS week ("same person until changed").
      // For TODAY it's AUTO-SAVED (no daily manual save); for other empty days
      // this week it's a pre-filled suggestion (Save to commit). Carried
      // operators are merged into the pool so their chips render.
      let carried = null;          // set only when NOT auto-committed (suggestion)
      let poolNext = punches || [];
      if (!(allocs && allocs.length) && !(date < today)) {
        try {
          const c = await api.get(`/api/manpower/allocations/carry?line_id=${lineId}&date=${date}&shift=${encodeURIComponent(shift)}`, token);
          if (c && c.items && c.items.length) {
            const have = new Set(poolNext.map(p => p.id));
            const merged = [...poolNext];
            c.items.forEach(it => { if (!have.has(it.id)) { have.add(it.id); merged.push(it); } });
            poolNext = merged;
            c.items.forEach(a => { const slot = next[a.process_id]; if (slot) { const i = slot.findIndex(x => x === null); if (i >= 0) slot[i] = a.id; } });

            const rows = [];
            Object.entries(next).forEach(([pid, ops]) => ops.forEach(opId => { if (opId) rows.push({ process_id: Number(pid), operator_id: opId }); }));
            if (writable && date === today && rows.length) {
              // Auto-commit today's standing crew so no daily save is needed.
              try {
                await api.post("/api/manpower/allocations", {
                  line_id: Number(lineId), shift_date: date, shift_name: shift,
                  rows, allocated_by: (user?.username || "auto-carry"),
                }, token);
                showToast(`Same crew auto-carried from ${c.carried_from} ✓`);
              } catch (e2) { carried = c.carried_from || null; }  // failed → show as suggestion
            } else {
              carried = c.carried_from || null;                  // future day → suggestion
            }
          }
  ...
  const manualPunch = async () => {
    ...
    try {
      ...
      // Keep the modal OPEN so several operators can be added in a row;
      // just clear the picker and refresh the pool. Use "Done" to close.
      setPickOpId(""); setPickSearch("");
      refresh();
    } catch (e) { showToast(e.message, "err"); }
  };

  // Remove an operator from the pool (delete their punch).
  const removeFromPool = async (opId) => {
    const op = pool.find(x => x.id === opId);
    try {
      await api.delete(
        `/api/manpower/punches?operator_id=${opId}&line_id=${Number(lineId)}&date=${date}&shift=${encodeURIComponent(shift)}`,
        token);
      showToast(`${op?.full_name || "Operator"} removed from pool`);
      refresh();
    } catch (e) { showToast(e.message, "err"); }
  };
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 24, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {canLead && (
                <FilterCell label="Line Leader">
                  {(() => {
                    ...
                    const zoneId   = curLine?.zone_id ?? null;
                    const zoneName = curLine?.zone_name || "Leaders";
                    const text = l => `${l.username}${l.employee_code ? ` · ${l.employee_code}` : ""}`
                      + `${l.label && l.label !== "No data" ? ` · ${l.label}` : ""}${l.trained ? " ✓" : ""}`;
                    const own    = zoneId == null ? leaders : leaders.filter(l => l.zone_id === zoneId);
                    const others = zoneId == null ? [] : leaders.filter(l => l.zone_id !== zoneId);
                    // A leader already saved from another zone must stay visible.
                    const shownOthers = allZoneLeaders ? others
                      : others.filter(l => String(l.id) === String(shiftLeader));
                    const byZone = {};
                    shownOthers.forEach(l => { const k = l.zone_name || "No zone"; (byZone[k] = byZone[k] || []).push(l); });
                    return (
                      <>
                        <select value={shiftLeader} disabled={leaderBusy || !writable}
                                onChange={e => saveLeader(e.target.value)} style={{ ...glassSelect, minWidth: 220 }}>
                          <option value="">— none —</option>
                          <optgroup label={zoneName}>
                            {own.map(l => <option key={l.id} value={l.id}>{text(l)}</option>)}
                          </optgroup>
                          {Object.entries(byZone).map(([zn, arr]) => (
                            <optgroup key={zn} label={`${zn} — other zone`}>
                              {arr.map(l => <option key={l.id} value={l.id}>{text(l)}</option>)}
                            </optgroup>
                          ))}
                        </select>
                        {others.length > 0 && (
                          <label style={{ fontSize: 10, color: "#64748b", display: "flex", alignItems: "center",
                                          gap: 5, cursor: "pointer", userSelect: "none" }}>
                            <input type="checkbox" checked={allZoneLeaders}
                                   onChange={e => setAllZoneLeaders(e.target.checked)} />
                            Show leaders from other zones
                          </label>
                        )}
                      </>
                    );
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
              ...
              {!locked && carriedFrom && <Pill color="#0891b2">↩ Same crew carried from {carriedFrom} — review & Save to confirm</Pill>}
          ...
          <div style={{
            ...
            flexWrap: "wrap", justifyContent: "center", maxWidth: "100%", minWidth: 0,
            background: "#fff",
        ...
        <div
          ...
          <div className="col-scroll" style={{ padding: 10, flex: 1, overflowY: "auto", maxHeight: "calc(100vh - 340px)" }}>
            {pool.length === 0 ? (
              ...
              pool.map(op => (
                <div key={op.id} style={{ position: "relative", display: "inline-block" }}>
                  <OperatorCard
                    op={op}
                  ...
                  {(writable && !locked) && (
                    <button
                      onClick={() => removeFromPool(op.id)}
                      title="Remove from pool"
                      style={{
                        position: "absolute", top: -6, right: -6, width: 18, height: 18,
                        borderRadius: "50%", border: "2px solid #fff", background: "#ef4444",
                        color: "#fff", fontSize: 11, lineHeight: "14px", cursor: "pointer",
                        padding: 0, zIndex: 3, boxShadow: "0 1px 2px rgba(0,0,0,.3)",
                      }}
                    >×</button>
                  )}
                </div>
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
              <span style={{ fontSize: 10, color: "#94a3b8" }}>
                Add as many as you need — the list stays open. Click Done when finished.
              </span>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setAddModal(false)} style={primaryBtn}>Done</button>
                <button onClick={manualPunch} disabled={!pickOpId}
                        style={{ ...glassBtn, opacity: pickOpId ? 1 : 0.4, cursor: pickOpId ? "pointer" : "not-allowed" }}>
                  + Add to Pool
                </button>
              </div>
            </div>
