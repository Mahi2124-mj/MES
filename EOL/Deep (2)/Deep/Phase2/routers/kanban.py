...
def _backfill_pending_windows(now_dt: datetime) -> int:
    ...
    try:
        with get_conn() as conn:
            cur = dict_cursor(conn)
            # 2026-09-18 — Skip lines whose tables were never created
            # (YRA-SA-6WAY, LOCATION PIN STACKING - LPS-5).  _fire_window()
            # raised UndefinedTable on them; the except below swallowed it, but
            # the POOLED connection went back poisoned and the next borrower
            # failed with "current transaction is aborted" — an error that looks
            # like it belongs to whatever ran next.  to_regclass() keeps them out
            # of the loop entirely.  Same guard as wallboard-cycles and
            # hourly-sync; this was the third place with the same hole.
            cur.execute("""SELECT id FROM mes_lines
                            WHERE db_table_name IS NOT NULL
                              AND db_table_name <> ''
                              AND to_regclass(db_table_name) IS NOT NULL
                              AND to_regclass(db_table_name || '_ct_log') IS NOT NULL""")
